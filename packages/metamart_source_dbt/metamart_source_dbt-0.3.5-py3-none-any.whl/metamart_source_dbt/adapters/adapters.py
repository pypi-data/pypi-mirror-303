...
from metamart_source_dbt.loaders import AllDbtNodeTypes
from metamart_source_dbt.models.metamart import Column, Edge
from metamart_source_dbt.package_definitions import config
...
@multimethod
def build_metamart_metadata(current: Any, version: Any) -> None:
    """
    ...
    raise NotImplementedError(
        f"No objects of type `{type(current)}` have no implementation of `build_metamart_metadata` for version `{version}`."
    )
...
@build_metamart_metadata.register
def build_metamart_metadata_from_node(current: AllDbtNodeTypes, version: Literal["v1"] = "v1") -> TableMetadata:
    """
...
@build_metamart_metadata.register
def build_metamart_metadata_from_edge(current: Edge, version: Literal["v1"] = "v1") -> GenericEdgeMetadataV1:
    """
...
def adapt_table_to_client(current: AllDbtNodeTypes, source: SourceSpec, version: Literal["v1"]) -> SourcedNodeV1:
    ...
    """
    spec_dict = {
        "name": current.metamart_.full_name,
        "namespace": current.metamart_.namespace,
