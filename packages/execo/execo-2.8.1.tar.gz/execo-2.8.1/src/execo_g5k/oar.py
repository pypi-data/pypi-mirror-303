...
def get_oar_job_info(oar_job_id = None, frontend = None,
    ...
    process.run()
    job_info = dict()
    start_date_result = re.search("^\s*start(T|_t)ime = (\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s*$", process.stdout, re.MULTILINE)
    ...
    scheduled_start_result = re.search("^\s*scheduled(S|_s)tart = (\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s*$", process.stdout, re.MULTILINE)
