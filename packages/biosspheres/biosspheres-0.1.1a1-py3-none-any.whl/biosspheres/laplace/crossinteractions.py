...
def v_0_sj_semi_analytic_v1d(
    ...
    for i in np.arange(0, final_length):
        legendre_functions[:, i] = pyshtools.legendre.PlmON(
            big_l, cos_theta_coord[i], csphase=-1, cnorm=0
        )
        pass
    ...
    for i in np.arange(0, quantity_theta_points):
        for j in j_range:
            legendre_functions[:, i, j] = pyshtools.legendre.PlmON(
                big_l, cos_theta_coord[i, j], csphase=-1, cnorm=0
            )
            pass
        pass
...
def k_0_sj_semi_analytic_v1d(
    ...
    for el in eles[1:num]:
        temp_l[:] = -el * ratio ** eles_plus_1[el] / l2_1[el] * transform
        np.sum(
            temp_l * legendre_functions[l_times_l_plus_l_divided_by_2[el], :],
            axis=1,
...
def k_0_sj_from_v_0_sj(
    data_v: np.ndarray, r_j: float, el_diagonal: sparse.dia_array
) -> np.ndarray:
    """
    Returns a numpy array that represents a numerical approximation of
    the matrix formed by the boundary integral operator K_{s,j}^0 with
            ...
            if m < el:
                ...
                if m > 1:
                    ...
                    d_legendre_functions[:] = (
                        ...
                        * np.sqrt(el * (el + 1) * 2)
                        - legendre_functions[
                            l_times_l_plus_l_divided_by_2[el] + 2, :
                        ]
                        * np.sqrt((el - 1) * (el + 2))
                    ) * -0.5
...
def ka_0_sj_semi_analytic_recurrence_v2d(
    ...
    for el in eles:
        ...
        data_ka[p2_plus_p_plus_q, l_square_plus_l[el]] = int_ka[
            0, pesykus[:, 0], pesykus[:, 1]
        ]
        data_ka[p2_plus_p_minus_q, l_square_plus_l[el]] = int_ka[
            1, pesykus[:, 0], pesykus[:, 1]
        ]
        data_ka[l_square_plus_l, l_square_plus_l[el]] = int_ka[0, eles, 0]
        for m in np.arange(1, el + 1):
            if m < el:
                leg_aux[index] = (
                    m
                ...
                if m > 1:
                    ...
                    d_legendre_functions[:] = (
                        legendre_functions[
                            l_times_l_plus_l_divided_by_2[1], :, :
                        ]
                        * -1.0
                    )
                    phi_part = -np.sqrt(3 / np.pi) * 0.5 * ephi_times_n
            ...
            data_ka[p2_plus_p_plus_q, l_square_plus_l[el] + m] = int_ka[
                0, pesykus[:, 0], pesykus[:, 1]
            ]
            data_ka[p2_plus_p_minus_q, l_square_plus_l[el] + m] = int_ka[
                1, pesykus[:, 0], pesykus[:, 1]
            ]
            data_ka[l_square_plus_l, l_square_plus_l[el] + m] = int_ka[
                0, eles, 0
            ]
...
def ka_0_sj_from_v_sj(
    data_v_sj: np.ndarray, r_s: float, el_diagonal: sparse.dia_array
) -> np.ndarray:
    """
    Returns a numpy array that represents a numerical approximation of
    the matrix formed by the boundary integral operator K_{s,j}^{*0} with
...
def w_0_sj_from_v_sj(
    data_v_sj: np.ndarray, r_j: float, r_s: float, el_diagonal: sparse.dia_array
) -> np.ndarray:
    """
    Returns a numpy array that represents a numerical approximation of
    ...
    """
    data_w = np.matmul(
        (el_diagonal / -(r_j * r_s)).dot(data_v_sj), el_diagonal.toarray()
    )
    return data_w
    ...
    for el in eles:
        ...
        for m in m_range:
            ...
            data_v_sj[:, l_square_plus_l[el] + m] = (
                r_j * r_s**2 / l2_1[el] * data_v_sj[:, l_square_plus_l[el] + m]
            )
            np.sum(
                temp_l_m * sin_m_phi[m - 1, :],
                axis=1,
        ...
        if el > 0:
            ...
            data_v_js[l_square_plus_l[el] + m_range, :] = data_v_sj[
                :, l_square_plus_l[el] + m_range
            ].T
            data_v_js[l_square_plus_l[el] - m_range, :] = data_v_sj[
                :, l_square_plus_l[el] - m_range
            ].T
            ...
            data_ka_js[l_square_plus_l[el], :] = (
                data_k_sj_minus[:, l_square_plus_l[el]] * -1.0
            )
            data_ka_js[l_square_plus_l[el] + m_range, :] = (
                data_k_sj_minus[:, l_square_plus_l[el] + m_range].T * -1.0
            )
            data_ka_js[l_square_plus_l[el] - m_range, :] = (
                data_k_sj_minus[:, l_square_plus_l[el] - m_range].T * -1.0
            )
            ...
            data_k_js_minus[l_square_plus_l[el] + m_range, :] = (
                data_v_js[l_square_plus_l[el] + m_range, :] * diagonal / r_s
            )
            data_k_js_minus[l_square_plus_l[el] - m_range, :] = (
                data_v_js[l_square_plus_l[el] - m_range, :] * diagonal / r_s
            )
            ...
            data_ka_sj[:, l_square_plus_l[el] + m_range] = (
                data_k_js_minus[l_square_plus_l[el] + m_range, :].T * -1.0
            )
            data_ka_sj[:, l_square_plus_l[el] - m_range] = (
                data_k_js_minus[l_square_plus_l[el] - m_range, :].T * -1.0
            )
            ...
            data_w_sj[:, l_square_plus_l[el]] = (
                data_ka_sj[:, l_square_plus_l[el]] * el / r_j
            )
            data_w_sj[:, l_square_plus_l[el] + m_range] = (
                data_ka_sj[:, l_square_plus_l[el] + m_range] * el / r_j
            )
            data_w_sj[:, l_square_plus_l[el] - m_range] = (
                data_ka_sj[:, l_square_plus_l[el] - m_range] * el / r_j
            )

            data_w_js[l_square_plus_l[el], :] = data_w_sj[
                :, l_square_plus_l[el]
            ]
            data_w_js[l_square_plus_l[el] + m_range, :] = data_w_sj[
                :, l_square_plus_l[el] + m_range
            ].T
            data_w_js[l_square_plus_l[el] - m_range, :] = data_w_sj[
                :, l_square_plus_l[el] - m_range
            ...
            data_k_sj_minus[:, l_square_plus_l[el]] = 0.0
            data_ka_js[l_square_plus_l[el], :] = 0.0

            data_w_sj[:, l_square_plus_l[el]] = 0.0
            data_w_js[l_square_plus_l[el], :] = 0.0
        pass
    a_js = np.concatenate(
        (
...
def a_0_sj_and_js_v2d(
    ...
    for el in eles:
        ...
        data_v_sj[p2_plus_p_plus_q, l_square_plus_l[el]] = coefficients[
            0, pesykus[:, 0], pesykus[:, 1]
        ]
        data_v_sj[p2_plus_p_minus_q, l_square_plus_l[el]] = coefficients[
            1, pesykus[:, 0], pesykus[:, 1]
        ]
        data_v_sj[l_square_plus_l, l_square_plus_l[el]] = coefficients[
            0, eles, 0
        ]
        ...
        for m in m_range:
            ...
            data_v_sj[p2_plus_p_plus_q, l_square_plus_l[el] + m] = coefficients[
                0, pesykus[:, 0], pesykus[:, 1]
            ]
            data_v_sj[p2_plus_p_minus_q, l_square_plus_l[el] + m] = (
                coefficients[1, pesykus[:, 0], pesykus[:, 1]]
            )
            data_v_sj[l_square_plus_l, l_square_plus_l[el] + m] = coefficients[
                0, eles, 0
            ]
