...
class BseqJob:
    def __init__(
        ...
        self.bseq_for_xctph_link_cmds: str = \
f'''
'''
...
'''
    ...
    def create_inputs_bseq(self):
        ...
        for Qpt_idx, Qpt in enumerate(Qpts):
            ...
            self.bseq_for_xctph_link_cmds += f'ln -sf ../bseq/{dir_name} ./bseq_for_xctph/Q_{str(Qpt_idx).strip()}\n' 
            os.chdir(f'./{dir_name}')
    ...
    def create_job_bseq(self):
        ...
        job_bseq += self.bseq_for_xctph_link_cmds
...
for (( i=$start; i<$stop; i++ )); do
    ...
    qpt_idx=$i+1
    echo "Qpt index $qpt_idx out of $stop Qpts." >> ../../bseq.out
...
{plotxct_commands}
    ...
    echo "Total time since start of calculation: $elapsed_time seconds" >> ../../bseq.out
    echo "Exiting folder {folder_variable}" >> ../../bseq.out
    cd ../../
done

echo -e "\\n\\nDone bseq calculation\\n\\n" >> ./bseq.out
'''
