...
class GcsPatch(dicom_slide.BasePatch):
  ...
  def get_gcp_data_credential_header(
      ...
      bearer_token_cache: Optional[cachetools.LRUCache] = None,
  ) -> Dict[str, str]:
    """Returns the credential header patch requests."""
    return self.source.get_credential_header(credential, bearer_token_cache)
...
class GcsImage:
  ...
  def get_credential_header(
    ...
    Args:
      ...
      bearer_token_cache: Optional cache to store bearer token.

    Raises:
      InvalidCredentialsError: If credential factory is not initialized.
    """
    headers = {}
    crediental_source_hash = self._credential_factory.credential_source_hash()
    if (
        credentials is None
        and crediental_source_hash
        and bearer_token_cache is not None
    ):
      credentials = bearer_token_cache.get(crediental_source_hash)
    if credentials is None:
      credentials = self.credentials
    else:
      credential_factory_module.refresh_credentials(credentials)
    if crediental_source_hash and bearer_token_cache is not None:
      bearer_token_cache[crediental_source_hash] = credentials
