...
class Landing:
    ...
    def _check_bronze_ingestion(self):
        """
        Get the distinct Sourcefile values from the bronze table self._bronze_table_path
        via self._spark.sql() and check that self._file_name is not ingested already.
        If the file is ingested already, update the bronze table by setting IsDeleted column to 1
        where the file name matches the ingested file.
        """
        # Get the distinct Sourcefile values from the bronze table
    ...
    def put_bronze(self):
        ...
        try:
            ...
            print(f'Failed to add delta columns and cleaning names: {e}')
        ...
        if self._loadtype =="overwrite":
            ...
            #Just append the data with a new DbxCreated
            try:
