...
class Landing:
    ...
    def get_sharepoint_file(self, file, destination_filename=None):
        ...
        try:
            ...
            print(f'Could not replace Pythonlocation: {e}')
    ...
    def _check_bronze_ingestion(self):
        ...
        distinct_files_df = self._spark.sql(f"SELECT DISTINCT delta__sourcefile FROM {self._bronze_table_path}")

        # Check if the current file is already ingested
        existing_files = [row.delta__sourcefile for row in distinct_files_df.collect()]
        if self._file_name in existing_files:
            # Update the bronze table to mark the ingested file as deleted
            self._spark.sql(f"UPDATE {self._bronze_table_path} SET delta__deleted = 1 WHERE delta__sourcefile = '{self._file_name}'")
    ...
    def put_bronze(self):
        ...
        try:
            ...
            if file_extension in ['.xlsx', '.xls'] and self._excel_tabnames:
                if isinstance(self._excel_tabnames, list):
                    stage = stage.filter(stage['tab_name'].isin(self._excel_tabnames))
                else:
                    stage = stage.filter(stage['tab_name'] == self._excel_tabnames)
        except Exception as e:
        ...
        if not table_exists:
            ...
            # Update delta__deleted flag if the file had already been ingested.
