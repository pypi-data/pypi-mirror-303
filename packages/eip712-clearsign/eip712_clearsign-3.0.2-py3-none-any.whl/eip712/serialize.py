...
_MESSAGE_PREFIX_HASH: str = _serialize_int(183)
_FIELD_PREFIX_V1_HASH: str = _serialize_int(72)

_FIELD_PREFIX_V2_HASHES: dict[EIP712InstructionType, str] = {
    EIP712InstructionType.TOKEN: _serialize_int(11),
    EIP712InstructionType.AMOUNT: _serialize_int(22),
    EIP712InstructionType.DATETIME: _serialize_int(33),
    EIP712InstructionType.RAW: _FIELD_PREFIX_V1_HASH,
}


def _serialize_message_instruction(instruction: EIP712MessageInstruction, version: EIP712Version) -> str:
    return (
        _MESSAGE_PREFIX_HASH
...
def _serialize_field_instruction(instruction: EIP712FieldInstruction, version: EIP712Version) -> str:
    match version:
        ...
        case EIP712Version.V2:
            return (
                _FIELD_PREFIX_V2_HASHES[instruction.format]
                + _serialize_chain_id(instruction.chain_id)
                + _serialize_contract_address(instruction.contract_address)
                + instruction.schema_hash
                + _serialize_string(instruction.field_path)
