...
class Config:
    ...
    def __init__(self, main_section: str = "vitaleey", skip_dataclass: bool = False, skip_command_group: bool = False):
    ...
    @staticmethod
    def _parse_dataclass(dataclass, config):
        """
    ...
    def load(self, environment=None):
        ...
        if not self.skip_command_group:
            config = config.get(command_group, {})

        if environment:
            if environment_names.is_config_name(environment):
                main_command_config = self.remove_env_sections(config)
                config = config.get(environment, {})
                for k, v in main_command_config.items():
                    if k not in config:
                        config[k] = v
            else:
                raise click.ClickException(f"Environment {environment} is not valid")

        if self.skip_dataclass:
            return config

        dataclass = self.get_dataclass()

        for key in self.dataclass.__annotations__:
            if not config.get(key):
                config[key] = getattr(self.dataclass, key)
