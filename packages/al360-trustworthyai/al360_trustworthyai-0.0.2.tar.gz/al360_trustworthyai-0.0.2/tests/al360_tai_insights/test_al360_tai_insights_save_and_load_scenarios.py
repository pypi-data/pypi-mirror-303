...
class TestRAIInsightsSaveAndLoadScenarios(object):
    ...
    def test_al360_tai_insights_empty_save_load_save(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Load
            al360_tai_2 = AL360_TAIInsights.load(save_1)

            # Validate, but this isn't the main check
            validate_al360_tai_insights(
                al360_tai_2, X_train, X_test,
    ...
    def test_al360_tai_insights_save_load_add_save(self, manager_type, adult_data):
        ...
        save_load_add_al360_tai_insights(
    ...
    def test_load_missing_dirs(self, target_dir, adult_data):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Load
            al360_tai_2 = AL360_TAIInsights.load(save_1)
            assert al360_tai_2 is not None
    ...
    def test_loading_al360_tai_insights_without_model_file(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            model_pkl_path = Path(tmpdir) / \
                "al360_tai_insights" / SerializationAttributes.MODEL_PKL
            os.remove(model_pkl_path)
            with pytest.raises(Exception):
                AL360_TAIInsights.load(save_path)
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ...
                                              ManagerNames.COUNTERFACTUAL,
                                              ManagerNames.DATA_BALANCE])
    def test_al360_tai_insights_add_save_load_save(self, manager_type, adult_data):
        data_train, data_test, y_train, y_test, categorical_features, \
            continuous_features, target_name, classes, \
            feature_columns, feature_range_keys = adult_data
        X_train = data_train.drop([target_name], axis=1)
        ...
        if manager_type == ManagerNames.CAUSAL:
            ...
            al360_tai_insights._data_balance_manager.add(
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Validate, but this isn't the main check
            validate_al360_tai_insights(
                al360_tai_2, data_train, data_test,
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ...
                                              ManagerNames.COUNTERFACTUAL,
                                              ManagerNames.DATA_BALANCE])
    def test_al360_tai_insights_save_load_add_dropped_features(self, manager_type,
                                                         adult_data):
        data_train, data_test, y_train, y_test, categorical_features, \
            continuous_features, target_name, classes, \
            feature_columns, feature_range_keys = adult_data
...
def save_load_add_al360_tai_insights(
    manager_type,
    data_train,
    data_test,
    target_name,
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        # Call a single manager
        if manager_type == ManagerNames.CAUSAL:
            al360_tai_2.causal.add(
                treatment_features=['age', 'hours_per_week']
            )
        elif manager_type == ManagerNames.COUNTERFACTUAL:
            al360_tai_2.counterfactual.add(
                total_CFs=10,
                desired_class='opposite',
                feature_importance=False
            )
        elif manager_type == ManagerNames.DATA_BALANCE:
            al360_tai_2._data_balance_manager.add(
                cols_of_interest=categorical_features
            )
        elif manager_type == ManagerNames.ERROR_ANALYSIS:
...
def validate_al360_tai_insights(
    ...
    if feature_range_keys is not None:
        assert feature_range_keys.sort() == \
            list(al360_tai_insights._feature_ranges[0].keys()).sort()
    if feature_columns is not None:
        assert al360_tai_insights._feature_columns == (feature_columns or [])
    if feature_metadata is not None:
        # mismatch between categorical_features passed in AL360_TAIInsights
        ...
        assert al360_tai_insights._feature_metadata == feature_metadata
    assert target_column not in al360_tai_insights._feature_columns
