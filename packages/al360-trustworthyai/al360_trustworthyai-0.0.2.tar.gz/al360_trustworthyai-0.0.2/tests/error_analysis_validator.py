...
from al360_trustworthyai.exceptions import DuplicateManagerConfigException
...
def setup_error_analysis(al360_tai_insights, add_ea=True, max_depth=3):
    if add_ea:
        if al360_tai_insights.model is None:
            with pytest.raises(UserConfigValidationException,
                               match='Model is required for error analysis'):
                al360_tai_insights.error_analysis.add(max_depth=max_depth)
        else:
            al360_tai_insights.error_analysis.add(max_depth=max_depth)
            with pytest.raises(DuplicateManagerConfigException):
                al360_tai_insights.error_analysis.add(max_depth=max_depth)
    al360_tai_insights.error_analysis.compute()


def validate_error_analysis(al360_tai_insights, expected_reports=1):
    if al360_tai_insights.model is None:
        return
    reports = al360_tai_insights.error_analysis.get()
    assert isinstance(reports, list)
    assert len(reports) == expected_reports
    ea_info_list = al360_tai_insights.error_analysis.list()
    for idx, report in enumerate(reports):
        ...
        assert tree_features == al360_tai_insights.error_analysis._feature_names

        ea_x_dataset = al360_tai_insights.error_analysis._dataset
        ea_true_y = al360_tai_insights.error_analysis._true_y

        expected_count = len(ea_x_dataset)
        if matrix is not None:
            predictions = al360_tai_insights.model.predict(ea_x_dataset)
        ...
        num_features = len(al360_tai_insights.error_analysis._feature_names)
