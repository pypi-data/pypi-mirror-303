...
from al360_trustworthyai._interfaces import (CausalConfig, CausalData, CausalPolicy,
                                       CausalPolicyGains,
...
def validate_causal(al360_tai_insights, data, target_column,
                    treatment_features, max_cat_expansion):
    if al360_tai_insights.task_type == ModelTask.CLASSIFICATION and \
        ...
        with pytest.raises(UserConfigValidationException,
            ...
            al360_tai_insights.causal.compute()
        _check_causal_properties(al360_tai_insights.causal.list(),
    ...
    results = al360_tai_insights.causal.get()
    ...
    results = al360_tai_insights.causal.get_data()
    assert results is not None
    assert isinstance(results, list)
    assert len(results) == 1
    ...
    al360_tai_insights.causal.add(treatment_features,
