...
from al360_trustworthyai._interfaces import (EBMGlobalExplanation, FeatureImportance,
                                       ModelExplanationData,
                                       PrecomputedExplanations)
from al360_trustworthyai._internal.constants import ExplainerManagerKeys as Keys
from al360_trustworthyai._internal.constants import (ExplanationKeys, ListProperties,
...
class ExplainerManager(BaseManager):
    ...
    def _load(path, al360_tai_insights):
        ...
        inst.__dict__['_' + Metadata.MODEL] = al360_tai_insights.model
        inst.__dict__['_' + CLASSES] = al360_tai_insights._classes
        inst.__dict__['_' + CATEGORICAL_FEATURES] = \
            al360_tai_insights.get_categorical_features_after_drop()
        target_column = al360_tai_insights.target_column
        train = al360_tai_insights.get_train_data().drop(columns=[target_column])
        test = al360_tai_insights.get_test_data().drop(columns=[target_column])
