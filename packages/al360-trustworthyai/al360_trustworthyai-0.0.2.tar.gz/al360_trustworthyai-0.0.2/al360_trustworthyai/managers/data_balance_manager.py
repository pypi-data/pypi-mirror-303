...
from al360_trustworthyai._interfaces import TaskType
from al360_trustworthyai._internal.constants import DataBalanceManagerKeys as Keys
...
from al360_trustworthyai._tools.shared.state_directory_management import \
    DirectoryManager
from al360_trustworthyai.databalanceanalysis import (AggregateBalanceMeasures,
                                               DistributionBalanceMeasures,
                                               FeatureBalanceMeasures)
from al360_trustworthyai.databalanceanalysis.data_balance_utils import (
...
class DataBalanceManager(BaseManager):
    ...
    def _validate(self):
        ...
        if self._train is None and self._test is None:
            raise ValueError(
                (
                    "Either `train` or `test` must be provided to AL360_TAIInsights"
    ...
    def _save(self, path):
        ...
        # If manager is not added to AL360_TAIInsights, don't save any of its configs
        if not self._is_added:
            return
    ...
    def _load(path, al360_tai_insights):
        ...
        inst.__dict__["_train"] = al360_tai_insights.train
        inst.__dict__["_test"] = al360_tai_insights.test
        ...
        classes = (
            list(map(str, al360_tai_insights._classes))
            if al360_tai_insights._classes is not None
            else []
        )
        df = pd.concat([al360_tai_insights.train, al360_tai_insights.test])
