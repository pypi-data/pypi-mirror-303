...
from al360_trustworthyai._interfaces import CounterfactualData
from al360_trustworthyai._internal.constants import FileFormats
...
class TestCounterfactualAdvancedFeatures(object):
    ...
    def test_counterfactual_vary_features(
        ...
        al360_tai_insights.counterfactual.compute()

        cf_obj = al360_tai_insights.counterfactual.get()[0]
    ...
    def test_counterfactual_manager_save_load(self, tmpdir):
        ...
        counterfactual_config_list_before_save = \
            al360_tai_insights.counterfactual._counterfactual_config_list
        assert len(counterfactual_config_list_before_save) == 2
        assert len(al360_tai_insights.counterfactual.get()) == 2
        cf_obj_1 = al360_tai_insights.counterfactual.get()[0]
        assert cf_obj_1 is not None
        cf_obj_2 = al360_tai_insights.counterfactual.get()[1]
        assert cf_obj_2 is not None

        save_dir = tmpdir.mkdir('save-dir')
        al360_tai_insights.save(save_dir)
        al360_tai_insights_copy = AL360_TAIInsights.load(save_dir)

        counterfactual_config_list_after_save = \
            al360_tai_insights_copy.counterfactual._counterfactual_config_list
        assert len(al360_tai_insights_copy.counterfactual.get()) == 2
        cf_obj_1 = al360_tai_insights_copy.counterfactual.get()[0]
        assert cf_obj_1 is not None
        cf_obj_2 = al360_tai_insights_copy.counterfactual.get()[1]
        assert cf_obj_2 is not None
        ...
        with pytest.warns(UserWarning,
                          match='ERROR-LOADING-COUNTERFACTUAL-EXPLAINER: '
                                ...
                                'explainer.'):
            al360_tai_insights_copy_new = AL360_TAIInsights.load(save_dir)
        counterfactual_config_list = \
            al360_tai_insights_copy_new.counterfactual._counterfactual_config_list
    ...
    def test_counterfactual_manager_request_counterfactuals(
        ...
        query_id = \
            al360_tai_insights.counterfactual._counterfactual_config_list[0].id
        counterfactual_data = \
            al360_tai_insights.counterfactual.request_counterfactuals(
        ...
        query_id = \
            al360_tai_insights.counterfactual._counterfactual_config_list[1].id
        with pytest.raises(
    ...
    def test_counterfactual_manager_get_data(self):
        ...
        counterfactual_data = al360_tai_insights.counterfactual.get_data()
        self._verify_counterfactual_data(counterfactual_data)
