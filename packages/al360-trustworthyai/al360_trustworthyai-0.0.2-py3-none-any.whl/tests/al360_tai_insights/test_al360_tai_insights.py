...
class TestRAIInsights(object):
    ...
    def test_al360_tai_insights_iris(self, manager_type):
        ...
        for model in models:
            run_al360_tai_insights(model, X_train, X_test, LABELS, None,
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.ERROR_ANALYSIS,
                                              ...
                                              ManagerNames.EXPLAINER])
    def test_al360_tai_insights_cancer(self, manager_type):
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ManagerNames.ERROR_ANALYSIS,
                                              ManagerNames.EXPLAINER])
    def test_al360_tai_insights_binary(self, manager_type):
    ...
    def test_al360_tai_insights_binary_mixed_types(
        ...
        run_al360_tai_insights(model, data_train, data_test, target_name,
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ...
                                              ManagerNames.EXPLAINER,
                                              ManagerNames.COUNTERFACTUAL])
    def test_al360_tai_insights_no_model(self, manager_type):
    ...
    @pytest.mark.parametrize('manager_type', [ManagerNames.CAUSAL,
                                              ...
                                              ManagerNames.EXPLAINER,
                                              ManagerNames.ERROR_ANALYSIS])
    def test_al360_tai_insights_housing(self, manager_type):
        ...
        for model in models:
            run_al360_tai_insights(model, X_train, X_test, LABELS, [],
                             manager_type, manager_args)


def run_al360_tai_insights(model, train_data, test_data, target_column,
    ...
    if manager_type == ManagerNames.EXPLAINER:
        setup_explainer(al360_tai_insights)
    elif manager_type == ManagerNames.ERROR_ANALYSIS:
        setup_error_analysis(al360_tai_insights)

    validate_al360_tai_insights(al360_tai_insights, train_data, test_data, target_column,
    ...
    if manager_type == ManagerNames.CAUSAL:
        ...
        validate_causal(al360_tai_insights, train_data, target_column,
        ...
        feature_importance = manager_args.get(ManagerParams.FEATURE_IMPORTANCE)
        validate_counterfactual(al360_tai_insights,
                                desired_class=desired_class,
                                desired_range=desired_range,
                                feature_importance=feature_importance)
    elif manager_type == ManagerNames.ERROR_ANALYSIS:
        validate_error_analysis(al360_tai_insights)
    elif manager_type == ManagerNames.EXPLAINER:
        validate_explainer(al360_tai_insights, train_data, test_data, classes)
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        validate_al360_tai_insights(
            al360_tai_insights, train_data, test_data,
        ...
        if manager_type == ManagerNames.ERROR_ANALYSIS:
            ...
            setup_error_analysis(al360_tai_insights, max_depth=4)
            validate_error_analysis(al360_tai_insights, expected_reports=2)
        elif manager_type == ManagerNames.EXPLAINER:
            validate_explainer(al360_tai_insights, train_data, test_data, classes)
...
def validate_al360_tai_insights(
    al360_tai_insights,
    train_data,
    test_data,
    target_column,
    ...
    pd.testing.assert_frame_equal(al360_tai_insights.train, train_data)
    pd.testing.assert_frame_equal(al360_tai_insights.test, test_data)
    assert al360_tai_insights.target_column == target_column
    assert al360_tai_insights.task_type == task_type
    assert al360_tai_insights.categorical_features == (categorical_features or [])
    expected_length = 0
    if categorical_features is not None:
        expected_length = len(categorical_features)
    assert len(al360_tai_insights.categorical_features) == expected_length
    assert len(al360_tai_insights._categories) == expected_length
    assert len(al360_tai_insights._categorical_indexes) == expected_length
    assert len(al360_tai_insights._category_dictionary) == expected_length
    for ind_data in al360_tai_insights._string_ind_data:
    ...
    if feature_metadata is not None:
        if feature_metadata.dropped_features is not None and \
            ...
            assert len(al360_tai_insights.get_categorical_features_after_drop()) == \
        ...
        if categorical_features is not None:
            ...
            assert len(al360_tai_insights.get_categorical_features_after_drop()) == 0

    if al360_tai_insights.model is None:
        assert al360_tai_insights._predict_output is None
        assert al360_tai_insights._predict_proba_output is None
    else:
        assert al360_tai_insights._predict_output is not None
        if task_type == ModelTask.CLASSIFICATION:
            assert al360_tai_insights._predict_proba_output is not None
            assert isinstance(al360_tai_insights._predict_proba_output, np.ndarray)
            assert len(al360_tai_insights._predict_proba_output.tolist()[0]) == \
                len(al360_tai_insights._classes)

    if task_type == ModelTask.CLASSIFICATION:
        classes = train_data[target_column].unique()
        classes.sort()
        np.testing.assert_array_equal(al360_tai_insights._classes,
                                      classes)
