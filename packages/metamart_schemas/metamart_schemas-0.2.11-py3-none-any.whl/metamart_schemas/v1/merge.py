...
from metamart_schemas.v1.generics import Code
from metamart_schemas.v1.metadata.edges import BaseEdgeMetadataV1
from metamart_schemas.v1.metadata.nodes import BaseNodeMetadataV1
...
@merge.register
def merge_metamart_node_v1_metadata(metadata: BaseNodeMetadataV1, other_metadata: BaseNodeMetadataV1) -> BaseNodeMetadataV1:
    """Merge two metamart node metadata objects
...
@merge.register
def merge_metamart_edge_v1_metadata(metadata: BaseEdgeMetadataV1, other_metadata: BaseEdgeMetadataV1) -> BaseEdgeMetadataV1:
    """Merge two metamart edge metadata objects
...
@merge.register
def merge_metamart_node_into_node_metadata(spec: NodeSpec, source_spec: SourcedNodeSpec) -> NodeSpec:
    ...
    new_spec.metadata.metamart = merge(new_spec.metadata.metamart, source_spec.metadata.metamart)
    new_spec.metadata.sources[source_spec.data_source.name] = source_spec.metadata
    return new_spec


@merge.register
def merge_metamart_edge_into_edge_metadata(spec: EdgeSpec, source_spec: SourcedEdgeSpec) -> EdgeSpec:
    """Merge metamart metadata from a sourced edge into an edge spec
