...
class MetamartEncoder(JSONEncoder):
    """The default JSON encoder for Metamart.
...
encoder = MetamartEncoder()


def dump_json(v, *, default: Optional[Callable] = None) -> str:
    """Dump an object to JSON following Metamart's serialization rules

    This uses the MetamartEncoder to serialize objects to JSON.
    ...
    """
    if default is None:
        return json.dumps(v, cls=MetamartEncoder)
    else:
        return json.dumps(v, default=default, cls=MetamartEncoder)
...
class MetamartYamlSerializer:
    """A YAML serializer for Metamart

    The MetamartYamlSerializer provides a simple interface for serializing and deserializing YAML files which complies with Metamart's serialization rules.
    ...
    def dump(cls, item: Any, stream: Optional[Union[IO, str, Path]] = None) -> str:
        """Dump an object to YAML following Metamart's serialization rules
    ...
    def prep_data(data: Any) -> Union[str, List[str]]:
        """Ensures that the data is in a format compliant with Metamart's serialization rules.
