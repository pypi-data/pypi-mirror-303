...
class MetamartBaseModel(HashableBaseModel):
    """The base class for all Metamart models

    This class provides a number of features which are useful for Metamart models:
    * hashable - this allows Metamart models to be used as keys in dictionaries
    * update - this allows Metamart models to be updated with new values
    * json_loads - this allows Metamart models to be loaded from JSON
    * json_dumps - this allows Metamart models to be dumped to JSON

    In addition there is are pydantic specific configuration changes which enforce consistent behavior across Metamart Models:
    ...
    * allow_population_by_field_name - this allows Metamart models to be updated with new values by field name
    * orm_mode - this allows Metamart models to be used with ORMs
    ...
    def update(self, new_values: Dict) -> BaseModel:
        """Automatically update a Metamart model with new values

        Update uses the `merge` function to update the current model with new values.
        Merge understands the nested structure of Metamart models and will update nested models correctly.
...
class PlaceHolderSchema(MetamartBaseModel):
...
class DefaultValue(MetamartBaseModel):
...
class Metadata(MetamartBaseModel):
...
class MalformedMetadata(MetamartBaseModel):
