...
def insert_data_to_chart(html_data,
                         ...
                         return_file=True):
    p_list = [
        'chart_name', 'name', 'x_list', 'y_list', 'legend', 'series', 'smooth', 'y_min'
    ]
    y_min = 0
    ...
    if series is not None:
        y_min_this = None
        for s in series:
            y_min_temp = min(s['data'])
            y_min_this = y_min_this if y_min_this is not None else y_min_temp
            y_min_this = min(y_min_temp, y_min_this)
        y_min = y_min_this

    p_data_list = [
        name, name, x_list, y_list, legend, series, smooth, y_min
    ]
    h_r = []
