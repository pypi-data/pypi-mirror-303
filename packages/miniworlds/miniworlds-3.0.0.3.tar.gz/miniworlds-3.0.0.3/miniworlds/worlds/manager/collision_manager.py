...
class CollisionManager:
    ...
    def _handle_sensor_events(self):
        if self.world.event_manager.registered_events["sensor"] == set():
            return
        for target, methods in self.world.event_manager.registered_events[
            "sensor"
        ].items():
    ...
    def _handle_actor_detecting_actor_methods(self):
        for event in self.world.event_manager.class_events["on_detecting"]:
            registered_events_copy = list(
                self.world.event_manager.registered_events[event].copy()
            )
    ...
    def _handle_actor_not_detecting_actor_methods(self):
        for event in self.world.event_manager.class_events[
            "on_not_detecting"
        ]:  # first level
            for method in self.world.event_manager.registered_events[
                event
    ...
    def _handle_actor_detecting_border_methods(self):
        for event in self.world.event_manager.class_events["border"]:
            for method in self.world.event_manager.registered_events[event]:
    ...
    def _handle_actor_detecting_on_the_world_methods(self):
        methods = (
            self.world.event_manager.registered_events["on_detecting_world"]
            .copy()
            .union(
                self.world.event_manager.registered_events["on_detecting_world"].copy()
            )
        )
        for method in methods:
    ...
    def _handle_actor_detecting_not_on_the_world_methods(self):
        methods = (
            self.world.event_manager.registered_events["on_not_detecting_world"]
            .copy()
            .union(
        ...
        for method in methods:
            # get detect world method from actor
            is_not_on_the_world = not method.__self__.detect_world()
            # call listener if no world detected
            if is_not_on_the_world:
                method_caller.call_method(method, None)
        del methods
