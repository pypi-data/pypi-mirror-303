...
# 计算贝叶斯网络
# 输入：data:array，labels:list
# 输出：保存图片
def GES_BIC(data, labels, pic_name):
    print('正在计算贝叶斯网络，请稍等...')
    Record = ges(data)
    pyd = GraphUtils.to_pydot(Record['G'], labels=labels)  # 将因果图转换为pydot对象，以便进行可视化
    tmp_png = pyd.create_png(f="png")
    fp = io.BytesIO(tmp_png)  # 创建一个字节流对象，用于读取图像数据。
    ...
    # save the graph
    # pyd.write_png(pic_name)
    plt.show(block=True)  # 和环境中的某些包冲突，可能导致图像未响应