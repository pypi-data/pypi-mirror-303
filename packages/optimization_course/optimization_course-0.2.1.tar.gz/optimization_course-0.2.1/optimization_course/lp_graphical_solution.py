...
def plot_feasible_region(
    ...
    add_explanation: bool = False,  # New flag for explanation text
) -> None:
    ...
    add_explanation : bool
        Flag to display an explanatory text for accessibility. Default is False.
    ...
    constraint_lines = []
    constraint_colors = ["blue", "orange", "green", "purple", "red"]
    ...
    # Plot iso-profit lines (objective function contours)
    if plot_iso_profit:
        for z in range(1, 5):
    ...
    # Add explanation text for accessibility if the flag is set
    if add_explanation:
        explanation_text = (
            "This plot represents a graphical solution to a linear programming "
            "problem."
        )
        ...
        if plot_feasible:
            explanation_text += (
                "The shaded gray region represents the feasible region, "
                "where all the constraints are satisfied. "
                "It consists of the following corner points: "
                f"{', '.join([f'({x:.2f}, {y:.2f})' for x, y in feasible_points])}. "
            )

        # Constraints Explanation
        explanation_text += (
            "Each line corresponds to a constraint, with the following labels "
            "and colors:\n"
        )
        for i in range(num_constraints):
            explanation_text += (
                f"Constraint {i + 1}: {A[i][0]}x1 + {A[i][1]}x2 <= {b[i]} "
                f"(Color: {constraint_colors[i % len(constraint_colors)]})\n"
            )

        # Gradient Explanation
        if plot_gradient:
            explanation_text += (
                f"The blue arrow represents the gradient of the objective function, "
                "pointing in the direction of maximum increase. "
                f"The gradient is ({c[0]:.2f}, {c[1]:.2f}). "
            )

        # Optimal Solution Explanation
        if plot_optimal:
            explanation_text += (
                f"The green point indicates the optimal solution, which occurs at "
                f"({opt_x1:.2f}, {opt_x2:.2f}). "
            )

        # Iso-Profit Line Explanation
        if plot_iso_profit:
            explanation_text += (
                "The dashed lines are iso-profit lines, showing equal levels of the "
                "objective function value. "
                "One of the iso-profit lines, shown in green, passes through the "
                "optimal point."
            )
