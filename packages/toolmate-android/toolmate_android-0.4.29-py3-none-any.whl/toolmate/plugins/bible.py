...
try:
    ...
    config.standardBibleBooks = []
    for i in range(1, 67):
        abb, fullName = BibleBooks.abbrev["eng"][str(i)]
        config.standardBibleBooks.append(abb)
        config.standardBibleBooks.append(fullName)
    ...
    def bible(function_args):
        ...
        bookSuggestions = {i: None for i in config.standardBibleBooks}
        keyword_processor.add_keywords_from_list({f"`{i}`": bookSuggestions for i in config.uniquebible_platform.textList})
