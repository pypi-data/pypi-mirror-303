...
from pyastroweatherio.dataclasses import SunData, MoonData, DarknessData
...
class AstronomicalRoutines:
    ...
    async def sun_data(self) -> dict:
        ...
        sd = SunData(data=self._sun_data)
        try:
            SunDataModel.validate(sd)
            return sd
    ...
    async def moon_data(self) -> dict:
        ...
        md = MoonData(data=self._moon_data)
        try:
            MoonDataModel.validate(md)
            return md
        except ValidationError as ve:
    ...
    async def darkness_data(self) -> DarknessData:
        ...
        dd = DarknessData(data=self._darkness_data)
        try:
            DarknessDataModel.validate(dd)
            return dd
        except ValidationError as ve:
