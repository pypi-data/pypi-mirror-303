...
class AstroWeather:
    ...
    def _get_location(
        ...
        try:
            GeoLocationDataModel.validate(geolocation)
            return geolocation
    ...
    async def _get_atmosphere(self, details_seventimer, details_metno) -> AtmosphereData | None:
        ...
        try:
            AtmosphereDataModel.validate(atmosphere)
            return atmosphere
        except ValidationError as ve:
    ...
    async def _get_condition(
        ...
        try:
            ConditionDataModel.validate(condition)
            return condition
        except ValidationError as ve:
    ...
    async def _get_location_data(self) -> None:
        ...
        try:
            TimeDataModel.validate(time_data)
        except ValidationError as ve:
        ...
        try:
            LocationDataModel.validate(item)
            items.append(item)
        except ValidationError as ve:
    ...
    async def _get_forecast_data(self, forecast_type, hours_to_show) -> None:
        ...
        for metno_index, datapoint in enumerate(self._weather_data_metno):
            ...
            try:
                ForecastDataModel.validate(item)
                items.append(item)
            except ValidationError as ve:
        ...
        for day in range(0, len(start_indexes)):
            ...
            for index in range(
                ...
                if details_forecast.forecast_time.hour == sun_next_rising.hour or index >= (forecast_data_len - 1):
                    ...
                    try:
                        NightlyConditionsDataModel.validate(item)
                    except ValidationError as ve:
    ...
    async def _get_deepsky_objects(self) -> list:
        ...
        if self._weather_data_uptonight is not None:
            ...
            for row in range(len(dso_target_name)):
                ...
                try:
                    UpTonightDSODataModel.validate(item)
                    items.append(item)
                except ValidationError as ve:
    ...
    async def _get_bodies(self) -> list:
        ...
        if self._weather_data_uptonight_bodies is not None:
            ...
            for row in range(len(body_target_name)):
                ...
                try:
                    UpTonightBodiesDataModel.validate(item)
                    items.append(item)
                except ValidationError as ve:
    ...
    async def _get_comets(self) -> list:
        ...
        if self._weather_data_uptonight_comets is not None:
            ...
            for row in range(len(comet_target_name)):
                ...
                try:
                    UpTonightCometsDataModel.validate(item)
                    items.append(item)
                except ValidationError as ve:
