...
def get_experiment_dir_and_name(experiment_path: str) -> List[str]:
    client = MlflowClient(tracking_uri='databricks')
    experiment = client.get_experiment_by_name(experiment_path)
    if experiment is not None:
        raise ValidationError(
            f"Sweep cannot be created at the experiment path '{experiment_path}' because an experiment already"
            ' exists there. Please provide a new experiment path.')

    experiment_dir, experiment_name = os.path.split(experiment_path)
    client = WorkspaceClient()
    status = None

    try:
        status = client.workspace.get_status(experiment_dir)
    except Exception as e:
        raise ValidationError(
            f'Could not create new experiment at {experiment_path} because experiment directory {experiment_dir}'
            f' was not found. Please provide an experiment path with a valid directory in your workspace.') from e

    if status.object_type != ObjectType.DIRECTORY:
        raise ValidationError(
            f'Could not create new experiment at {experiment_path}. Expected {experiment_dir} to be a '
            f'directory but found {status.object_type} instead. Please provide an experiment path with a '
            'valid directory in your workspace.')
...
def validate_create_sweep_inputs(train_data_path: str,
