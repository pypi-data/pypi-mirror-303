...
from .log import log, log_destroy, getframeinfo, currentframe
from .property import CertifierPropMap, property_new, property_get, property_set, property_set_defaults_from_cfg_file, property_is_option_set
...
#         "file": str(getframeinfo(currentframe().f_back).filename),
#         "line": str(getframeinfo(currentframe().f_back).lineno),
#     }
...
def certifier_set_property(certifier: Certifier |  None, name: int, value):    
    ...
    TODO need to add assert error handling and verify what returned on error and success
    ...
    try:
        ...
        if (current_time < certificate.not_valid_before_utc):
            ...
            if not as_helper:
                ...
                error_message = "Certificate has expired. Not valid after: " + str(certificate.not_valid_after_utc)
                set_last_error(certifier, 12, gen_application_error_msg(error_message, None)) 
                assert False
            ...
            assign_last_error(certifier, CertifierError(output=CERTIFIER_REGISTRATION_STATUS_CERT_TIME_VALID))
