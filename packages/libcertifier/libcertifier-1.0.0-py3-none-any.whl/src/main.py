...
from .constants import CERTIFIER_OPT

def main():
    try:
        arg_parser = cli_setup()
        args = arg_parser.parse_args(argv[1:] or ['--help'])
...
def xpki_perform(args):
    ...
    if (args.command == 'get-cert-validity'):
        ...
        if validity:
            validity = XPKI_CLIENT_CERT_STATUS(validity).name

        last_error = get_last_error(certifier)
        return_code = last_error.application_error_code + last_error.library_error_code

        certifier_create_info(last_error, return_code, validity)
