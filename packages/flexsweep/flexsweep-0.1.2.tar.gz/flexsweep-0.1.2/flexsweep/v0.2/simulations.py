        ...
        if (
            ...
            if self.sweep_class == "hard":
                ...
                for end, start in iterables:
                    ...
                    if (
                        round(end, 1)
                        not in self.sweep_diffusion_time[current_sweep]
                    ):
...
def simulate(
    ...
    if output_folder is not None:
        ...
        if keep_simulations:
            ...
            simulated_list = np.array([x.replace(output_folder+'/sweeps/', '').replace('sweep_','').replace('.trees','') for x in simulated_list],dtype=int).max()
            
            simulated_list = np.arange(simulated_list+1,simulated_list+sweep_replicas+1)
    ...
    if _errors != 0:
        ...
        for i in range(max_tries):
            ...
            sims_tries, sims_k = retry_simulation(parameters,simulated_list,contig,model,recombination_map,slim_region,timeout,nthreads,output_folder)
...
def pool_exec(
    ...
    sims_p = Parallel(
        n_jobs=nthreads, batch_size=batch_size, verbose=2
