...
def calculate_stats(
    ...
    try:
        h12_v = h12_enard(ts, rec_map, window_size=int(5e5) if neutral else int(1.2e6))
        h12_v = run_h12(ts, rec_map, _iter=_iter, neutral=neutral)
    except:
        h12_v = np.nan
    ...
    df_window = pd.DataFrame(
        [[_iter, int(6e5), int(1e6), int(5e5), 1.0, h12_v, haf_v]],
...
def summary_statistics(
    ...
    Parameters
    ----------
    sims : list or iterable
        A list or iterable containing the simulation/empirical data results, each representing
        haplotype data to compute summary statistics from.
    output_file: str
        File name to write the dataframe into a parquet format.
    ...
    neutral_save : str or None, optional (default=None)
        A file path to save the neutral simulation results. If None, the neutral
        results are not saved or loaded. If specified, the neutral simulation
    ...
    # Opening neutral exepctation
    if neutral_save is not None:
        assert neutral_save.endswith(".pickle"), "Please use pickle format"
        with open(neutral_save, "wb") as handle:
            neutral_norm = pickle.load(handle)
        # If neutral stats opened, ensure neutral stats are not estimated again
        sims["neutral"] = []
    else:
        neutral_norm = None
    ...
    df_fv_sweep, neutral_norm = normalization(
        sweeps_stats, neutral_stats_norm, neutral_values=neutral_norm, nthreads=nthreads
    )

    # Save neutral expectations
    if (neutral_save is not None) and (os.path.exists(neutral_save) is False):
        with open(neutral_save, "wb") as handle:
            pickle.load(neutral_norm, handle)
    ...
    df_fv_w.to_parquet(output_file)
