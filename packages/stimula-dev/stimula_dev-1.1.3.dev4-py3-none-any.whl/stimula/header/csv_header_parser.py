...
class HeaderParser(Parser):
    ...
    def parse_csv(self, header):
        ...
        primary_key = self.find_primary_key(table)
        ...
        return {'table': table.name, 'primary-key': primary_key, 'columns': parsed_cells}
    ...
    def find_primary_key(self, table):
        ...
        # assert there's exactly one primary key
        assert len(primary_keys) == 1, 'Only tables with a single primary key are supported'
