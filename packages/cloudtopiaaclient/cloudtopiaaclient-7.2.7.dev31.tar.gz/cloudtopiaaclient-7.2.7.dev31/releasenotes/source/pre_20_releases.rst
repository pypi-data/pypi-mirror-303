...
* Add an --ct-endpoint-type cli optional argument to be able to select endpoint interface type
...
* Openstack --ct-image-api-version 2 image show <image_id> fails
...
* Add global ``--ct-cloud`` option to select from a list of cloud configurations.
...
* add authentication via ``--ct-trust-id`` for Identity v3
