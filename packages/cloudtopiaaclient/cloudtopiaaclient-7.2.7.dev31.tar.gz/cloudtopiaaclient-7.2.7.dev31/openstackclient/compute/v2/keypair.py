...
class CreateKeypair(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                'Keypair type '
                '(supported by --ct-compute-api-version 2.2 or above)'
            ),
        )
        parser.add_argument(
            '--user',
            ...
            help=_(
                ...
                '(supported by --ct-compute-api-version 2.10 or above)'
            ),
        )
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.type:
            if not sdk_utils.supports_microversion(compute_client, '2.2'):
                msg = _(
                    '--ct-compute-api-version 2.2 or greater is required to '
        ...
        if parsed_args.user:
            if not sdk_utils.supports_microversion(compute_client, '2.10'):
                msg = _(
                    '--ct-compute-api-version 2.10 or greater is required to '
...
class DeleteKeypair(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                'The owner of the keypair. (admin only) (name or ID). '
                'Requires ``--ct-compute-api-version`` 2.10 or greater.'
            ),
        )
        identity_common.add_user_domain_option_to_parser(parser)
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.marker:
            if not sdk_utils.supports_microversion(compute_client, '2.35'):
                msg = _(
                    '--ct-compute-api-version 2.35 or greater is required '
