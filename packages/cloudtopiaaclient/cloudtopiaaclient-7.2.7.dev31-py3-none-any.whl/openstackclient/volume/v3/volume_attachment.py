...
class CreateVolumeAttachment(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                ...
                'settings '
                '(supported by --ct-volume-api-version 3.54 or later)'
            ),
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if not sdk_utils.supports_microversion(volume_client, '3.27'):
            msg = _(
                "--ct-volume-api-version 3.27 or greater is required to "
        ...
        if parsed_args.mode:
            if not sdk_utils.supports_microversion(volume_client, '3.54'):
                msg = _(
                    "--ct-volume-api-version 3.54 or greater is required to "
...
class CompleteVolumeAttachment(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        if not sdk_utils.supports_microversion(volume_client, '3.44'):
            msg = _(
                "--ct-volume-api-version 3.44 or greater is required to "
