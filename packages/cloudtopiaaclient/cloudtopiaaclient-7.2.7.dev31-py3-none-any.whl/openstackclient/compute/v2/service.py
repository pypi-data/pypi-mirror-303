    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                ...
                "``--ct-compute-api-version`` 2.53 or greater, the ID is "
...
Using ``--ct-compute-api-version`` 2.53 or greater will return the ID as a UUID
...
class SetService(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        up_down_group.add_argument(
            ...
            action='store_true',
            help=_(
                'Force up service. Requires ``--ct-compute-api-version`` '
                '2.11 or greater.'
            ),
        )
        up_down_group.add_argument(
            ...
            action='store_true',
            help=_(
                'Force down service. Requires ``--ct-compute-api-version`` '
                '2.11 or greater.'
            ),
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if force_down is not None:
            if not sdk_utils.supports_microversion(compute_client, '2.11'):
                msg = _(
                    '--ct-compute-api-version 2.11 or later is ' 'required'
                )
                raise exceptions.CommandError(msg)
            try:
