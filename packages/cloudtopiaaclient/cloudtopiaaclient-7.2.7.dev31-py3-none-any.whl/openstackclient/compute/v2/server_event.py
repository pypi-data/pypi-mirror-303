...
class ListServerEvent(command.Lister):
    ...
    Specify ``--ct-compute-api-version 2.21`` or higher to show events for a
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                ...
                "(supported with --ct-compute-api-version 2.58 or above)"
            ),
        )
        parser.add_argument(
            '--changes-before',
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.marker:
            if not sdk_utils.supports_microversion(compute_client, '2.58'):
                msg = _(
                    '--ct-compute-api-version 2.58 or greater is required to '
...
class ShowServerEvent(command.ShowOne):
    ...
    Specify ``--ct-compute-api-version 2.21`` or higher to show event details
    for a deleted server, specified by ID only. Specify
    ``--ct-compute-api-version 2.51`` or higher to show event details for
    non-admin users.
    """
