...
class NxLog:
    ...
    a folder named ``nxlog/`` relative to the executing script. The output path
    ...
    Args:
        ...
        filename (str, optional): The NxLog filename. If none is given, a
            unique serially numbered filename is created. The ``.nxlog``
    ...
    _parent = None
    _log_level = None
    _default_output_path = Path("nxlog/")
    ...
    def _get_filepath(self):
        ...
        if not filepath.suffix == ".nxlog":
            filepath = filepath.with_suffix(".nxlog")
        return filepath
