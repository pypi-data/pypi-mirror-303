...
# -----------------------------------------------------------------------------------------------------------------
async def check_index(_pathm, _opt: str):
    option_map = {
...
def compare_index(_baseA, _currB):
...
# -----------------------------------------------------------------------------------------------------------------
async def res_out(_pathm, _trx, _kv, _opt: str):
    query_map = {
...
# -----------------------------------------------------------------------------------------------------------------
async def month_load(_pathm, _idxmonth: str, _opt: str):
    query_map = {
...
# -----------------------------------------------------------------------------------------------------------------
# 이건 현재 stage 를 파악하는것임
async def job_exists(_pathm, _idxmonth: str, _opt: str):
    query_map = {
        AF_BRONZE: _pathm.query_bronze_yearmonth,
        AF_SILVER: _pathm.query_silver_yearmonth,
        AF_GOLD_S01: _pathm.query_gold01_yearmonth,
...
async def job_list(_pathm, _resume_month, _opt: str):
    ...
    if not target_index:
        raise ValueError(f'Invalid option provided: {_opt}')
    trx, kv_store = await check_index(_pathm, target_index)
    res_item = await res_out(_pathm, trx, kv_store, target_index)
...
async def diff_stage(_pathm, _opt: str):
    ...
    async def load_res(_index):
        trx, kv_store = await check_index(_pathm, _index)
        return await res_out(_pathm, trx, kv_store, _index)
    res_a = await load_res(idx_a)
    res_b = await load_res(idx_b)
    partial, job, complete = compare_index(res_b, res_a)
