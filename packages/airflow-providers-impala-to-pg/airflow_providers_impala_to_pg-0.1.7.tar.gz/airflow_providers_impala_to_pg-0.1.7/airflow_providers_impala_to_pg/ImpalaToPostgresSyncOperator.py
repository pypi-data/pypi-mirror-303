...
class ImpalaToPostgresSyncOperator(BaseOperator):
    ...
    def extract_impala_table(sql_query):
        match = re.search(r'select\s+\*\s+from\s+([^\s;]+)', sql_query, re.IGNORECASE)
        if match:
            return match.group(1)
        return None

    def get_impala_table_columns(impala_cursor, table_name):
        impala_cursor.execute(f"DESCRIBE {table_name}")
        columns = [row[0] for row in impala_cursor.fetchall()]
        return columns
    ...
    def extract_columns(self, impala_cursor, sql_query: str):
        ...
        if match:
            ...
            if columns_str.strip() == '*':
                # Handle SELECT * case
                impala_table_name = self.extract_impala_table(sql_query)
                columns = self.get_impala_table_columns(impala_cursor, impala_table_name)
                logging.info(f"Query uses SELECT *, fetching column names for table: {impala_table_name}")
            else:
