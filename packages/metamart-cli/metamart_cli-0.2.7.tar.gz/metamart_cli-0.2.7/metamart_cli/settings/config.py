...
class EnvironmentVariables:
    config_file = "METAMART_CLI_CONFIG"
...
def default_config_paths():
    ...
    if operating_system == "Darwin":
        ...
        config_path = [f"{home_path}/.config/metamart"]
    elif operating_system == "Linux":
        home_path = os.path.expanduser("~")
        config_path = [f"{home_path}/.config/metamart"]
    elif operating_system == "Windows":
        home_path = os.getenv("APPDATA")
        config_path = [f"{home_path}/metamart"]
...
def load_warning():
    message = (
        ...
        f"1) The config file is malformed, in which case you can edit it or execute `metamart config init` to recreate it.\n"
...
class BaseMetamartConfig(LazyConfig):
    class Config:
    ...
    def load(self, filename: Optional[str] = None, with_warning=True) -> None:
        try:
            ...
            load_warning()
            default_config = str(pkg_resources.files("metamart_cli") / "config_default.yaml")
