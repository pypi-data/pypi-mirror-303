...
from metamart_cli.api.config.setup import config_app
from metamart_cli.settings.config import (
    BasicAuthSettings,
    MetamartConfig,
    ServerSettingsV1,
    config,
)
from metamart_cli.utilities.styling import MetamartColors, default_styler
from metamart_cli.utilities.styling import print as print_styled
from metamart_cli.utilities.validators import (
    password_callback,
    url_callback,
    username_callback,
    workspace_callback,
...
class InitDefaults:
    ...
    def url_default(cls):
        try:
            ...
            default_url = "https://app.metamart.io"
...
def cli_init_config(
    ...
    config = MetamartConfig(auth=auth, server=server)
    config.save()
