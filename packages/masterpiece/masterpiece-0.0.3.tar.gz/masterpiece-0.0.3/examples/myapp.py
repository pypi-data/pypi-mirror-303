...
class MyHome(Application):
    ...
    # Class attributes can be configured in two ways: either through class configuration files
    # (.myapp/config/*.json), automatically created for each class when the application is first started,
    # or via startup arguments. If both are defined, the startup arguments take precedence.
    solar_plant: float = 0
    ...
    def create_home(self):
        """Create a default home structure. This can be overwritten
        by the instance hierarchy defined in the serialization file."""
