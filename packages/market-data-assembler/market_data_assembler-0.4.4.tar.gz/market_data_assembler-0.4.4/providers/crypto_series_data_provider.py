...
class CryptoSeriesDataProvider:
    ...
    def _load_for_day(self, instrument: str, target_day: datetime) -> None:
        ...
        print(f"current thread: {threading.current_thread().name} loaded ohlc_series {instrument}, {target_day}")
        trades = self.trades_loader_class(target_day, instrument).get_trades()
        print(f"current thread: {threading.current_thread().name} loaded trades {instrument}, {target_day}")
        grouped_trades = self._group_trades(trades, target_day)
        print(f"current thread: {threading.current_thread().name} grouped_trades {instrument}, {target_day}")
        updated_series = self._update_ohlc_with_trades(ohlc_series, grouped_trades)
        print(f"current thread: {threading.current_thread().name} updated_series {instrument}, {target_day}")
        book_depths = []
        if self.book_depth_loader_class:
            book_depths = self.book_depth_loader_class(target_day, instrument).get_book_depths()
            print(f"current thread: {threading.current_thread().name} loaded book_depths {instrument}, {target_day}")
            grouped_book_depths = self._group_book_depths(book_depths, target_day)
            print(f"current thread: {threading.current_thread().name} grouped_book_depths {instrument}, {target_day}")
        ...
        print(
            f'{instrument}: saved {archive_name}, ohlc_series: {len(ohlc_series)}, trades: {len(trades)}, book_depths: {len(book_depths)}')
