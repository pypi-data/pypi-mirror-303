...
from .utils import arrayindex, dot, emitter, norm, receiver
...
class Response(abc.ABC):
    ...
    def compute_gw_response(self, t: ArrayLike, link: ArrayLike = LINKS) -> np.ndarray:
        ...
        The link esponses are expressed as dimensionless relative frequency
    ...
    def _interpolate_tps(self, tau: ArrayLike, sc: ArrayLike = SC) -> np.ndarray:
        ...
        def interpolate(t, data) -> InterpolatedUnivariateSpline:
    ...
    def write(
        ...
        t0: float = 0,
        mode: str = "a",
        timeframe: Literal["tps"] | Literal["tcp"] | Literal["both"] = "both",
    ) -> None:
...
class ReadResponse(Response):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.interpolants = {
            link: InterpolatedUnivariateSpline(
                t_interp, data[link], k=self.interp_order, ext="zeros"
            )
...
class ReadStrain(ResponseFromStrain):
    ...
    def set_strain(
        ...
        self.hplus = InterpolatedUnivariateSpline(
            t_interp, hplus, k=self.strain_interp_order, ext="zeros"
        )
        self.hcross = InterpolatedUnivariateSpline(
            t_interp, hcross, k=self.strain_interp_order, ext="zeros"
        )
