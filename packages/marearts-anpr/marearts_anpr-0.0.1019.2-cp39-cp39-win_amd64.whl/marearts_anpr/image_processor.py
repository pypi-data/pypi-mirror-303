...
class CustomImageProcessor:
    def __init__(self, cfg):
        ...
        self.expected_keys = ['crop_size', 'do_center_crop', 'do_normalize', 'do_rescale', 'do_resize', 
                              'feature_extractor_type', 'image_mean','image_std', 
                              'resample', 'rescale_factor', 'size']
        
        # Special case: either 'feature_extractor_type' or 'processor_class' is expected
        self.special_keys = ['feature_extractor_type', 'processor_class','image_processor_type']
    ...
    def validate_cfg(self):
        special_key_count = 0
        for key in self.cfg:
            if key not in self.expected_keys and key not in self.special_keys:
                raise ValueError(f"Unexpected key '{key}' in cfg. Please remove or handle it.")
            if key in self.special_keys:
                special_key_count += 1
        
        if special_key_count != 1:
            raise ValueError("Exactly one of 'feature_extractor_type' or 'processor_class' must be present in the configuration.")
