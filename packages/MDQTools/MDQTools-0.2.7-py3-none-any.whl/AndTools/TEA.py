...
class TEA:
    ...
    def encrypt(self, v, key):
        ...
        for i in range(0, len(v), 8):
            o = xor(v[i:i + 8], tr)
            tr = xor(code(o, secret_key), to)
            to = o
            r += tr
        return r

    def decrypt(self, v, key):
        if isinstance(key, str):
            secret_key = bytearray.fromhex(key.replace(' ', ''))
        else:
            secret_key = key
        l = len(v)
        prePlain = decipher(v, secret_key)
        pos = (prePlain[0] & 0x07) + 2
        r = prePlain
        preCrypt = v[0:8]
        for i in range(8, l, 8):
            x = xor(decipher(xor(v[i:i + 8], prePlain), secret_key), preCrypt)
