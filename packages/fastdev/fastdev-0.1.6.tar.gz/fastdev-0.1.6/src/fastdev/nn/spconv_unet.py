...
class SpUNetBase(nn.Module):
    ...
    def forward(self, points: torch.Tensor):
        B, N, D = points.shape
        if D != 3:
            raise ValueError(f"Input points must have 3 dimensions, got {D} instead.")
        # TODO: Check if this is the correct way to normalize the points
        grid_coord = torch.div(points - points.min(-2, keepdim=True).values, 0.01, rounding_mode="trunc").int()
