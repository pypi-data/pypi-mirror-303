...
class WeightClip(keras.constraints.Constraint):
    ...
    def __init__(self, clip_value: float = 1.0) -> None:
        """
        Clips all the weights/biases to be within the range [-1, 1] to help with quantisation later on.
    ...
    def __call__(self, w: Any):
        ...
        :param w: Tensor or variable representing the wights
...
class LinnModel:
    ...
    def construct_model(
        ...
        :param layer_definition: a list of tuples of the form (layer_width,) or (layer_width, activation) which defines
        the model. This includes both the input and output layer size. If not provided the default model will be used.
        :param show_summary: (bool) determines whether the model summary is displayed
        :param optimiser: optimiser fed to the keras compile option.
        ...
        self.model.compile(
            optimizer=optimiser, loss=loss, metrics=[met for met in metrics]
        )
    ...
    def _check_data_model_dim(
        ...
        if self.model is None:
            _LOG.warning(
                "Keras model is not initialised. Dimension checking will be skipped."
            )
            return False
    ...
    def _transform(self, data_array: np.ndarray, is_input: bool):
        """
        Transform the data into a domain amenable for quantisation.
        :param data_array: Data array to be transformed
        :param is_input: is it an input oir output?
    ...
    def set_training_data(
        ...
        specified. Scaling will ensure the model inputs and outputs are in the domain [-1, 1]. Call this after
    ...
    def fit_model(
        ...
        **keras_kwargs: Optional[Any],
        ...
        if validation_data is not None:
            ...
            if not success:
                raise RuntimeError(
                    f"Model is not initialised. Call {self.__class__.__name__}.construct_model to create"
                    f" the model."
                )
    ...
    def predict(
        ...
        **keras_kwargs: Optional[Any],
        ...
        """
        Function that can be used to call the network for testing the predictive capabilities.
        :param inputs: The input data to be run through the model
        :param scale: Whether the data should be scaled
        :param unscale_output: Whether we should also unscale the data
        :param keras_kwargs: parameters to be passed to thhe keras predict function if needed.
...
def get_linn(
    ...
    Neural Network Instrument into the `.linn` format needed to upload to
    the device.

    Args:
        model (keras.models.Model): The keras model
        input (IOMapping): Either `IOMapping.Serial` or `IOMapping.Parallel`,
                           depending on whether the input data is timeseries
                           windows or parallel inputs from multiple channels.
        output (IOMapping): As above, for output, either a timeseries or
...
def save_linn(
    ...
    Args:
        ...
        output (IOMapping): As above, for output, either a timeseries or
                            parallel channel outputs.
        file_name (str): Path like string
