...
@auto_schema(
    ...
    description=(
        "This functionality performs the design and verification of steel beam members "
        "in accordance with Eurocode 3 (EN 1993-1-1). The design process considers key "
        ...
        "- Check for local buckling and torsional buckling\n"
        "- Application of safety factors and load combinations\n\n"
        "The functionality provides detailed results for the design, including assessments "
        "and decisions for each design condition."
    )
)
def report_ec3_beam_column(matl: SteelMaterial_EC, sect: SteelSection_EN10365, load: MemberForce, length: SteelLength_EC, eff_len: EffectiveLength, factor: SteelMomentModificationFactor_EC) -> ResultBytes:
    ...
    file_path = generate_report_xls(dll, 'Report_EC3_BeamColumn', json_data_list)
