...
from metamart_source_cube.api import CubeSchema, DimensionSchema, MeasureSchema
from pydantic import BaseModel


class MetamartID(BaseModel):
    name: str
    namespace: str
...
class CubeNode(BaseNode):
    ...
    def from_schema(cls, cube: CubeSchema, namespace: str, source: Optional[SourceNode]) -> "CubeNode":
        return cls(
            node_id=MetamartID(name=cube.name, namespace=namespace),
            metadata=cube,
            source_node=source,
        )
...
class DimensionNode(BaseNode):
    ...
    def from_schema(cls, dimension: DimensionSchema, cube: CubeNode) -> "DimensionNode":
        return cls(
            node_id=MetamartID(name=f"{cube.node_id.name}.{dimension.name}", namespace=cube.node_id.namespace),
            cube_ref=cube,
            metadata=dimension,
        )
...
class MeasureNode(BaseNode):
    ...
    def from_schema(cls, measure: MeasureSchema, cube: CubeNode) -> "MeasureNode":
        return cls(
            node_id=MetamartID(name=f"{cube.node_id.name}.{measure.name}", namespace=cube.node_id.namespace),
            cube_ref=cube,
            metadata=measure,
        )
