...
class AsyncModelhub:
    ...
    async def _process_prompt(self, prompt: PROMPT_TYPE):
        ...
        prompt = await self._replace_image_with_id(prompt)
    ...
    async def _prepare_chat_args(
        ...
        **kwargs,
        ...
        return {
            "prompt": await self._process_prompt(prompt),
            "model": model,
            "parameters": {**new_params, **kwargs},
            "stream": stream,
        }

    async def get_models(self) -> ModelInfoOutput:
        return await self._get("models", cast_to=ModelInfoOutput)
    ...
    async def chat(
        ...
        **kwargs,
        ...
        return await self._post(
            "chat",
            body=await self._prepare_chat_args(
                prompt=prompt,
                model=model,
                history=history,
