...
def get_actontimespan(info):
    timestr = "%s %s" % (info['ActionDay'], info['ActionTime'])
    timeArray = time.strptime(timestr, "%Y%m%d %H%M%S")
    x = int(time.mktime(timeArray) * 1000)
    return x
...
def fin_get_history_ohlc(exchang_id: str, instrument_id: str, range: str,
    ...
    dfohlc['ActionTimeSpan'] = dfohlc.apply(get_actontimespan, axis=1)
