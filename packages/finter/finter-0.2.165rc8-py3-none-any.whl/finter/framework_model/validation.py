...
ITEM = namedtuple("freq_validation",
                  ['duration_sec', 'validation_range', 'validation_end'])
DAYS = 'days'
HOURS = 'hours'
MINUTES = 'minutes'
...
class ValidationHelper(IValidation):
    ...
    def __init__(self, model_path=None, model_info=None):
        ...
        self.end = outsample_end_date(datetime.now(),
                                      n_week=FREQ_TO_VALUE[model_info["freq"]].validation_end[DAYS])
        self.lmit_secs = FREQ_TO_VALUE[model_info["freq"]].duration_sec
    ...
    def validate_combine_getter(self, end):
        try:
            # Validate index
            model_freq = self.__MODEL_INFO["freq"]
            model_range = timedelta(**FREQ_TO_VALUE[model_info[model_freq]].validation_range)
            validation_start = self.times_before(end, model_range, self.exchange)
            print("start: %s" % validation_start)
            start_time = datetime.now()

            start_date = self.times_before(end, model_range, self.exchange)
            end_date = self.times_before(end, timedelta(days=1), self.exchange)
    ...
    def validate_start_dependency(self):
        ...
        def _getter(_end: datetime, _bef: int) -> pd.DataFrame:
            return self.__MODEL.get(
                self.times_before(_end, timedelta(days=_bef), exchange), int(_end.strftime("%Y%m%d"))
            )
    ...
    def validate_end_dependency(self):
        ...
        def _getter(_end: datetime, _bef: int) -> pd.DataFrame:
            return self.__MODEL.get(
                self.times_before(_end, timedelta(days=100), exchange),
                self.times_before(_end, timedelta(days=_bef), exchange),
            )
    ...
    @staticmethod
    def times_before(end, times: timedelta, exchange="crypto"):
        def last_trading_times(end, exchange="crypto"):
            start = end - timedelta(days=30)
            return int(
                list(iter_trading_times(start, end, exchange))[-1].strftime("%Y%m%d%H%M%S")
            )
