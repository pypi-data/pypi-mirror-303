...
# validation (start=days_before(end, val)=validation_range, end)
FREQ_TO_VALIDATION_DAYS_RANGE = {
    "1T": (3, 7),
    "10T": (3, 7),
    "30T": (3, 7),
    "1h": (90, 730),
...
class ValidationHelper(IValidation):
    ...
    def __init__(self, model_path=None, model_info=None):
        ...
        self.end = outsample_end_date(datetime.now(), n_week=FREQ_TO_VALIDATION_DAYS_RANGE[model_info["freq"]][1] // 7)
    ...
    def validate_combine_getter(self, end):
        try:
            ...
            model_range = FREQ_TO_VALIDATION_DAYS_RANGE[model_freq][0]
            validation_start = self.days_before(end, model_range, self.exchange)
            print("start: %s" % validation_start)
            start_time = datetime.now()

            start_date = self.days_before(end, model_range, self.exchange)
            end_date = self.days_before(end, 1, self.exchange)
