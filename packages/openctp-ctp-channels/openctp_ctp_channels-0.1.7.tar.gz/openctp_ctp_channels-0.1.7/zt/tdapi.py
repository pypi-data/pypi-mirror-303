...
class CTdSpiImpl(tdapi.CThostFtdcTraderSpi):
    ...
    def __init__(
        ...
        self._order_rtn_queue = queue.Queue()
        self._order_cancel_queue = queue.Queue()
    ...
    def OnRtnOrder(self, pOrder: tdapi.CThostFtdcOrderField):
        ...
        if pOrder is not None and pOrder.OrderSysID:
            print(">>>> OnOrder", pOrder.OrderSysID)
            if pOrder.OrderStatus == tdapi.THOST_FTDC_OST_NoTradeQueueing:
                self._order_rtn_queue.put(pOrder.OrderSysID)
            elif pOrder.OrderStatus == tdapi.THOST_FTDC_OST_Canceled:
                self._order_cancel_queue.put(None)
    ...
    def OnRtnTrade(self, pTrade: tdapi.CThostFtdcTradeField):
        ...
        if pTrade is not None:
            print(">>>> OnTrade", pTrade.OrderSysID)
            if pTrade.OrderSysID:
                self._trade_queue.put(pTrade.OrderSysID)
    ...
    def OnRspQryAccountregister(
        self,
        pAccountregister: tdapi.CThostFtdcAccountregisterField,
        pRspInfo: tdapi.CThostFtdcRspInfoField,
        nRequestID: int,
        bIsLast: bool,
    ):
        """请求查询银期签约关系响应"""
        self._check_rsp(pRspInfo, pAccountregister, bIsLast)

    def qry_contract_back(self):
        """请求查询签约银行"""
        print("> 请求查询签约银行")
    ...
    def OnRspQryContractBank(
        self,
        pContractBank: tdapi.CThostFtdcContractBankField,
        pRspInfo: tdapi.CThostFtdcRspInfoField,
        nRequestID: int,
        bIsLast: bool,
    ):
        """请求查询签约银行响应"""
        self._check_rsp(pRspInfo, pContractBank, bIsLast)
    ...
    def OnRspFromBankToFutureByFuture(
        self,
        pReqTransfer: tdapi.CThostFtdcReqTransferField,
        pRspInfo: tdapi.CThostFtdcRspInfoField,
        nRequestID: int,
        bIsLast: bool,
    ):
        """期货发起银行资金转期货请求响应"""
        print("  期货发起银行资金转期货请求响应")
        self._check_rsp(pRspInfo, pReqTransfer, bIsLast)

    def OnRtnFromBankToFutureByFuture(
        self, pRspTransfer: tdapi.CThostFtdcRspTransferField
    ):
        print("  期货发起银行资金转期货请求通知")
        self._check_rsp(tdapi.CThostFtdcRspInfoField(), pRspTransfer)
...
if __name__ == "__main__":
    ...
    # spi.qry_account_register()
    # spi.qry_contract_back()
    # spi.ReqFromBankToFutureByFuture()
    spi.settlement_info_confirm()
    # spi.qry_instrument()
    # spi.qry_instrument(exchange_id="CZCE")
    # spi.qry_instrument(product_id="AP")
    # spi.qry_instrument(instrument_id="TL2406")
    # spi.qry_instrument_commission_rate("cu2403")
    # spi.qry_instrument_commission_rate("ZC309")
    # spi.qry_instrument_margin_rate()
    # spi.qry_instrument_margin_rate(instrument_id="T2403")
    ...
    # 两报撤单
    # for i in range(2):
    #     spi.limit_order_insert("CFFEX", "TF2406", 103)
    #     sys_id = spi._order_rtn_queue.get()
    #     time.sleep(5)
    #     spi.order_cancel1("CFFEX", "TF2406", sys_id)
    #     spi._order_cancel_queue.get()
    #     while True:
    #         if not spi._order_rtn_queue.empty():
    #             spi._order_rtn_queue.get()
    #         else:
    #             break

    # 两笔成交
    # spi.limit_order_insert("CFFEX", "TF2406", 103.35)
    # time.sleep(5)
    # spi.limit_order_insert("CFFEX", "TF2406", 103.35)
    ...
    # time.sleep(10)
    # spi.limit_order_insert("", "FG405", 1499)
    # while True:
    #     sys_id = spi._order_rsp_queue.get()
    #     time.sleep(5)
    #     spi.order_cancel1("CZCE", "FG405", sys_id)
