...
from plurally.models.action.format import TableToTextSimple
...
""",
    ...
    def str_adapter(src_node, tgt_node):
        nodes = [
            TableToTextSimple(
                TableToTextSimple.InitSchema(
                    name="Conversation History To Text",
                    pos_x=(src_node.pos_x + tgt_node.pos_x) / 2,
                    pos_y=(src_node.pos_y + tgt_node.pos_y) / 2,
                )
            )
        ]
        connections = [(0, "history", 1, "table"), (1, "text", 2, None)]
