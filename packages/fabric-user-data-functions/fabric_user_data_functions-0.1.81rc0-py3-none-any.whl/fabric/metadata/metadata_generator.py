...
SUPPORTED_UDF_PARAMETER_AND_RETURN_TYPES = ["str", "int", "float", "bool", "datetime", "list", "dict", "set", "tuple", "None", "FabricLakehouseClient", "FabricSqlConnection", "FabricLakehouseFilesClient", "UserDataFunctionContext"]
ERROR_CODE = 1
SUCCESS = 0
...
def _ensure_return_type_is_supported(return_type_name: str, func_name: str, errors: list):
    if return_type_name not in SUPPORTED_UDF_PARAMETER_AND_RETURN_TYPES:
        errors.append(f"The function '{func_name}' has a return type '{return_type_name}' that is not supported. Supported types are: {SUPPORTED_UDF_PARAMETER_AND_RETURN_TYPES}")
...
def _ensure_parameter_type_is_supported(parameter_name: str, parameter_type_name: str, func_name: str, errors: list):
    ...
    if parameter_type_name not in SUPPORTED_UDF_PARAMETER_AND_RETURN_TYPES:
        errors.append(f"The function '{func_name}' has a parameter '{parameter_name}' with a data type '{parameter_type_name}' that is not supported. Supported types are: {SUPPORTED_UDF_PARAMETER_AND_RETURN_TYPES}")
