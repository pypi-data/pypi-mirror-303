    ...
    def doPrintMapEApi(self):
        try:
            ...
            for key in CApiFlow.getInstance().eApiConfig.mapEApi.keys():
                ...
                count += 1
            IuLog.doInfo("api", f"{listApiStr}\n")
            listApiPackageStr = ""
            count = 1
            
            for key in CApiFlow.getInstance().eApiConfig.mapEApiPackage.keys():
                eApiPackage: EApiPackage = CApiFlow.getInstance().eApiConfig.mapEApiPackage.doGet(key)
                countStr = str(count).ljust(4, " ")
                listApiPackageStr = "\n".join(
                    [
                        listApiPackageStr,
                        (
                            f"{countStr} {eApiPackage.name} {eApiPackage.version}\n" 
                            ),
                    ]
                )
                count += 1
            IuLog.doInfo("apiPackage", f"{listApiPackageStr}\n")
