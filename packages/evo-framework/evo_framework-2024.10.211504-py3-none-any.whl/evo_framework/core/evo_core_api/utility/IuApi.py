    ...
    async def doInstallPypi(packageName:str):
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                eApiPackage = EApiPackage()
                eApiPackage.doGenerateID(packageName,isHash=True)
                eApiPackage.name = packageName
                eApiPackage.version = f"{package_version}"
                CApiFlow.getInstance().eApiConfig.mapEApiPackage.doSet(eApiPackage)
                IuLog.doDebug(__name__, f"{packageName} version: {package_version}")
