...
class HugpiInternetExplorer:
    ...
    def _get_url_extract_content(self,urls):
        start = time.time()
        sample_content = " ".join(self._fetch_url_content(urls[0]).split()[:1000])
        end = time.time()
    ...
    def _get_results(self, query: str, top_n: int = 4,max_urls=4) -> List[Tuple[str, str, float]]:
        ...
        sample_content = self._get_url_extract_content(urls)
    ...
    def answer_query(self, query: str,**kwargs) -> Message:
        results = self._get_results(query,max_urls=kwargs.get("max_urls",4))
