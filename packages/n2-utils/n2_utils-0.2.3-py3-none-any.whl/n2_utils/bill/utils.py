...
from .models import Application, CompanyApplication
...
def create_bill(data, company_id, user_id, customer_id):
    ...
    try:
        ...
        company_application = CompanyApplication.objects.filter(application_id=application.id).order_by('-created_at').first()

        if not company_application:
            raise ObjectDoesNotExist("ERP uygulaması bulunamadı")

        url_security = os.getenv('IS_PROD', False)
        protocol = 'http' if url_security else 'https'

        res = requests.post(
            f'{protocol}://{application.url}/api/bookkeep/bill_integration/',
            json=data,
            headers={
                'Authorization': company_application.token,
...
def get_bill(bill_id: UUID, company_id: UUID, type='export'):
    ...
    try:
        ...
        url_security = os.getenv('IS_PROD', False)
        protocol = 'https' if url_security else 'http'
