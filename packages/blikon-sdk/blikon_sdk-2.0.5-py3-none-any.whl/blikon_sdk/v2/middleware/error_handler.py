...
class ErrorHandlingMiddleware:
    ...
    async def _handleError(
        ...
        if 500 <= error_detail.status_code < 600:
            ...
            await self._report_error(error_detail, tb_str, request.method)
    ...
    async def _report_error(self, error_detail: ErrorDetail, traceback: str, method: str) -> None:
        try:
            ...
            api_response = await client.post(f"{base_url}{relative_url}", headers=headers, json=critical_error.dict())

            # Check the response and log the result
            if api_response.status_code == 200:
                self.log_service.info("Critical error succesfully logged into the Contingency Service", source="blikon_sdk", function="_report_error")
            else:
                self.log_service.error(
                    f"Failed to log error into the Contingency Service. Status code: {api_response.status_code}. Response: {api_response.text}", source="blikon_sdk", function="_report_error")
                raise HTTPException(
                    status_code=api_response.status_code,
                    detail=f"The SDK was not able to log the critical error. Response: {api_response.text}"
                )
        except Exception as e:
            self.log_service.error(f"Unable to log error into the Contingency Service. Error message: {str(e)}", source="blikon_sdk", function="_report_error")
