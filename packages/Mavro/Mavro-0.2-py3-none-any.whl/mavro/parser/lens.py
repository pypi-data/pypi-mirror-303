        ...
        try:
            for ln_num, ln in enumerate(self.lns, start=1):
                ...
                if cont.startswith("let ") or cont.startswith("const "):
                    ...
                    raise SyntaxError("Python 'def' keyword is not supported in Mavro. Use 'fn' instead")
                    ...
                    indent += 4
                elif cont.startswith("execute "):
                    parts: list[str] = cont.split(" ", 1)
                    cont = f"subprocess.run({parts[2]}, shell={parts[1]})"
                elif cont.startswith(".") and cont != "...":
                    cont = cont.lstrip(".").lstrip()
                    parts: list[str] = cont.split(" ", 1)
                    while True:
                        if parts[0] == "":
                            parts.pop(0)
