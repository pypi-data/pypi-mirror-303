...
class EMSys():
            ...
            while index_item_atual < index_ultimo_item:
                ...
                if 'isqueiro' in item_value.lower():
                    ...
                    try:
                        ...
                        console.print(f"Erro ao cancelar edição de item: {e}", style="bold green")
                    await worker_sleep(3)
        ...
        for item in data_list:
            try:
                new_item = {
                    ...
                    'custo_min': float(item['R$ Custo Min.'].replace('worker_automate_hub/utils/utils_nfe_entrada.py.', '').replace(',', '.')),
    ...
    async def alterar_nop(self, cfop):
