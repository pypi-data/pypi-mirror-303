...
from alarm.utils.ycl_abandon_abnormal import get_time

use_dac = False
if use_dac:
    import common.redis
    import alarm.utils.control_delta
    import alarm.views.alarm_timing_utils
else:
    import alarm.utils.fake_cache
...
class DacFacade:
    ...
    def get(self, key, default=None):
        if use_dac:
            ...
            return utils.fake_cache.default_cache.get(key, default)

    def set(self, key, value, timeout):
        if use_dac:
            return common.redis.default_cache.set(key, value, timeout)
        else:
            return utils.fake_cache.default_cache.set(key, value, timeout)

    def delete(self, key):
        if use_dac:
            return common.redis.default_cache.delete(key)
        else:
            return utils.fake_cache.default_cache.delete(key)

    def clear(self):
        if not use_dac:
            utils.fake_cache.default_cache.clear()
...
def query_actions(global_result, alarm_rule_id: int) -> dict:
    if use_dac:
        return utils.control_delta.query_actions(global_result, alarm_rule_id)
    else:
...
def get_and_deal_rule_dict_data(period, **kwargs):
    ...
    if use_dac:
        ...
        from utils.ycl_abandon_abnormal import get_time_str
        # directory = r"E:/zhanwan_code/tools/data/" + get_time(end).date().strftime("%m%d")
        directory = r"D:/ws/alarm/" + get_time(end).date().strftime("%m%d")
