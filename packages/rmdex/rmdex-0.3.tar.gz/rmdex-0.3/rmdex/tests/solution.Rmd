...
mf_survived <- table(titanic$sex, titanic$survived)
mf_survived
```
...
pclass_survived <- table(titanic$pclass, titanic$survived)
pclass_survived
```
...
children <- subset(titanic, titanic$age < 15)
head(children)
```
...
pclass_children <- table(children$pclass, children$survived)
pclass_children
```
...
third_class_female <- subset(titanic, titanic$sex == 'female' & titanic$pclass == '3')
...
port_by_saved = table(third_class_female$embarked, third_class_female$survived)
port_by_saved
```
