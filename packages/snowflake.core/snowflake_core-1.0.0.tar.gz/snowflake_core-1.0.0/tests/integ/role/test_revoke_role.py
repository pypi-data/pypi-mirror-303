...
def test_revoke_role(roles, test_role_name, test_database_role_name, database):
    roles[test_role_name].grant_role(
        role_type="DATABASE ROLE", role=Securable(name=test_database_role_name, database=database.name)
    )
...
@pytest.mark.use_accountadmin
def test_revoke_privileges(roles, test_role_name, database, schema, temp_table):
    # grant on a table
...
@pytest.mark.use_accountadmin
def test_revoke_privileges_on_all(roles, test_role_name, database):
...
@pytest.mark.use_accountadmin
def test_revoke_future_privileges(roles, test_role_name, database):
...
@pytest.mark.use_accountadmin
def test_revoke_grant_option_for_privileges(roles, test_role_name, database, schema, temp_table):
    # grant on a table
    roles[test_role_name].grant_privileges(
        privileges=["SELECT"],
        securable_type="TABLE",
...
@pytest.mark.use_accountadmin
def test_revoke_grant_option_for_privileges_on_all(roles, test_role_name, database):
    # all schemas in a database
    roles[test_role_name].grant_privileges_on_all(
        privileges=["USAGE"],
...
@pytest.mark.use_accountadmin
def test_revoke_grant_option_for_future_privileges(roles, test_role_name, database):
    # all future schemas in a database
    roles[test_role_name].grant_future_privileges(
        privileges=["USAGE"],
        securable_type="SCHEMA",
