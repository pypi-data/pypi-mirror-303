...
def test_clone(streams, temp_stream):
    ...
    try:
        ...
        assert type(temp_stream.fetch().stream_source) is type(cloned_handle.stream_source)
...
def test_clone_across_schema(temp_stream, temp_schema):
    ...
    try:
        assert type(temp_stream.fetch().stream_source) is type(created_handle.fetch().stream_source)
