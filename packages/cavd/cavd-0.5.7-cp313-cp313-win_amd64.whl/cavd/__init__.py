...
from cavd.modules.struc_analysis import get_struct_radii, get_skestruct_radii, struct_to_AtomNetwork
...
def cal_channel_merge_clusters(
    ...
    symprec = 0.01
    sym_vornet, voids = get_vornet_labeled(vornet, struct.symm_ops, symprec)
...
def outChannelToPOSCAR(
    ...
    add_fcs_vornet = vornet.add_facecenters(faces)
    sym_vornet, voids = get_vornet_labeled(add_fcs_vornet, struct.symm_ops)
    prefixname = filename.replace(".cif", "")
...
def outVesta(
    ...
    sym_vornet, voids = get_vornet_labeled(add_fcs_vornet, struct.symm_ops, ntol)
...
def comDescriptors(
    ...
    symm_number = struct.space_group_info['space_group_number']
    symm_sybol = struct.space_group_info['space_group_name']
...
def channelCom(
    ...
    sym_vornet, voids = get_vornet_labeled(add_fcs_vornet, struct.symm_ops, symprec)
    prefixname = filename.replace(".cif", "")
    writeNETFile(prefixname + "_origin.net", atmnet, sym_vornet)
...
def crystal_structure_analysis(filename, migrant, ntol = 0.02, rad_flag = True,
    ...
    """
    晶体结构分析函数,用于处理cif文件,进行 Voronoi 分解和通道分析。
    ...
    """
    struct, radii = get_struct_radii(filename, migrant, rad_flag, rad_dict)
    atmnet = struct_to_AtomNetwork(struct, radii)
    vornet, edge_centers, fcs, faces = atmnet.perform_voronoi_decomposition(rad_flag=True,saveVorCells=True,ntol=ntol)
    add_fcs_vornet = vornet.add_facecenters(faces)
    sym_vornet, voids = get_vornet_labeled(add_fcs_vornet, struct.symm_ops, ntol)
    prefixname = filename.replace(".cif", "")
...
def skeleton_structure_analysis(filename, migrant, storage_path = None, ntol = 0.02, rad_flag = True,
    ...
    - conn_val: 连接值列表。
    """
    struct, radii = get_skestruct_radii(filename, migrant, rad_flag, rad_dict)
    atmnet = AtomNetwork.read_from_CIF(filename, radii, rad_flag)
    vornet, edge_centers, fcs, faces = atmnet.perform_voronoi_decomposition(True, ntol)
    add_fcs_vornet = vornet.add_facecenters(faces)
    sym_vornet, voids = get_vornet_labeled(add_fcs_vornet, struct.symm_ops, ntol)
