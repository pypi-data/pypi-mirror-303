			...
			if v is not None:
				for intermediate_name in v:
					for particle_name in v[intermediate_name]:
						if particle_name not in naming_scheme[naming_scheme.find('->'):].split(' '):
							raise ValueError(f"'{intermediate_name}' cannot be built; '{particle_name}' not present in naming_scheme '{values['naming_scheme']}'.")
		return v
...
def run_func(**params):
	...
	(rapidsim_tuple, fully_reco, nPositive_missing_particles, 
	 ...
	 true_PID_scheme, combined_particles, map_NA_codes) = rapidsim_output
	...
	rapidsim_tuple_reco = run_network(
		...
		map_NA_codes, params['dropMissing'],
