...
class tuple_manager:
	...
	def __init__(self, 
		...
		if entry_stop:
			...
			arrays = self.raw_tuple.arrays(list_of_branches, library="np", entry_stop=entry_stop)
		...
		for branch in self.tuple:
			if tag_to_find_particle_names in branch:
				self.list_of_particles.append(branch.replace(f'_{tag_to_find_particle_names}',''))
	...
	def get_branches(self, branches, transformers=None, numpy=False, scale_factor=1., transform_by_index=False):
		data = self.tuple[branches]*scale_factor

		if transformers:
			
			data = tfs.transform_df(data, transformers, transform_by_index=transform_by_index)
