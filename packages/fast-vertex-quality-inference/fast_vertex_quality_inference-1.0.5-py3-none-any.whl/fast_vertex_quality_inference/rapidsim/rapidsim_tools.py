	...
	with uproot.open(fileName) as f:
		...
		for branch in branches_to_keep:
			if "origX_TRUE" in branch:
				if re.match(r'^NA_\d{8}$', naming_scheme[idx]) or naming_scheme[idx] == 'NA':
		...
		for particle in true_PID_scheme:
			...
			if particle in map_NA_codes:
				if f'{map_NA_codes[particle]}_PX' in list(branches.keys()):
					branches[f'{map_NA_codes[particle]}_ID_TRUE'] = np.ones(np.shape(branches[f'{map_NA_codes[particle]}_PX']))*true_PID_scheme[particle]
...
def check_config(config, USE_EVTGEN, naming_scheme, model_scheme=None, geometry='LHCb', acceptance='AllIn', useEvtGen='TRUE', evtGenUsePHOTOS='TRUE', dropMissing=True):
	...
	for key in naming_scheme:
		...
		if not re.match(r'^NA_\d{8}$', naming_scheme[key]) and naming_scheme[key] != 'NA':
