...
class UpdatedTransformer:
	...
	def process(self, data_raw):
		...
		if ("TRUEORIGINVERTEX_X" in self.column or "TRUEORIGINVERTEX_Y" in self.column) or ("origX_TRUE" in self.column or "origY_TRUE" in self.column):
			return data
...
def transform_df(data, transformers, transform_by_index=False):
	...
	if transform_by_index:
		for (branch, data_i), (transformer_key, transformer) in zip(data.items(), transformers.items()):
			convert_units = False
			for P in ["P","PT","PX","PY","PZ"]:
				if f"_{P}_" in branch or branch[-(len(P)+1):] == f"_{P}":
					convert_units = True
			if "residualfrac" in branch: convert_units = False
			# print(transformer_key, transformer)
			# print(branch)
