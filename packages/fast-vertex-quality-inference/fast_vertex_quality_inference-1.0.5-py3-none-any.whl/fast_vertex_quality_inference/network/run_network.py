...
def run_network(rapidsim_tuple, fully_reco, nPositive_missing_particles, nNegative_missing_particles, true_PID_scheme, combined_particles, map_NA_codes, dropMissing, mother_particle_name, intermediate_particle_name, daughter_particle_names):

	re_smear_electrons = True
	# re_smear_electrons = False
	...
	# print('A:', list(data_tuple.tuple.keys()))
	...
	# print('B:', list(data_tuple.tuple.keys()))
	# quit()
	...
	if re_smear_electrons:
		for particle in true_PID_scheme:
			if true_PID_scheme[particle] in (11,-11):
				...
				if dropMissing and (re.match(r'^NA_\d{8}$', particle) or particle == 'NA'):
					continue
				...
				with display.status_execution(status_message=f"[bold green]Manually smearing electron momenta ({particle})...", complete_message=f"[bold green]{particle} momenta smeared :white_check_mark:"):
					...
					# 	print(branch)
					relevant_electron_smearing_network_Transformers = {'DAUGHTER3_PX_TRUE':electron_smearing_network.Transformers['DAUGHTER3_PX_TRUE'],
																		'DAUGHTER3_PY_TRUE':electron_smearing_network.Transformers['DAUGHTER3_PY_TRUE'],
																		'DAUGHTER3_PZ_TRUE':electron_smearing_network.Transformers['DAUGHTER3_PZ_TRUE'],
																		}
