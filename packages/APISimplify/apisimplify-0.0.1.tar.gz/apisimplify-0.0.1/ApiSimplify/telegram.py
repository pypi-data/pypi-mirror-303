...
class TelegramBot:
    def __init__(self, token):
        ...
        self.commands = {}
        self.processed_messages = set()  # Initialize the set to track processed messages
        self.user_points = {}  # Dictionary to keep track of user points
    ...
    def add_command(self, command_name, response):
        ...
        self.commands[command_name] = response  # Store the lambda directly

    def handle_command(self, command, chat_id):
        """Handle a command and return a response."""
        if command in self.commands:
            return self.commands[command](chat_id)  # Call the lambda with chat_id
        return "I don't understand that command."

    def add_user_points(self, chat_id, points):
        """Add points to a user's balance."""
        if chat_id in self.user_points:
            self.user_points[chat_id] += points  # Increase existing balance
        else:
            self.user_points[chat_id] = points  # Set initial balance
