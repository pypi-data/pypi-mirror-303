...
class ChattyResponseFactory:
    ...
    def create(messages: List[Dict[str, Any]]) -> ChattyResponse:
        ...
        chatty_context = ChattyContext.default().response_id = id
        subtype = MessageSubtype.CHATTY_RESPONSE
        messages = [MessageRequest.from_response(message, chatty_context, subtype) for message in messages]
    ...
    def update(chatty_response: ChattyResponse, messages: List[Dict[str, Any]]) -> ChattyResponse:
        ...
        chatty_context = ChattyContext.default().response_id = chatty_response.id
        subtype = MessageSubtype.CHATTY_RESPONSE
        messages = [MessageRequest.from_response(message, chatty_context, subtype) for message in messages]
