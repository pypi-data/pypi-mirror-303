...
from yta_general_utils.image.color import get_most_common_green_rgb_color_and_similars
from yta_general_utils.image import ImageParser
from yta_general_utils.image.region import ImageRegionFinder
from yta_general_utils.image.converter import pil_image_to_numpy, numpy_image_to_pil
...
def get_greenscreen_areas_details(image: Union[str, Any]):
    ...
    image = ImageParser.parse_as_pillow(image)
...
def insert_video_into_moving_greescreen(greenscreen_google_drive_url: str, video: Union[VideoFileClip, CompositeVideoClip, ImageClip, str], output_filename: Union[str, None] = None):
    ...
    """
    video = VideoParser.video_to_moviepy_video(video)
...
def __process_moving_greenscreen_frame(greenscreen_google_drive_url: str, frame_number: int, do_force: bool = False):
    ...
    green_rgb_color, similar_greens, green_regions = get_greenscreen_areas_details(get_frame_from_video_by_frame_number(gs_clip, frame_number))
