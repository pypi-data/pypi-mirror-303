from yta_general_utils.image import parse_as_pillow_image
...
class ImageRegionFinder:
    ...
    @classmethod
    def find_green_regions(cls, image: Union[str, Image.Image, np.ndarray]):
        """
        ...
        """
        if not isinstance(image, np.ndarray):
            image = pil_image_to_numpy(parse_as_pillow_image(image).convert('RGB'))
            
        return cls.find_regions(image, PixelFilterFunction.is_green)
    
    @classmethod
    def find_transparent_regions(cls, image: Union[str, Image.Image, np.ndarray]):
        """
        ...
        """
        if not isinstance(image, np.ndarray):
            image = pil_image_to_numpy(parse_as_pillow_image(image).convert('RGBA'))
