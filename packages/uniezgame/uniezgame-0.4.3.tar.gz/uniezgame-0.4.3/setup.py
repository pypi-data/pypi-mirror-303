...
unigame_module = Extension(
    module_name,
    sources=['uniezgame/mylib.cpp'],  # Zaktualizowane do właściwej ścieżki
...
def copy_dlls():
    # Upewnij się, że folder build istnieje
    current_dir = os.path.dirname(os.path.abspath(__file__))
    site_packages_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Lib', 'site-packages')
    build_dir = os.path.join(site_packages_dir, module_name)  # Katalog z plikiem pyd w site-packages

    # Upewnij się, że folder uniezgame istnieje
