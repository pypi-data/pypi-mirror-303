...
def var_func(par):
    def func(d : LarvaDataset):
        d.e[nam.var(par)] = d.s[par].dropna().groupby('AgentID').mean()/d.s[par].dropna().groupby('AgentID').std()
    return func
...
def unwrap_func(par, in_deg):
    def func(d : LarvaDataset):
        ...
        d.s[nam.unwrap(par)] = aux.apply_per_level(s, aux.unwrap_deg).flatten()
    return func

@funcs.param("dst")
def dst_func(point=''):
    def func(d : LarvaDataset):
        aux.compute_dst(d.s, point)
    return func
