...
def copy_data():
    ...
    for table_map in table_map_list:
        tp.run(run_table, (table_map, global_scn), {}, None, table_map[0] + '.' + table_map[1])
...
def copy_table():
    ...
    def copy_one_table(table_map):
        ...
        dst_cur.execute('drop table if exists ' + src_schema + '."' + src_table.lower() + '"')
        dst_conn.commit()
        ...
        log.info(src_schema + '.' + src_table + ' created')
...
def copy_constraint():
    ...
    def copy_one_constraint(table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        src_db = DBConn(src_db_type, src_ip, src_port, src_database, src_read_username, src_read_password)
        dst_db = DBConn(dst_db_type, dst_ip, dst_port, dst_database, dst_username, dst_password)
    ...
    tp = ThreadPool(32, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(copy_one_constraint, (table_map,))
    tp.wait()
    src_db.close()
    dst_cur.close()
    dst_conn.close()
