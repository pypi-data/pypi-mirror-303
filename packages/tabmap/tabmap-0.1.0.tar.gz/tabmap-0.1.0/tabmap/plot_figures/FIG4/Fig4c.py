# %% [markdown]
# ### Figure of methods performance on PD dataset

# %%
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Set to the index of the GPU you want to use (e.g., 0, 1, 2, ...)
import pandas as pd
import numpy as np
...
index_map = {item: idx for idx, item in enumerate(feature_names)}
#1: ['worst radius', 'mean radius', 'mean area', 'worst concave points', 'mean texture']
#2: ['worst radius', 'worst concave points', 'worst perimeter', 'worst texture', 'mean concave points'] 96.23
#3: ['worst texture', 'worst concavity', 'worst symmetry', 'worst concave points', 'mean concavity']
#4: ['worst perimeter', 'worst concave points', 'worst radius', 'worst area', 'mean concave points']
#5: ['worst perimeter', 'worst concave points', 'worst radius', 'worst area', 'mean concave points']
indices = [index_map.get(item, None) for item in ['worst perimeter', 'worst concave points', 'worst radius', 'worst area', 'mean concave points']]
...
for trial in range(5):
    ...
    for fold_id, (train_idx_all, test_idx) in enumerate(skf.split(features, labels)):
        ...
        for model_id in ['tabmap', 'LR', 'RF', 'GB', 'XGB']: 
            ...
            if model_id in DL_MODELS_IMAGE_BASED:
                ...
                # explainer = shap.LinearExplainer(tuner.final_model, features[train_idx], feature_names=list(feature_names))
                # shap_values = explainer.shap_values(features[test_idx])
                # shap.initjs()  # Initialize JavaScript visualization in Jupyter
                # shap.force_plot(explainer.expected_value, shap_values[0,:], features[test_idx][0,:], feature_names=feature_names)
                
            predictions_test = pd.DataFrame({"model": model_id, "fold": fold_id, "trial": trial, "true": labels[test_idx].ravel(), "pred": y_pred.ravel()})
    ...
    predictions_test_df.set_index(["model", "trial", "fold"]).to_csv(f"{results_path}/{data_set}/model_preds.csv", sep='\t')
    performance_test_df.set_index(["model", "trial", "fold"]).to_csv(f"{results_path}/{data_set}/model_performance.csv", sep='\t')
