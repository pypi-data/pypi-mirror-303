...
class FlowActor(Actor):
    ...
    async def on_assign(
        ...
        try:
            ...
            nodes_without_instream = [node for node in nodes_without_instream if node.id not in connected_arg_nodes]
                ...
                for spawned_event in spawned_events:
                    ...
                    if spawned_event.target == returnNode.id:
                        track = await self.track_mutation(
                            ...
                            exception=str(spawned_event.exception) if spawned_event.exception else None,
