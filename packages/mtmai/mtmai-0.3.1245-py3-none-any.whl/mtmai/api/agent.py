...
# @router.post("/graph_example", response_model=ChatMessagesResponse)
# async def graph_example(session: SessionDep, req: AgentChatMessageRequest):
#     """练习：后端的graph 直接对接前端 state"""
#     count_statement = (
#         select(func.count())
#         .select_from(UiMessage)
#         .where(UiMessage.chatbot_id == req.chat_id)
