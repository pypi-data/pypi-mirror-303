...
def append_trend_type(df, price_column):
    """ Process the dataframe to find oscillations and append trend type to the dataframe """
...
def unify_oscillations(oscillation_points):
    ...
    for current_start, current_end, current_type in oscillation_points[1:]:
        ...
        # Check if the current and last oscillations are of the same type and the time gap is less than 30 seconds
...
def find_nearest_index(df, target_time):
    # Find the absolute difference from the target_time and get the index with the minimum difference
...
def process_oscillations(df, time_column, price_column, plot=False):
    ...
    :param symbol: The asset symbol to filter by.
    :param plot: Optional, whether to plot the results.
    ...
    if plot:
        ...
        ax.plot(df.index, df[price_column], label=f'Prices')
        ...
        ax.set_title(f'Price Oscillations')
        ax.set_xlabel('Time')
...
# Example usage (commented out as this is now a library function)
