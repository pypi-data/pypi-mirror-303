...
async def retry_operation(operation, max_attempts=3, delay=2, *args, **kwargs):
    """
    ...
    while attempts < max_attempts:
        try:
            ...
            attempts += 1
            print(f"Error during operation: {e} (Attempt {attempts}/{max_attempts})")
            if attempts < max_attempts:
                print(f"Retrying in {delay} seconds...")
                await asyncio.sleep(delay)
            else:
                print(f"Max attempts reached. Operation failed after {max_attempts} attempts.")
                raise e
