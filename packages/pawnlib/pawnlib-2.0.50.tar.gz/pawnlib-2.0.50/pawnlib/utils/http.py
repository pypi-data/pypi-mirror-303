...
from pawnlib.config.logging_config import ConsoleLoggerAdapter
...
class AsyncGoloopWebsocket(AsyncCallWebsocket):
    ...
    def __init__(
        ...
        self.logger = ConsoleLoggerAdapter(logger, "AsyncCallWebsocket", self.verbose > 0)
        ...
        self.api_client = AsyncIconRpcHelper(url=url, logger=logger)
    ...
    async def default_process_transaction(self, tx, block_height):
        ...
        try:
            ...
            # if from_address in self.address_filter or to_address in self.address_filter:
            #     await self.log_message(f"Transaction involving filtered address detected: {from_address} -> {to_address}", level="info")
            ...
            if method == "setStake":
                ...
                await self.log_message(f"🔵 <Staking> {from_highlighted} has staked 💰 {stake_value} ICX.", level="info")

            elif method == "unStake":
                stake_value = self.get_stake_value(tx_data)
                await self.log_message(f"🔵 <Staking> {from_highlighted} has unstaked 💰 {stake_value} ICX.", level="info")

            elif method == "Send":
                _value = hex_to_number(tx.get('value', 0),  is_tint=True, symbol='ICX', debug=True)
                await self.log_message(
                    f"💸 <{shorten_tx_hash}> <Send> {from_highlighted} 👉 {to_highlighted} 💰 {_value} ",
                    level="info"
                )
            else:
    ...
    def get_stake_value(self, tx_data):
        try:
            return hex_to_number(tx_data['params']['value'], is_comma=True)
        except (KeyError, TypeError):
            return "N/A"

    def get_send_value(self, tx_data):
        try:
            return hex_to_number(tx_data['params']['value'], is_comma=True)
        except (KeyError, TypeError):
            return "N/A"
...
class AsyncIconRpcHelper:
    ...
    async def get_block_hash(self, tx_hash) -> dict:
        return await self.execute_rpc_call(method='icx_getBlockByHash', params={"hash": tx_hash}, return_key="result")
