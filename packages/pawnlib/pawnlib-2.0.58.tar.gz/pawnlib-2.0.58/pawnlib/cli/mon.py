...
def run_wallet_client(args, logger):
    ...
    print(settings['max_transaction_attempts'])
...
def prompt_for_env_variables(settings: dict) -> dict:
    ...
    Special handling for BASE_DIR and LOG_FILE defaults.
    ...
    for key, default_value in settings_uppercase.items():
        # Special handling for BASE_DIR and LOG_FILE
        if key == "BASE_DIR" and default_value == ComposeDefaultSettings.DEFAULT_BASE_DIR:
            default_value = ComposeDefaultSettings.BASE_DIR
        elif key == "LOG_FILE" and default_value == ComposeDefaultSettings.DEFAULT_LOG_FILE:
            default_value = ComposeDefaultSettings.LOG_FILE
...
def filter_service_specific_settings(service_name: str, args: argparse.Namespace) -> dict:
    """
    Filter service-specific settings from argparse Namespace dynamically.

    Args:
        service_name (str): The name of the service (e.g., 'ssh', 'wallet').
        args (argparse.Namespace): Parsed arguments from argparse.

    Returns:
        dict: A dictionary containing service-specific settings.
    """
    args_dict = vars(args)  # Convert args to dictionary
    print(args_dict)
    service_specific_settings = {}

    # Loop through args and dynamically collect service-specific settings
    for key, value in args_dict.items():
        if service_name in key:  # Filter based on the service name in key
            service_specific_settings[key] = value

    return service_specific_settings
...
def filter_service_specific_settings(service_name: str, args: argparse.Namespace) -> dict:
    ...
    # Loop through args and dynamically collect service-specific settings based on service name
    for key, value in args_dict.items():
        if service_name in key or key in ['log_type', 'verbose', 'slack_webhook_url', 'send_slack']:
            service_specific_settings[key] = value
...
def run_compose_init(args, logger, parser):
    ...
    # environments = prompt_for_env_variables(service_args)
    image = builder.get_valid_input("Enter the image name", default="jinwoo/pawnlib")
    volumes = builder.get_volumes("./logs:/app/pawns")
