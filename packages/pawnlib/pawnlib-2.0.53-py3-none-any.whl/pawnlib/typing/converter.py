...
def hex_to_number(
    ...
    if isinstance(converted_value, (int, float)):  # Only apply if it's a number
        if is_tint or (converted_value >= const.TINT):  # Apply tint condition
            converted_value /= const.TINT  # Perform division
            changed_text = "(tint)"
