...
def find_stable_points(cl):
  ...
  for i,s in enumerate(cl.sign_change_indices[:-1]):  # Include 0th index and all sign changes
      ...
      a, b = max(cl.s_min,cl.s_grid[cl.sign_change_indices[i]]-cl.ds), min(cl.s_grid[cl.sign_change_indices[i]]+cl.ds,cl.s_max)
      if cl.sdot(a,cl.x(a)) < cl.sdot(b,cl.x(b)):
        try:
          route=root(lambda s: cl.sdot(s,cl.x(s)),bracket=[a,b],method='brentq').root
  ...
  if cl.sdot(cl.s_min,cl.x(cl.s_min))>0: 
       cl.unstable_roots.append(cl.s_min)
  if cl.sdot(cl.s_min,cl.x(cl.s_min))<0: 
       cl.roots.append(cl.s_min)
  if cl.sdot(cl.s_max,cl.x(cl.s_max))>0: 
       cl.roots.append(cl.s_max)
  if cl.sdot(cl.s_max,cl.x(cl.s_max)<0: 
       cl.unstable_roots.append(cl.s_max)
  cl.roots=np.sort(np.unique(cl.roots))
  cl.unstable_roots=np.sort(np.unique(cl.unstable_roots)  
  ...
  cl.all_roots=np.sort(np.unique(np.r_[cl.roots,cl.unstable_roots]))
  ...
  cl.all_indices = np.where(np.isin(cl.s_grid, cl.all_roots))[0]
...
def ode_v(cl):
    def dv_over_ds(v, s):
        ...
        return (cl.δ*v-cl.W(s,cl.x(s))/cl.sdot(s,cl.x(s)))
    v_ode=[]
    if cl.all_indices[0]==cl.stable_indices[0]
        for i,s in enumerate(cl.all_indices):
            v_ode[i]=solve_ivp(dv_over_ds,[cl.all_roots[2*i]]+cl.ds,cl.all_roots[2*i+1]-cl.ds],
            [cl.W(cl.s_grid(cl.all_roots[2*i]],cl.x(cl.s_grid(cl.all_roots[2*i])])
    ...
    rudy=cl.roots[i]
    v0=cl.W(rudy,cl.x(rudy))/cl.δ
    v_ode[i]= odeint(dv_over_ds, v0, cl.s_grid(indices[i]:indices[i+1]).flatten()
