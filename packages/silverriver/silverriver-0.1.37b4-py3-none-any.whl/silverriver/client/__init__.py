...
class Crux:
    ...
    def upload_logs(self, prefix: str, blob: bytes) -> bool:
        """
        Upload logs to the Crux API.

        Args:
            prefix (str): The destination path for the uploaded logs.
            blob (bytes): The logs to upload.

        Returns:
            success (bool): The response from the API after uploading the logs.
        """
        return self._client.upload_logs(prefix, blob)
