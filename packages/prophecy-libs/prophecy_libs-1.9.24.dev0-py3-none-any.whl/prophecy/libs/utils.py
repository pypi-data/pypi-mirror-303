...
from pyspark.sql.functions import col, when, trim, regexp_replace, lower, upper, initcap
...
def clean_data(
        df: DataFrame,
        columns: list,
        replaceNullForTextFields: bool,  # Provide a string to replace nulls in text fields
        replaceNullTextWith: str,
        replaceNullForNumericFields: bool,
        replaceNullNumericWith: int,
        # Provide a numeric value to replace nulls in number fields
    ...
    for col_name in columns:
        # Get the data type of the column using the schema
        col_type = df.schema[col_name].dataType

        # If the column is a string type, apply text-based operations
        if isinstance(col_type, StringType):
            # Replace null text fields with the provided value
            if replaceNullForTextFields:
                df = df.withColumn(col_name,
                                   when(col(col_name).isNull(), replaceNullTextWith).otherwise(
                                       col(col_name)))
        ...
        # If the column is numeric, apply numeric-based operations
            ...
            if replaceNullForNumericFields:
                df = df.withColumn(col_name,
                                   when(col(col_name).isNull(), replaceNullNumericWith).otherwise(
                                       col(col_name)))
    return df
