...
class OrMatcher(Matcher):
    ...
    def match(self, value: Any):
        ...
        raise ValidationError(
            f"Failed to match \"{value}\" using any provided matchers. Last error: {last_exc}"
        ) from last_exc
    ...
    def __init__(
        ...
        for key, val in obj.items():
            if isinstance(val, dict):
                self._obj[key] = FieldMatcher(
                    is_required = True,
                    matcher = ObjectMatcher(val, infer_func)
                )
            else:
                self._obj[key] = infer_func(val)

        self._dict_matcher = ConvertMatcher(
            func = dict,
            name = "dict"
        )
