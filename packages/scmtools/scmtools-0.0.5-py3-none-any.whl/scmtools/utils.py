...
def clone_causal_models_deepcopy(source: nx.DiGraph, destination: nx.DiGraph):
    ...
    for node in destination.nodes:
        if dowhy.graph.CAUSAL_MECHANISM in source.nodes[node]:
            destination.nodes[node][dowhy.graph.CAUSAL_MECHANISM] = deepcopy(source.nodes[node][dowhy.graph.CAUSAL_MECHANISM])
...
def copy_causal_model_with_frozen_mechanisms(
    ...
    # dowhy.graph.clone_causal_models(source=causal_model.graph, destination=causal_graph_with_frozen_nodes)
    ...
    for node in freeze_mechanisms_of:
        ...
        causal_graph_with_frozen_nodes.nodes[node][dowhy.graph.CAUSAL_MECHANISM] = gcm.EmpiricalDistribution()
    ...
    for node in causal_model_frozen.graph.nodes:
        causal_model_frozen.graph.nodes[node][dowhy.graph.PARENTS_DURING_FIT] = dowhy.graph.get_ordered_predecessors(causal_graph=causal_model_frozen.graph, node=node)
