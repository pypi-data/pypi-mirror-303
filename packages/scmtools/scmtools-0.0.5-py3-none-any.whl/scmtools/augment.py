...
def augment_causal_model_with_black_box(
    ...
    # dowhy.graph.clone_causal_models(source=fit_causal_model.graph, destination=black_box_subgraph)
    ...
    for node in augmented_causal_model.graph.nodes:
        augmented_causal_model.graph.nodes[node][dowhy.graph.PARENTS_DURING_FIT] = dowhy.graph.get_ordered_predecessors(
