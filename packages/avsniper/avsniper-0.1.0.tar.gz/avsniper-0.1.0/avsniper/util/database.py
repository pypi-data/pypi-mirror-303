...
class Database(object):
    ...
    def insert_update_one(self, table_name: str, **kwargs) -> dict:
    ...
    @connect
    def execute(self, conn: Connection, sql: str, args: any) -> dict:
        cursor = conn.execute(sql, tuple(args,))
        status = {'rowcount': cursor.rowcount}
        conn.commit()
        return status
    ...
    def update(self, conn: Connection, table_name, filter_data, **kwargs):
        ...
        cursor = conn.execute(sql, tuple(u_values + f_values, ))
        status = {'rowcount': cursor.rowcount}
        conn.commit()
        return status
