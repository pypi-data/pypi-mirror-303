...
def get_sendfile_response(request, resource_url, *, mime="", filename="", attachment=False):
    if rpr_settings.get_debug_serve_resource() and django_settings.DEBUG:
        response = serve(request, resource_url, document_root=rpr_settings.get_media_root())
    ...
    filename = filename or Path(resource_url).name
