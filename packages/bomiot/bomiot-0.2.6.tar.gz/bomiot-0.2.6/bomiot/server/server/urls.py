...
def module_list_calculate_factorial(module, urlpatterns):
    try:
...
def module_name_calculate_factorial(module_name, urlpatterns):
    try:
        settings_name = 'bomiotconf'
...
module_list = list(map(lambda data: module_list_calculate_factorial(data, urlpatterns), [pkg.key for pkg in pkg_resources.working_set]))
module_name_list = list(map(lambda data: module_name_calculate_factorial(data, urlpatterns), [p for p in os.listdir(getcwd()) if os.path.isdir(p)]))
