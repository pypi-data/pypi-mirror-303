...
def module_list_calculate_factorial(module):
    try:
        ...
        if exists:
            ...
            if app_mode == 'plugins':
                ...
                if has_urls:
                    res_feedback.append(path(f'{module}/', include(f'{module}.urls')))
                ...
                if module == project_name:
                    ...
                    res_feedback.append(path('/', TemplateView.as_view(
                    ...
                    for url in find_urls:
                        if importlib.util.find_spec(f'{project_name}.{url}.urls') is not None:
                            res_feedback.append(path(f'{project_name}/{url}/', include(f'{project_name}.{url}.urls')))
            return res_feedback
        else:
            return None
    except:
...
def module_name_calculate_factorial(module_name):
    try:
        settings_name = 'bomiotconf'
...
module_list = list(map(lambda data: module_list_calculate_factorial(data), [pkg.key for pkg in pkg_resources.working_set]))
module_name_list = list(map(lambda data: module_name_calculate_factorial(data), [p for p in os.listdir(getcwd()) if os.path.isdir(p)]))
print(2, module_list)
print(3, module_name_list)
