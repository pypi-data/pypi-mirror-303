...
from promptarchitect.exceptions import (
...
def test_initialize_without_options():
    ...
    with pytest.raises(PromptValidationError, match=expected_error_message):
        EngineeredPrompt()
...
def test_initialize_with_too_many_options(valid_prompt_file):
    ...
    with pytest.raises(PromptConfigurationError, match=expected_error_message):
        EngineeredPrompt(
