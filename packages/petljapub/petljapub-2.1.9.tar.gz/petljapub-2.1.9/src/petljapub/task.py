...
class Task:
    ...
    def timelimit(self):
        return round(float(self.metadatum('timelimit')) * 1000)
    ...
    def run(self, sol, lang, testcase, timelimit=1000, output=None):
        if timelimit != None and type(timelimit) != "float":
    ...
    def measure_all_runtimes(self, force=False, repeat=3, timelimit=None, solutions=[], langs=[]):
        ...
        logger.info(self.id(), "-", "running tests to measure time...", verbosity=2)
    ...
    def calibrate(self, langs=[], min_timelimit=50, max_timelimit=1000, margin=1.25, force=False, prefix="", petlja_set_timelimit=False):
        ...
        timelimit = math.ceil(margin * max(OK_times))
    ...
    def petlja_publish(self, session=None, prefix=""):
        ...
        title = self.title()
        alias = prefix + self.alias()
        # create problem
        try:
            problem_id = petlja_api.create_problem(session, title, alias)
        except ValueError:
            ...
            try:
                problem_id = petlja_api.get_problem_id(session, alias)
            except ValueError:
        ...
        try:
            # upload statement
            tmp_st_file = tempfile.NamedTemporaryFile("w+", delete=False)
            tmp_st_file.write(self.st_content())
            tmp_st_file.seek(0)
            petlja_api.upload_statement(session, problem_id, tmp_st_file.name)
            tmp_st_file.close()
        ...
        try:
            # upload testcases
            self.tests_zip()
            petlja_api.upload_testcases(session, problem_id, self.zipped_testcases_path())
        ...
        try:
            ...
            if self.timelimit():
                timelimit = self.timelimit()
                petlja_api.set_time_limit(session, problem_id, timelimit)
    ...
    def petlja_get_problem_id(self, session=None, prefix=""):
        ...
        try:
            ...
            logger.error(f"Problem {self.title()} with alias {self.alias()} does not exist on petlja.org. Try running 'petljapub petlja-publish' first.")
            return None
    ...
    def petlja_get_competition_id(self, session=None, competition_dir=None):
        ...
        try:
            ...
            competition_id = petlja_api.get_competition_id(session, competition.alias)
            logger.info("Competition id:", competition_id, verbosity=5)
            return competition_id, competition
        except ValueError:
            logger.error(f'There is no competition with the alias {competition.alias}. Try running "petljapub create-competition" first.')
            return None, None
    ...
    def petlja_set_timelimit(self, timelimit, session=None, prefix=""):
        ...
        problem_id = self.petlja_get_problem_id(session, prefix)
        if problem_id == None:
            logger.error("Could not set time limit")
            return
        petlja_api.set_time_limit(session, problem_id, timelimit)

    def petlja_api_submit_solution_raw(self, sol, lang):
        languageId = {
            "cs": "1",
            "py": "9",
            "cpp": "11"
            ...
            if not result['succeeded']:
                ...
                if result['value'].startswith('compile error'):
                    ...
                    for testcase in result['value'].split(";")[:-1]:
                        pattern = r"test case #(\d+) \[(\d+) ms/(\d+) MB\] \+(\w+)"
                        match = re.search(pattern, testcase)
                        if match:
                            test_case_number = match.group(1)
                            execution_time = match.group(2)
                            memory_usage = match.group(3)
                            status = match.group(4)
                            testcase_results.append({"status": status,
    ...
    def petlja_test_on_all_testcases(self, sol, lang, session=None, competition_dir=None, detailed=False, timelimit=None):
        ...
        # Try to find a competition on petlja.org
        competition_id, competition = self.petlja_get_competition_id(competition_dir=competition_dir, session=session)
        # check if this is a valid competition dir
        if competition_id == None:
            logger.error(f"Only problems added to competitions can be tested")
            return None
            
        # try to find the task on petlja
        problem_id = self.petlja_get_problem_id(session=session, prefix=competition.task_prefix())
        if problem_id == None:
            logger.error(f"Only created problems can be tested")
            return None
        ...
        try:
            src = pathlib.Path(self.src_file_name(sol, lang))
            logger.info("Submitting task:", src)
            
            if detailed:
                result = asdict(petlja_api.submit_solution_detailed(session, competition_id, problem_id, src))
                # result = self.petlja_api_submit_solution_raw(sol, lang)
                ...
                score = petlja_api.submit_solution(session, competition_id, problem_id, src)
        ...
        if timelimit != None:
            try:
                petlja_api.set_time_limit(session, problem_id, self.timelimit())
    ...
    def petlja_test_all_scores(self):
        tests_passed = True
        num_tested = 0
        for sol in self.solutions():
            ...
            for lang in sol["lang"]:
                sol_id = sol["name"]
                score = self.petlja_test_on_all_testcases(lang, sol_id)
                num_tested += 1
                if not score:
                    tests_passed = False
                    continue
    ...
    # run all tests (all solutions specified in -st.md on all testcases)
    # for the given tasks
    def petlja_measure_all_runtimes(self, force=False, competition_dir=None, langs=[], solutions=[], timelimit=None):
        # if the timing file already exists and run is not force, just
        # return data read from the file
        time_json = self.time_json_path()
        if not force and os.path.isfile(time_json):
        ...
        for sol in self.solutions():
            ...
            for lang in sol["lang"]:
                ...
                results = self.petlja_test_on_all_testcases(sol["name"], lang, session=session, competition_dir=competition_dir, detailed=True, timelimit=timelimit)
                if results == None:
