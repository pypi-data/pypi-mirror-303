...
class TestQuerySubgraph:
    ...
    def test_prepare_node_index(
        ...
        expected_checksum = query_subgraph_instance._compute_checksum(
            {
                "similarity_metric": query_subgraph_instance.similarity_metric,
                "embedding_model_name": query_subgraph_instance.embedding_model_name,
                "texts": query_subgraph_instance.node_text_map,
            }
        )
        ...
        expected_checksum = query_subgraph_instance._compute_checksum(
            {
                "similarity_metric": query_subgraph_instance.similarity_metric,
                "embedding_model_name": query_subgraph_instance.embedding_model_name,
                "texts": query_subgraph_instance.edge_text_map,
            }
        )
    ...
    def test_query_graph(
        ...
        # Mock semantic_search_faiss to return expected results
        ...
        mock_get_query_embedding.return_value = [mock_query_embedding.tolist()]
        ...
        with patch.object(
            ...
            # Mock prepare_edge_index to do nothing (assuming edges are not included)
            with patch.object(
        ...
        # Ensure that semantic_search_faiss was called correctly
        mock_semantic_search.assert_called_once()
        semantic_search_call_args = mock_semantic_search.call_args[1]
        assert (
            semantic_search_call_args["query_embeddings"][0].tolist()
            == mock_query_embedding.tolist()
        ), "semantic_search_faiss was not called with the correct query embeddings."
        assert (
            semantic_search_call_args["corpus_embeddings"].tolist()
            == query_subgraph_instance.node_embeddings.tolist()
        ), "semantic_search_faiss was not called with the correct corpus embeddings."
        assert (
            semantic_search_call_args["corpus_precision"]
            == query_subgraph_instance.precision
        ), "semantic_search_faiss was not called with the correct corpus_precision."
...
class TestQuerySubgraphEdgeCases:
    ...
    @patch("nxlu.processing.embed.semantic_search_faiss", autospec=True)
    @patch("nxlu.processing.embed.faiss.IndexFlatIP")
    @patch("nxlu.processing.embed.SentenceTransformerEmbedding.get_text_embeddings")
    def test_query_graph_with_large_k_values(
        ...
        mock_semantic_search.side_effect = [
            [{"corpus_id": i, "score": 0.9 - i * 0.1} for i in range(5)],  # Node search
        ...
        with patch.object(
            query_subgraph_instance.embedding_model,
            "get_query_embedding",
            return_value=[mocked_query_embedding.tolist()],
            autospec=True,
        ):
