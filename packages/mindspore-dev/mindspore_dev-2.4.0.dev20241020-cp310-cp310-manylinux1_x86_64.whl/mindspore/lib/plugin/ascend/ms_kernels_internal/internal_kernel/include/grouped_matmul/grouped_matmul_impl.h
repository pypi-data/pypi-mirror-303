 ...
 */
#ifndef MS_KERNELS_INTERNAL_KERNEL_GROUPED_MATMUL_IMPL_H_
#define MS_KERNELS_INTERNAL_KERNEL_GROUPED_MATMUL_IMPL_H_

#include "include/internal_kernel.h"
#include <vector>

namespace mindspore::internal {
class GroupedMatmulImpl : public InternelKernelImpl {
 public:
  GroupedMatmulImpl(const OpParamPtr &param) : InternelKernelImpl(param) {}
  virtual ~GroupedMatmulImpl() {}
 ...
 private:
  ...
  std::vector<uint8_t *> input_lists_;
  uint8_t *output_list_;
  uint32_t core_num_ = 1;
  size_t workspace_size_ = 0;
};
}  // namespace mindspore::internal
#endif  // MS_KERNELS_INTERNAL_KERNEL_GROUPED_MATMUL_IMPL_H_
