...
class AuthConfig:
    def __init__(self, account_id="", issuer_url="", client_id="", cognito_domain="", id_token="", refresh_token="", api_key=""):
...
def get_api_key():
    def _api_key_help():
        raise RuntimeError(f"Could not find Fireworks API key. Either pass API_KEY as an "
                          f"environment variable, or initialize ~/.fireworks/auth.ini, "
                          f"for example through `flumina set-api-key`")
