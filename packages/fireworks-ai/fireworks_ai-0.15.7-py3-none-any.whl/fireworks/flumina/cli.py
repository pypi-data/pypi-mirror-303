...
from .crud import (
  ...
  create_deployed_model, delete_deployed_model
)
...
def extract_components_from_resource_id(path, resource_type: str = "models"):
    ...
    if len(components) >= 4 and components[0] == "accounts" and components[2] == resource_type:
        ...
        raise ValueError(f"Expected resource name in the format accounts/{{account_id}}/{resource_type}/{{resource_id}} "
                         f"but got {path}")


def resolve_user_scoped_resource_name(name: str, user_account_id: str, resource_type: str):
    if '/' not in name:
...
async def _flumina_get_schema(args):
    user_account_id = await resolve_account_id(args)
    deployment_account_id, deployment_id = resolve_user_scoped_resource_name(args.deployment_name, user_account_id, resource_type="deployments")
...
async def _flumina_create_model(args):
    user_account_id = await resolve_account_id(args)
    model_account_id, model_id = resolve_user_scoped_resource_name(args.model_name, user_account_id, resource_type="models")
    ...
    if addon_type != "":
        ...
        if len(split) != 4 or split[0] != "accounts" or split[2] != "models":
            raise ValueError(f"--base-model must be a resource name like accounts/my-account/models/my-model")
    ...
    await upload_files_to_gcs(filename_to_uri=model_upload_endpoint['filenameToSignedUrls'])
...
async def _flumina_create_deployed_addon(args):
    user_account_id = await resolve_account_id(args)
    addon_account_id, addon_id = resolve_user_scoped_resource_name(args.addon_name, user_account_id, resource_type="models")
    deployment_account_id, deployment_id = resolve_user_scoped_resource_name(args.deployment_name, user_account_id, resource_type="deployments")

    # Validate addon exists and is the correct  type
    addon = await get_model(addon_account_id, addon_id)
    if addon['kind'] != "FLUMINA_ADDON":
        raise ValueError(f"Model {addon_account_id}/{addon_id} is not a Flumina addon (it is a {addon['kind']})")
    base_model_account_id, base_model_id = extract_components_from_resource_id(addon['peftDetails']['baseModel'])
    ...
    model_account_id, model_id = extract_components_from_resource_id(deployment['baseModel'])
    model = await get_model(model_account_id, model_id)
    if model['kind'] != "FLUMINA_BASE_MODEL":
        raise ValueError(f"Model {model_account_id}/{model_id} deployed on deployment {deployment_account_id}/{deployment_id} is not a Flumina model (it is a {model['kind']})")

    if (base_model_account_id != model_account_id) or (base_model_id != model_id):
        raise ValueError(f"Addon {addon_account_id}/{addon_id} expected base model {base_model_account_id}/{base_model_id} but deployment {deployment_account_id}/{deployment_id} has model {model_account_id}/{model_id}")
...
async def _flumina_deploy(args):
    ...
    try:
        ...
        get_logger().info(f"Model {model_account_id}/{model_id} already exists. Deploying that one...")
    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            get_logger().info(f"Model {model_account_id}/{model_id} does not exist, creating from current directory...")
    ...
    while True:
        try:
            ...
            await get_openapi_json(model_account_id, model_id, deployment_acct_id, deployment_id)
            break
        except httpx.HTTPStatusError as e:
