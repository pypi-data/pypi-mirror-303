...
def login(args):
    ...
    sql = args['sql']
    res = sql.exists_in_table('user', condition_dict={'username': username, 'password': password})
    if res:
        return {
            "success": True,
            "login_res_type": 0,
...
def register(args):
    ...
    if sql.exists_in_table('user', condition_dict={'username': username}):
        ...
        sql.insert_item('user', values=[username, email, password], col_names=['username', 'email', 'password'])
        return {"success": True,
                "register_res_type": 0,
