...
def auto_set_column_width(worksheet):
    ...
    for column_cells in worksheet.columns:
        ...
        for cell in column_cells:
            ...
            # Skip the column if all cells are merged cells
            continue

        for cell in column_cells:
            if isinstance(cell, Cell):  # Only process regular cells
                try:
...
def copy_worksheet(username, password, server_ip, server_name, share_name, source_file_path, source_sheet_name, target_file_path, target_sheet_name, port=445):
    ...
    for row in source_sheet.iter_rows():
        for cell in row:
            ...
            merged_range = next((cell_range for cell_range in source_sheet.merged_cells.ranges if cell.coordinate in cell_range), None)
            if merged_range is not None:
                # Get the top-left cell's value from the merged range
                min_row, min_col, _, _ = range_boundaries(str(merged_range))
                top_left_cell = source_sheet.cell(row=min_row, column=min_col)

                # Only set the value if new_cell is not a MergedCell
                if new_cell.coordinate not in source_sheet.merged_cells:
                    new_cell.value = top_left_cell.value  # This works since new_cell is not a MergedCell
            else:
