...
def copy_worksheet(username, password, server_ip, server_name, share_name, source_file_path, source_sheet_name, target_file_path, target_sheet_name, port=445):
    ...
    for row in source_sheet.iter_rows():
        for cell in row:
            ...
            # Check if the current cell is in a merged range
            merged_cell_ranges = [merged_cell for merged_cell in source_sheet.merged_cells.ranges if cell.coordinate in merged_cell]
            if merged_cell_ranges:
                # Get the top-left cell of the merged range
                top_left_cell = source_sheet.cell(row=merged_cell_ranges[0].min_row, column=merged_cell_ranges[0].min_col)
                new_cell.value = top_left_cell.value  # Set the value from the top-left cell
            else:
