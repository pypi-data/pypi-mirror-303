...
def copy_worksheet(username, password, server_ip, server_name, share_name, source_file_path, source_sheet_name, target_file_path, target_sheet_name, port=445):
    ...
    for row in source_sheet.iter_rows():
        for cell in row:
            ...
            in_merged_cells = any(cell.coordinate in merged_range for merged_range in source_sheet.merged_cells.ranges)
            if in_merged_cells:
                # Get the value of the merged cell
                merged_cell_value = source_sheet.cell(row=cell.row, column=cell.column).value
                new_cell.value = merged_cell_value
            else:
