...
def get_grpc_data(stub_api_fn, request_data=None, type="proto", identity=None, metadata=[]):
    # Setup the request data
    request_data = get_request_data(request_data)

    # If identity is provided, add it to the metadata
    if identity is not None:
        metadata.append(("x-user-identity", identity))

    # Wait for a response
    response = stub_api_fn(request_data, metadata=metadata)
...
async def get_grpc_data_async(stub_api_fn, request_data=None, type="proto", identity=None, metadata=[]):
    # Setup the request data
    request_data = get_request_data(request_data)
    ...
    # Wait for a response
    response = await stub_api_fn(request_data, metadata=metadata)
...
def get_grpc_data_stream(stub_api_fn, request_data=None, type="proto", identity=None, metadata=[]):
    # Setup the request data
    request_data = get_request_data(request_data)
...
async def get_grpc_data_stream_async(stub_api_fn, request_data=None, type="proto", identity=None, metadata=[]):
    # Setup the request data
    request_data = get_request_data(request_data)
