...
class RPowerPlan(Base):
    ...
    def sort_by_power(self, solarpower: list, ts_utc_now: float) -> list:
        ...
        """
        ts_utc_quantized = self.quantize(3600, ts_utc_now)
        self.debug(
            f"Sorting {len(solarpower)} days of forecast starting at {self.timestampstr(ts_utc_quantized)}"
        )
    ...
    def on_powerplan(self, ts_utc_now: float) -> None:
        ...
        tsstr: str = self.timestampstr(ts_utc_now)
    ...
    def consider_net_energy_balance(self, ts: float) -> bool:
        ...
        elapsed_ts = ts - self.quantize(self.energy_balancing_interval, ts)
    ...
    def consider_heating(self, ts: float) -> int:
        ...
        if self.consider_net_energy_balance(ts):
            ...
            balancing_slot_start_ts = self.quantize(self.energy_balancing_interval, ts)
        ...
        if self.main_boiler_temperature > self.maximum_boiler_temperature:
            ...
            return 0
        hour = self.timestamp_hour(ts)
        for pp in self.heating_plan:
            ppts: float = pp["Timestamp"]
            h: int = self.timestamp_hour(ppts)
    ...
    def create_heating_plan(self) -> list:
        ...
        for hp in self.power_plan:
            ...
            self.debug(
                f"Heating entry {self.timestampstr(ts)} with spot {spot} and state {state}"
            )
