...
class MapTypeCreator:
    ...
    async def get_collection(self, c: Client, col_id: collection.ID) -> collection.Collection:
        ...
        ret = await self.con[id_sap].exchange(c)
        async with self.lock:
            try:
                ...
                if gotten.has_sap(id_sap.sap):
                    ...
                    self.adapter.set_collection(ret)  # todo: make as ADAPTER.merge_collection(ret)
