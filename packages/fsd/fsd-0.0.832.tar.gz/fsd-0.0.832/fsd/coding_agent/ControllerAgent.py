...
class ControllerAgent:
    ...
    async def get_pro_coding_requests(
        ...
        role, crawl_logs, original_prompt_language,file_attachments
    ):
        ...
        for group in plan.get('groups', []):
            ...
            for agent in coding_agents:
                agent.initial_setup(context_files, instructions, context, crawl_logs, file_attachments)
    ...
    async def get_coding_requests(self, instructions, context, file_lists, context_files, role, crawl_logs, original_prompt_language, file_attachments):
        """Generate coding requests based on instructions and context."""
        self.coder.initial_setup(context_files, instructions, context, crawl_logs, file_attachments)
    ...
    async def explainer_code_task_pipeline(self, tier, solution, role, original_prompt_language, file_attachments):
        """Pipeline for regular coding tasks."""

        try:
            logger.info("-------------------------------------------------")
            logger.debug("#### Initiating regular_code_task_pipeline")  
            asyncio.create_task(self.knowledge.get_knowledge_summary_plan(solution))
            ...
            commits = await (self.get_pro_coding_requests if tier == "Pro" else self.get_coding_requests)(
                final_request, all_context, list(final_working_files), context_files, role, crawl_logs, original_prompt_language, file_attachments
            )
    ...
    async def codingProgress(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files):
        ...
        asyncio.create_task(self.knowledge.get_knowledge_summary_plan(idea_plan))
        logger.info(" #### The `File Manager Agent` is processing files.")
