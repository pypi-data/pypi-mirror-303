        ...
        while True:
            if not isFirst:
                ...
                try:
                    ...
                    error_message = f"An error occurred in the QA Mode loop: {str(e)}"
                    logger.error(error_message)
                    log_error(error_message, "ExplainerController.get_started")
                    logger.info(" #### You're in `QA Mode`! Type your question to get support, or click `Exit` to leave.")
                    logger.info("\n #### Are you satisfied with this development plan? Enter \"yes\" if satisfied, or provide feedback for modifications: ")
            ...
            if user_prompt == "h":
                ...
                try:
                    ...
                    error_message = f"An error occurred in the main processing loop: {str(e)}"
                    logger.error(error_message)
                    log_error(error_message, "ExplainerController.get_started")
