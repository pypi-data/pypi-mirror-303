...
from fsd.util.utils import read_file_content, log_error
...
class SelfHealingAgent:
    ...
    async def get_fixing_request(self, instruction, file_content, all_file_content, tech_stack):
        ...
        try:
            ...
            error_message = f"The `SelfHealingAgent` encountered an error during the fixing request: {e}"
    ...
    async def replace_all_code_in_file(self, file_path, result):
        ...
        if file_path:
            ...
            error_message = f"The `SelfHealingAgent` could not locate the file: `{file_path}`"
            logger.debug(f" #### {error_message}")
            log_error(error_message, file_path)
