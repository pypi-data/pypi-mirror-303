...
class CompileCommandRunner:
    ...
    def run_localhost_command(self, command, current_path):
        ...
        if port_already_in_use_line:
            ...
            log_error(error_message, str(return_code))
    ...
    def update_file(self, file_name, content):
        ...
        try:
            ...
            error_message = f" #### `FileUpdater` failed to update {file_name}: {str(e)}"
            logger.error(error_message)
            log_error(error_message, str(e))
        ...
        while retries < max_retries:
            ...
            try:
                ...
                if http_error.response.status_code == 429:
                    ...
                    error_message = f" #### `HTTPErrorHandler` encountered an HTTP error: {http_error}"
                    logger.error(error_message)
                    log_error(error_message, str(http_error))
                    raise
            except Exception as e:
                error_message = f" #### `ErrorHandler` encountered an error during the fixing process: {str(e)}"
                logger.error(error_message)
                log_error(error_message, str(e))
            ...
            while retry_count < self.max_retry_attempts:
                if step['method'] == 'bash':
                    ...
                    if return_code != 0:
                        ...
                        log_error(f" #### `CommandExecutor` failed with return code {return_code}", error_message)
                    ...
                    error_message = f" #### `StepExecutor` encountered an unknown method: {step['method']}"
                    logger.error(error_message)
                    log_error(error_message, step['method'])
                    break

            if retry_count == self.max_retry_attempts:
                error_message = f" #### `StepExecutor`: Step failed after {self.max_retry_attempts} attempts: {step['prompt']}"
                logger.error(error_message)
                log_error(error_message, step['prompt'])
                fixing_steps = await self.get_error_planner_requests(
                    error_message, step['prompt'], self.detector, compile_files, original_prompt_language)
        ...
        for step in steps:
            ...
            while retry_count < self.max_retry_attempts:
                if step['method'] == 'bash':
                    ...
                    error_message = f" #### `FixingStepExecutor` encountered an unknown method: {step['method']}"
                    logger.error(error_message)
                    log_error(error_message, step['method'])
                    break

            if retry_count == self.max_retry_attempts:
                error_message = f" #### `FixingStepExecutor`: Step failed after {self.max_retry_attempts} attempts: {step['error_resolution']}"
                logger.error(error_message)
                log_error(error_message, step['error_resolution'])
                fixing_steps = await self.get_error_planner_requests(
                    error_message, step['error_resolution'], self.detector, compile_files, original_prompt_language)
                fixing_result = await self.execute_fixing_steps(fixing_steps, compile_files, code_files)
                if fixing_result == "Execution stopped by user":
