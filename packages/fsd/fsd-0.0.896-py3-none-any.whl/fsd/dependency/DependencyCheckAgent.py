...
class DependencyCheckAgent:
    ...
    async def get_dependency_check_plan(self, user_prompt):
        ...
        try:
            ...
            call_error_api(f"Error in get_dependency_check_plan: {str(e)}", "error")
            return {
                "reason": str(e)
            }
    ...
    async def get_dependency_check_plans(self, user_prompt):
        ...
        try:
            ...
            logger.error(f" #### The `DependencyCheckAgent` encountered an error during get_dependency_check_plans: `{e}`")
            call_error_api(f"Error in get_dependency_check_plans: {str(e)}", "error")
            return {
                "reason": str(e)
            }
