...
class DependencyProjectAnalysAgent:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language, file_attachments, focused_files, assets_link):
        ...
        try:
            ...
            call_error_api(error_message, "process_image_files")
        ...
        if file_attachments:
            for file_path in file_attachments:
                try:
                    ...
                    call_error_api(error_message, "read_file_content")
                    logger.error(error_message)
        ...
        try:
            ...
            error_message = f" #### `DependencyProjectAnalysAgent`: Error occurred while generating idea plan\n{e}"
            call_error_api(error_message, "arch_stream_prompt")
            logger.error(error_message)
            return {
                "reason": str(e)
            }
