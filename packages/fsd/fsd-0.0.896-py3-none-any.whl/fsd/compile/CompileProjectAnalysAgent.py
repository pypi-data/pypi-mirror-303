...
from fsd.util.utils import read_file_content, call_error_api, process_image_files
...
class CompileProjectAnalysAgent:
    ...
    def initial_setup(self, dependency_files, OS_architecture):
        ...
        try:
            ...
            error_message = f"Error printing repository tree: {str(e)}"
            call_error_api(error_message, "initial_setup")
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language, file_attachments, focused_files, assets_link):
        ...
        try:
            ...
            error_message = f"Error processing image files: {str(e)}"
            call_error_api(error_message, "get_idea_plan")
            logger.error(error_message)
        ...
        if file_attachments:
            for file_path in file_attachments:
                try:
                    ...
                    error_message = f"Error reading attachment file {file_path}: {str(e)}"
                    call_error_api(error_message, "get_idea_plan")
                    logger.error(error_message)
        ...
        if focused_files:
            for file_path in focused_files:
                try:
                    ...
                    error_message = f"Error reading focused file {file_path}: {str(e)}"
                    call_error_api(error_message, "get_idea_plan")
                    logger.error(error_message)
        ...
        try:
            ...
            error_message = f"AIGateway encountered an error while generating idea plan: {str(e)}"
            call_error_api(error_message, "get_idea_plan")
            logger.error(error_message)
            return {
