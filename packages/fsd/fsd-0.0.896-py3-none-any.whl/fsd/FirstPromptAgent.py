...
class FirstPromptAgent:
    ...
    async def get_prePrompt_plans(self, user_prompt):
        ...
        try:
            ...
            call_error_api("Error during pre-prompt planning", str(e))
            return {"reason": str(e)}
    ...
    def _parse_response(self, response):
        ...
        try:
            ...
            logger.error(f" The `Receptionist Agent` encountered an error and is attempting to repair itself.")
            logger.debug(f"DAMAGE RESPOND: {response.choices[0].message.content}")
            call_error_api("JSON parsing error", response.choices[0].message.content)
