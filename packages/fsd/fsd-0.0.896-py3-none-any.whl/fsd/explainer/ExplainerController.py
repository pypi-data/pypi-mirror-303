...
class ExplainerController:
    ...
    async def get_prePrompt(self, user_prompt, file_attachments, focused_files):
        ...
        try:
            ...
            call_error_api(error_message, "get_prePrompt")
            raise
    ...
    async def get_normal_answer(self, user_prompt, language, role, file_attachments, focused_files, assets_link, crawl_logs):
        ...
        try:
            ...
            error_message = f"Error in get_normal_answer: {str(e)}"
            logger.error(error_message)
            call_error_api(error_message, "get_normal_answer")
            raise
    ...
    async def get_file_answer(self, user_prompt, language, files, role, file_attachments, focused_files, assets_link, crawl_logs):
        ...
        try:
            ...
            error_message = f"Error in get_file_answer: {str(e)}"
            logger.error(error_message)
            call_error_api(error_message, "get_file_answer")
            raise
    ...
    async def get_explaining_files(self, prompt, file_attachments, focused_files):
        ...
        try:
            ...
            error_message = f"Error in get_explaining_files: {str(e)}"
            logger.error(error_message)
            call_error_api(error_message, "get_explaining_files")
            raise
        ...
        while True:
            if not isFirst:
                ...
                try:
                    ...
                    error_message = f"Error in get_started loop: {str(e)}"
                    logger.error(error_message)
                    call_error_api(error_message, "get_started_loop")
                    logger.info(" #### You're in `QA Mode`! Type your question to get support, or click `Exit` to leave.")
                    logger.info("\n #### Are you satisfied with this development plan? Enter \"yes\" if satisfied, or provide feedback for modifications: ")
            ...
            if user_prompt == "h":
                try:
                    ...
                    logger.error(error_message)
                    call_error_api(error_message, "explainer_code_task_pipeline")
            else:
                try:
                    ...
                    error_message = f"Error in processing user prompt: {str(e)}"
                    logger.error(error_message)
                    call_error_api(error_message, "process_user_prompt")
