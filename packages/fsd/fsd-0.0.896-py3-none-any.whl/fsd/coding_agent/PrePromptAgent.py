...
class PrePromptAgent:
    ...
    async def get_prePrompt_plan(self, user_prompt):
        ...
        try:
            ...
            error_message = f"JSON decode error in PrePromptAgent: {str(e)}"
            call_error_api(error_message, response.choices[0].message.content)
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            error_message = f"The `PrePromptAgent` encountered an error during plan generation: {str(e)}"
