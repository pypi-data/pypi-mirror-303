...
class CompilePrePromptAgent:
    ...
    async def get_prePrompt_plan(self, all_file_contents, user_prompt):
        ...
        try:
            ...
            error_message = "JSON decoding error in CompilePrePromptAgent"
            call_error_api(error_message, str(response.choices[0].message.content))
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            error_message = f"The `CompilePrePromptAgent` encountered an error: `{e}`"
    ...
    async def get_prePrompt_plans(self, user_prompt):
        ...
        try:
            ...
            error_message = f"Error in get_prePrompt_plans: {str(e)}"
            call_error_api(error_message, str(e))
            logger.error(error_message)
            return {"reason": str(e)}
