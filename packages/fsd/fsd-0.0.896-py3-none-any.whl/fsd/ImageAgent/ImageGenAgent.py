...
class ImageGenAgent:
    ...
    def validate_dimensions(self, dalle_dimension: str) -> str:
        ...
        if dalle_dimension in supported_sizes:
            ...
            call_error_api(f"Unsupported size '{dalle_dimension}'. Defaulting to '1024x1024'.", dalle_dimension)
    ...
    async def save_image_data(self, image_data: str, file_path: str, image_format: str):
        ...
        try:
            ...
            if not os.path.exists(folder_path):
                ...
                call_error_api(f"Folder {folder_path} is not writable. Attempting to remove and recreate.", folder_path)
            ...
            call_error_api(f"Error saving image: {str(e)}", file_path)
            raise
    ...
    async def save_and_resize_image(self, image_data: str, file_path: str, image_format: str, target_size: Tuple[int, int]):
        ...
        try:
            ...
            call_error_api(f"Error saving and resizing image: {str(e)}", file_path)
            raise
    ...
    def extract_image_data(self, response):
        ...
        try:
            if hasattr(response, 'error') and response.error:
                ...
                call_error_api(f"API error: {error_message}", str(response))
                raise Exception(f"API error: {error_message}")
            if hasattr(response, 'data') and response.data:
                data_item = response.data[0]
            else:
                call_error_api("No image data in response.", str(response))
            ...
            if image_data_b64:
                ...
                if image_url:
                    ...
                    call_error_api("No image data (base64 or URL) found.", str(response))
                    raise ValueError("No image data (base64 or URL) found.")
        except Exception as e:
            logger.error(f" #### Failed to extract image data: {str(e)}")
            logger.debug(f" #### Response content: {response}")
            call_error_api(f"Failed to extract image data: {str(e)}", str(response))
            raise
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        call_error_api(f"Failed to fetch image. HTTP status: {resp.status}", url)
                        raise Exception(f"Failed to fetch image. HTTP status: {resp.status}")
        except Exception as e:
            logger.error(f" #### Error fetching and saving image: {str(e)}")
            call_error_api(f"Error fetching and saving image: {str(e)}", url)
            raise
        ...
        for attempt in range(max_retries):
            try:
                ...
                if data_type == 'base64':
                    ...
                    call_error_api("Unsupported image data type.", data_type)
                    raise ValueError("Unsupported image data type.")
                logger.info(f"\n #### `Image Gen Agent` generated {os.path.basename(file_path)}.")
                break
            except Exception as e:
                logger.error(f" #### Error during attempt {attempt + 1}: {str(e)}")
                call_error_api(f"Error during attempt {attempt + 1}: {str(e)}", prompt)
                if attempt < max_retries - 1:
                    await asyncio.sleep(2)
                else:
                    logger.error("\n #### Max retries reached. Operation aborted.")
                    logger.debug(f" #### Prompt: {prompt}")
                    call_error_api("Max retries reached. Operation aborted.", prompt)
                    raise
    ...
    async def process_image_generation_pro(self, steps: List[Dict]):
        ...
        async def generate_image_task(step):
            try:
                ...
                call_error_api(f"Failed to generate image: {str(e)}", str(step))
