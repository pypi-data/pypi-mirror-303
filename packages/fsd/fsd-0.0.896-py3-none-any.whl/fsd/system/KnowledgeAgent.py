...
class KnowledgeAgent:
    ...
    async def get_knowledge_summary(self, user_prompt):
        ...
        try:
            ...
            error_message = f" #### The `KnowledgeAgent` encountered an error: {e}"
    ...
    async def get_knowledge_summary_plan(self, user_prompt):
        ...
        if not os.path.exists(knowledge_file_path):
            try:
                ...
                error_message = f"Error creating knowledge file: {e}"
                logger.error(error_message)
                call_error_api(error_message, str(e))
                return
        ...
        try:
            ...
            error_message = f"Error writing to knowledge1.txt: {e}"
            logger.error(error_message)
            call_error_api(error_message, str(e))
        ...
        try:
            ...
            error_message = f"Error handling coding agent response: {e}"
            logger.error(error_message)
            call_error_api(error_message, str(e))
