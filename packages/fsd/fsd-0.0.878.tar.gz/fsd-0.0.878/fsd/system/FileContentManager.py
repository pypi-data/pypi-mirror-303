...
class FileContentManager:
    ...
    def parse_search_replace_blocks(response: str) -> List[Tuple[str, str]]:
        ...
        # Enforces 3 to 10 delimiters, word boundaries, and line anchors
        pattern = r'^<{3,10}\bSEARCH\b\s*\n(.*?)={3,10}\s*\n(.*?)>{3,10}\bREPLACE\b$'

        # Compile the pattern with DOTALL, IGNORECASE, and MULTILINE flags
        regex = re.compile(pattern, re.DOTALL | re.IGNORECASE | re.MULTILINE)

        # Find all matches in the response
        matches = regex.findall(response)

        # Raise an error if no blocks are found, with logging
        if not matches:
            logger.error("No valid SEARCH/REPLACE blocks found. Check delimiter counts and formatting.")
