...
class ControllerAgent:
            ...
            if self.is_coding_file(full_file):
                ...
                if main_path:
                    ...
                    try:
                        ...
                        error_message = f"Error processing file `{full_file}`"
        ...
        for group in plan.get('groups', []):
            ...
            for result in results:
                if isinstance(result, Exception):
                    ...
                    log_error(error_message, str(result))
                    logger.error(f"{error_message}: {result}")
    ...
    async def log_result(self, main_path: str, result: str):
        ...
        try:
            ...
            error_message = f"Error writing to log file {full_log_path}"
            log_error(error_message, str(e))
            logger.error(f" #### {error_message}: {str(e)}")
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    try:
                        ...
                        error_message = f"Error processing file `{file_name}`"
                        log_error(error_message, str(e))
                        logger.error(f" #### {error_message}: {str(e)}")
                else:
    ...
    async def explainer_code_task_pipeline(self, tier, solution, role, original_prompt_language, file_attachments, assets_link, snow_mode):
        ...
        try:
            ...
            error_message = "The `Coding Agent` has encountered some bugs. We apologize for the inconvenience. Please try again!"
    ...
    async def regular_code_task_pipeline(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files, snow_mode):
        try:
            ...
            error_message = f" #### The `Coding Agent` has encountered an error. We apologize for the inconvenience. Please try again!"
            log_error(error_message, str(e))
    ...
    async def get_started(self, user_prompt, tier, file_attachments, focused_files, snow_mode):
        ...
        if pipeline in ["1", 1]:
            ...
            error_message = f"Invalid pipeline: {pipeline}"
            log_error(error_message, user_prompt)
