...
from fsd.util.utils import parse_payload, log_error
...
class CompileControllerAgent:
    ...
    async def start_CLI_compile_process(self, instruction, code_files, original_prompt_language, file_attachments, focused_files):
        ...
        try:
            ...
            error_message = f"Error in get_compile_file_plannings: {str(e)}"
            logger.error(error_message)
            log_error(error_message, "get_compile_file_plannings")
        ...
        try:
            ...
            error_message = f"Error in process_image_links: {str(e)}"
            logger.error(error_message)
            log_error(error_message, "process_image_links")
            assets_link = []
        ...
        try:
            ...
            logger.error(error_message)
            log_error(error_message, "get_idea_plans")
        ...
        while True:
            ...
            try:
                ...
                error_message = f"Error in parse_payload: {str(e)}"
                logger.error(error_message)
                log_error(error_message, "parse_payload")
                continue
        ...
        try:
            ...
            error_message = f"Error in get_task_plan: {str(e)}"
            logger.error(error_message)
            log_error(error_message, "get_task_plan")
        try:
            await self.command.execute_steps(task, compile_files, code_files, original_prompt_language)
        except Exception as e:
            error_message = f"Error in execute_steps: {str(e)}"
            logger.error(error_message)
            log_error(error_message, "execute_steps")
    ...
    async def get_started(self, user_prompt, original_prompt_language, file_attachments, focused_files):
        ...
        try:
            ...
            logger.error(error_message)
            log_error(error_message, "get_prePrompt")
            return
