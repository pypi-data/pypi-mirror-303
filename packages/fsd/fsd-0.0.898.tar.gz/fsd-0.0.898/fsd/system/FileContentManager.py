...
class FileContentManager:
    ...
    def parse_search_replace_blocks(response: str) -> List[Tuple[str, str]]:
        ...
        if not matches:
            ...
            log_error(error_message, response)
        ...
        while i < len(lines):
            ...
            if line.startswith(cls.HEAD):
                ...
                while i < len(lines) and not lines[i].strip().startswith(cls.DIVIDER):
                    ...
                    i += 1
                if i >= len(lines):
                    error_message = "SEARCH block not properly closed with DIVIDER."
                    logger.error(error_message)
                    log_error(error_message, response)
                    raise ValueError("Incomplete SEARCH/REPLACE block: Missing DIVIDER.")
                ...
                while i < len(lines) and not lines[i].strip().startswith(cls.UPDATED):
                    ...
                    i += 1
                if i >= len(lines):
                    error_message = "REPLACE block not properly closed with UPDATED."
                    logger.error(error_message)
                    log_error(error_message, response)
                    raise ValueError("Incomplete SEARCH/REPLACE block: Missing UPDATED.")
        ...
        for search, replace in blocks:
            if search:
                ...
                if new_content is None:
                    ...
                    logger.error(error_message)
                    log_error(error_message, search)
