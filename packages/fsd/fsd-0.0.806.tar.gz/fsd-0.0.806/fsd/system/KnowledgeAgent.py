...
class KnowledgeAgent:
    ...
    async def get_knowledge_summary(self, user_prompt):
        ...
        system_prompt = f"""You are an expert notetaker to not whatever matter to be reused, keep it as concise as short and focused as possible. Follow these guidelines strictly when responding to instructions:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"Current file context: {current_summary}. This is the scope that needs to be analyzed ---- \n{user_prompt}"
            }
        ]
    ...
    async def get_knowledge_summary_plan(self, user_prompt):
        ...
        # Append the new summary to the existing file
        with open(knowledge_file_path, 'a') as f:
            f.write(f"\n\n{summary}")
