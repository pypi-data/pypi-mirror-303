...
class FileContentManager:
    ...
    UPDATED = ">>>>>>> REPLACE"
    DEFAULT_FENCE = ("```", "```")
    SIMILARITY_THRESHOLD = 0.6  # Can be made configurable
    ...
    def parse_search_replace_blocks(response: str) -> List[Tuple[str, str]]:
        ...
        pattern = rf'{re.escape(FileContentManager.HEAD)}\n(.*?)\n{re.escape(FileContentManager.DIVIDER)}\n(.*?)\n{re.escape(FileContentManager.UPDATED)}'
    ...
    def replace_part_with_missing_leading_whitespace(whole: str, part: str, replace: str) -> Optional[str]:
        ...
        for i in range(len(whole_lines) - part_len + 1):
            ...
            if part_tup == whole_tup:
                ...
                leading_whitespace = re.match(r'\s*', whole_lines[i]).group()
