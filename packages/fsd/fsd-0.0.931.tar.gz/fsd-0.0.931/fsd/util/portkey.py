...
class AzureModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        ...
        return await self._execute_with_retry(self.portkey.chat.completions.create, **common_params)
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        ...
        return await self._execute_with_retry(self.portkey.chat.completions.create_stream, **common_params)
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in AzureModel (attempt {attempt + 1}): {str(e)}")
...
class BedrockModel(BaseModel):
    ...
    async def coding_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float, model_version: str = "anthropic.claude-3-5-sonnet-20241022-v2:0") -> Dict:
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in BedrockModel (attempt {attempt + 1}): {str(e)}")
                if "Too many requests" in str(e) and attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                    if attempt == max_retries - 2:
                        if kwargs.get("model") == "anthropic.claude-3-5-sonnet-20241022-v2:0":
                            kwargs["model"] = "anthropic.claude-3-5-sonnet-20240620-v1:0"
                            logger.debug("Switching to v1 model for the last retry")
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in BedrockOpusModel (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                else:
                    raise
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in GeminiModel (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                else:
                    raise
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in GeminiFlashModel (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                else:
                    raise
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in LlamaModel (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                else:
                    raise
    ...
    async def _execute_with_retry(self, func, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error in CodingLlamaModel (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                else:
                    raise
...
class AIGateway:
    ...
    def _select_model(self, weights, exclude=None):
        ...
        available_models = {k: v for k, v in weights.items() if k not in exclude}
    ...
    async def prompt(self,
        ...
        logger.debug(f"All models failed to respond after trying: {', '.join(tried_models)}")
        raise Exception(f"All models failed to respond after trying: {', '.join(tried_models)}")
    ...
    async def arch_prompt(self,
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    logger.debug("Retrying in 1-3 seconds...")
    ...
    async def coding_prompt(self,
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    logger.debug("Retrying in 2-3 seconds...")
                    await asyncio.sleep(random.uniform(2, 3))
                else:
                    logger.error(f"All attempts failed for coding prompt. Last error: {str(e)}")
                    raise Exception(f"All attempts failed for coding prompt. Last error: {str(e)}")

    async def _stream_response(self, model, messages, max_tokens, temperature, top_p):
    ...
    async def stream_prompt(self,
        ...
        while len(tried_models) < len(self.STREAM_MODEL_WEIGHTS):
            ...
            try:
                ...
                model = self.get_model(model_type)
                final_response = await self._stream_response(model, messages, max_tokens, temperature, top_p)
                return final_response
            except Exception as e:
                logger.debug(f"Error in streaming from {model_type} model: {str(e)}")

        logger.debug(f"All models failed to respond for stream prompt after trying: {', '.join(tried_models)}")
        raise Exception(f"All models failed to respond for stream prompt after trying: {', '.join(tried_models)}")
    ...
    async def explainer_stream_prompt(self,
        ...
        logger.debug(f"All models failed to respond for explainer stream prompt after trying: {', '.join(tried_models)}")
        raise Exception(f"All models failed to respond for explainer stream prompt after trying: {', '.join(tried_models)}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.debug("All attempts failed for arch stream prompt")
                    raise Exception("All attempts failed for arch stream prompt")
        ...
        while len(tried_models) < len(self.IMAGE_MODEL_WEIGHTS):
            ...
            try:
                if model_type == "dalle3":
                    ...
                    logger.debug(f"Using {model_id} model for image generation")
