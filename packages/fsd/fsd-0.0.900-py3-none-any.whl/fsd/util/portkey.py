    ...
    async def prompt(self, 
        ...
        while len(tried_models) < len(self.MODEL_WEIGHTS):
            ...
            try:
                ...
                error_message = f"Error in prompting {model_type} model: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_model")
        
        error_message = "All models failed to respond"
        logger.debug(error_message)
        log_error(error_message, "all_models")
    ...
    async def arch_prompt(self, 
        ...
        for attempt in range(3):
            try:
                ...
                error_message = f"Error in prompting {model_type} model for architecture: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_model_architecture")
                if attempt < 2:
                    ...
                    logger.debug(error_message)
                    log_error(error_message, "arch_prompt")
                    raise Exception(error_message)
    ...
    async def coding_prompt(self, 
        ...
        for attempt in range(3):
            try:
                ...
                error_message = f"Error in prompting {model_type} model for coding: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_model_coding")
                if attempt < 2:
                    ...
                    logger.debug(error_message)
                    log_error(error_message, "coding_prompt")
                    raise Exception(error_message)
    ...
    async def stream_prompt(self, 
        ...
        while len(tried_models) < len(self.STREAM_MODEL_WEIGHTS):
            ...
            try:
                ...
                error_message = f"Error in streaming from {model_type} model: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_model_streaming")
        
        error_message = "All models failed to respond for stream prompt"
        logger.debug(error_message)
        log_error(error_message, "all_models_streaming")
        raise Exception(error_message)
    ...
    async def explainer_stream_prompt(self, 
        ...
        while len(tried_models) < len(self.STREAM_EXPLAINER_MODEL_WEIGHTS):
            ...
            try:
                ...
                error_message = f"Error in streaming from {model_type} model: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_model_explainer_streaming")
        
        error_message = "All models failed to respond for explainer stream prompt"
        logger.debug(error_message)
        log_error(error_message, "all_models_explainer_streaming")
        raise Exception(error_message)
    ...
    async def arch_stream_prompt(self, 
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_message = f"Error in streaming from {model_type} model: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_model_arch_streaming")
                if attempt < max_retries - 1:
                    ...
                    logger.debug(error_message)
                    log_error(error_message, "arch_stream_prompt")
                    raise Exception(error_message)
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        ...
        while len(tried_models) < len(self.IMAGE_MODEL_WEIGHTS):
            ...
            try:
                ...
                error_message = f"Error in generating image with {model_type}: {str(e)}"
                logger.debug(error_message)
                log_error(error_message, f"{model_type}_image_generation")
                if len(tried_models) == len(self.IMAGE_MODEL_WEIGHTS):
                    error_message = "All image generation models failed"
                    log_error(error_message, "all_image_generation_models")
                    raise Exception(error_message)
