...
class KnowledgeAgent:
    ...
    async def get_knowledge_summary(self, user_prompt):
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"Provide a concise, focused summary of the key points as short as possible to remember, learn and serve human better. Use markdown format. Separate the summary into the following sections: 'Code', 'User Preferences', 'Cautions', and 'Bugs'. Highlight code snippets where relevant. Also, analyze user behavior patterns: ---- \n{user_prompt}"
            }
        ]
    ...
    async def get_knowledge_summary_plan(self, user_prompt):
        ...
        await self.knowledge_manager.handle_coding_agent_response(os.path.join(self.repo.get_repo_path(), '.knowledge.md'), summary)
