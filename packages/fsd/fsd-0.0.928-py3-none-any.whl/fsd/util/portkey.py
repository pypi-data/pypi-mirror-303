        ...
        for attempt in range(max_retries):
            try:
                ...
                if "Too many requests" in str(e) and attempt < max_retries - 1:
                    await asyncio.sleep(random.uniform(2, 5))
                    if attempt == max_retries - 2:  # Last retry, switch to v1
                        kwargs["model"] = "anthropic.claude-3-5-sonnet-20240620-v1:0"
                        logger.info("Switching to v1 model for the last retry")
        ...
        for attempt in range(3):
            try:
                ...
                if "Too many requests" in str(e) and attempt < 2:
                    logger.debug(f"Rate limit hit. Retrying in 2-3 seconds...")
                    await asyncio.sleep(random.uniform(2, 3))
                    if attempt == 1:  # Last retry, switch to v1
                        model.portkey.chat.completions.create.model = "anthropic.claude-3-5-sonnet-20240620-v1:0"
                        logger.info("Switching to v1 model for the last retry")
                elif attempt < 2:
