...
class FileContentManager:
    ...
    def parse_search_replace_blocks(response: str) -> List[Tuple[str, str]]:
        ...
        returning search and replace content as tuples. More flexible to handle missing < or = characters.
        """
        # Regular expression pattern to capture multiple SEARCH/REPLACE blocks, allowing for missing < or = characters
        pattern = r'<?<<?<<?<*\s*SEARCH\s*\n(.*?)=*\s*\n(.*?)>*>*>*>*\s*REPLACE'

        # Find all matches in the response
        matches = re.findall(pattern, response, re.DOTALL | re.IGNORECASE)

        # If no matches found, try a more lenient pattern
        if not matches:
            lenient_pattern = r'SEARCH\s*\n(.*?)\n.*?REPLACE\s*\n(.*?)(?:\n|$)'
            matches = re.findall(lenient_pattern, response, re.DOTALL | re.IGNORECASE)

        # Raise an error if still no blocks are found
        if not matches:
