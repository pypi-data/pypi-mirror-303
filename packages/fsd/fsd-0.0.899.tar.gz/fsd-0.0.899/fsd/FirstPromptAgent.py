...
class FirstPromptAgent:
    ...
    async def get_prePrompt_plans(self, user_prompt):
        ...
        try:
            ...
            error_message = " The `FirstPromptAgent` encountered an error during pre-prompt planning"
    ...
    def _parse_response(self, response):
        ...
        try:
            ...
            error_message = " The `Receptionist Agent` encountered an error and is attempting to repair itself."
            logger.error(f"{error_message}")
            log_error(error_message, response.choices[0].message.content)
