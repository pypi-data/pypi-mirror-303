...
class CompileTaskPlanner:
    ...
    async def get_task_plan(self, instruction, os_architecture, original_prompt_language):
        ...
        try:
            ...
            error_response = call_error_api(f"Error generating task plan", str(e))
            return {"reason": str(e)}
    ...
    async def get_task_plans(self, instruction, original_prompt_language):
        ...
        try:
            ...
            logger.error(f"Error retrieving task plan: {e}")
            error_response = call_error_api(f"Error retrieving task plan", str(e))
            return {"reason": str(e)}
