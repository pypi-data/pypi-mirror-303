...
from fsd.util.utils import read_file_content, process_image_files, call_error_api
...
class FileManagerAgent:
    ...
    async def get_file_planning(self, idea):
        ...
        try:
            ...
            call_error_api("JSON decode error in get_file_planning", str(e))
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            logger.debug(f" #### The `FileManagerAgent` encountered an unexpected error: `{e}`")
            call_error_api("Unexpected error in get_file_planning", str(e))
            return {
                "reason": str(e)
            }
    ...
    async def get_adding_file_planning(self, idea, tree, file_attachments):
        ...
        try:
            ...
            logger.debug("\n #### The `FileManagerAgent` encountered a JSON decode error and is attempting to repair it")
            call_error_api("JSON decode error in get_adding_file_planning", str(e))
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            logger.debug(f" #### The `FileManagerAgent` encountered an unexpected error during adding file planning: `{e}`")
            call_error_api("Unexpected error in get_adding_file_planning", str(e))
            return {
                "reason": str(e)
            }
    ...
    async def get_moving_file_planning(self, idea, tree, file_attachments):
        ...
        try:
            ...
            logger.debug("\n #### The `FileManagerAgent` encountered a JSON decode error and is attempting to repair it")
            call_error_api("JSON decode error in get_moving_file_planning", str(e))
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            logger.debug(f" #### The `FileManagerAgent` encountered an unexpected error during moving file planning: `{e}`")
            call_error_api("Unexpected error in get_moving_file_planning", str(e))
            return {
                "reason": str(e)
            }
    ...
    async def get_moving_file_plannings(self, idea, tree, file_attachments):
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` encountered an error while retrieving moving file plannings: `{e}`")
            call_error_api("Error in get_moving_file_plannings", str(e))
            raise
    ...
    async def get_adding_file_plannings(self, idea, tree, file_attachments):
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` encountered an error while retrieving adding file plannings: `{e}`")
            call_error_api("Error in get_adding_file_plannings", str(e))
            raise
    ...
    async def get_file_plannings(self, idea):
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` encountered an error while retrieving file plannings: `{e}`")
            call_error_api("Error in get_file_plannings", str(e))
            raise