...
class ErrorDetection:
    ...
    async def get_task_plan(self, error):
        ...
        try:
            ...
            error_message = "JSON decoding error in ErrorDetection"
            call_error_api(error_message, str(response.choices[0].message.content))
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            error_message = f"ErrorDetection failed to get task plan: {e}"
    ...
    async def get_task_plans(self, error):
        ...
        try:
            ...
            error_message = f"Error in get_task_plans: {str(e)}"
            call_error_api(error_message, str(e))
            logger.error(error_message)
            return {"reason": str(e)}
