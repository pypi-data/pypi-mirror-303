...
class ExplainableActionCheckAgent:
    ...
    async def get_action_check(self, conversation):
        ...
        try:
            ...
            call_error_api(f"Error in ExplainableActionCheckAgent.get_action_check: {str(e)}", str(e))
            return {
                "action": "",
                "status": "2"
            }
    ...
    async def perform_action_check(self, conversation):
        ...
        try:
            ...
            logger.error(f" #### The `ExplainableActionCheckAgent` encountered an error:\n During the action check process\n Error: {e}")
            call_error_api(f"Error in ExplainableActionCheckAgent.perform_action_check: {str(e)}", str(e))
            return {
                "action": "",
                "status": "2"
            }
