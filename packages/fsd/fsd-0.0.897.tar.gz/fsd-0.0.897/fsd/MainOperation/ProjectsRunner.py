...
class ProjectsRunner:
    ...
    async def run_xcode_project(self, basename, role, file_attachments, focused_files, max_retries=25):
        ...
        while retries < max_retries:
            ...
            try:
                ...
                if 'Select a development team in the Signing & Capabilities editor.' in bug_log_content:
                    ...
                    log_error("Signing issue detected", "Select a development team in the Signing & Capabilities editor.")
                    return
                ...
                if not damagefile:
                    ...
                    log_error("Self-healing issue", "Unable to locate damaged files")
                    return
                ...
                try:
                    ...
                    if http_error.response.status_code == 429:
                        ...
                        log_error("HTTP Error", str(http_error))
                        raise
        ...
        if not bugFixed:
            ...
            log_error("Build failure", f"Failed after {max_retries} attempts")
            ...
            if "error:" in line.lower():
                ...
                if match:
                    ...
                    try:
                        ...
                        log_error("File not found", f"File: {file_name}")
                else:
                    ...
                    log_error("Unparseable error", line.strip())
