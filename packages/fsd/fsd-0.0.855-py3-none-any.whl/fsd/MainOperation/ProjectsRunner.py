...
class ProjectsRunner:
    ...
    def log_errors(self, error_log):
        ...
        error_regex = re.compile(r'(/[^:]+\.swift):(\d+):(\d+): (error|note): (.+)')

        lines = error_log.split('\n')

        for line in lines:
            if "error:" in line.lower() or "note:" in line.lower():
        ...
        # Add build failure information
        build_failure_regex = re.compile(r'The following build commands failed:')
        if build_failure_regex.search(error_log):
            build_failure_lines = error_log.split('The following build commands failed:')[1].strip().split('\n')
            for line in build_failure_lines:
                if line.strip():
                    error_details.append({
                        'file': 'Build Failure',
                        'line': 'N/A',
        ...
        for error in error_details:
            output_string += f"File: {error['file']} - Line: {error['line']} - Column: {error['column']}\n"
            output_string += f"Type: {error['type']} - Message: {error['message']}\n"
            output_string += f"Damaged code: {error['code']}\n"
