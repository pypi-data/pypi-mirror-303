...
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, 
    ...
    args:
        slides (df or array-like): 
            array-like structure containing list of slides with their absolute paths, if df, these ids assumed to be
            stored under the 'slide_path' column
    ...
    if isinstance(slides, pd.DataFrame):
        slide_ids = slides.slide_path.values
    else:
        slide_ids = slides

    ## Could have used np.ones instead of np.full
    default_df_dict = {'slide_path': slide_ids, 'process': np.full((total), 1, dtype=np.uint8)}
