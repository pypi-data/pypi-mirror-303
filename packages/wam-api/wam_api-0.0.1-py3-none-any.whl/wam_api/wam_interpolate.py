...
class WAMInterpolator:
    ...
    def get_density(self, input_dt, lat, lon, alt):
        ...
        Parameters:
            ...
            prefer_wfs (bool): If True, use 'wfs' data type exclusively. @@@ EDITS
            wfs_start_dt (datetime): If provided and prefer_wfs is True, use this date as the start date of the forecast. @@@ EDITS
...
def main():
    # Define Required Date/Time and Lat/Long/Alt
    dt = datetime(2023, 5, 11, 18, 12, 22)
    lat, lon, alt = -33.4, -153.24, 550.68

    # Create an Instance of the WAMInterpolator
    wam_interpolator = WAMInterpolator()
    density = wam_interpolator.get_density(dt, lat, lon, alt)
    print(f"Interpolated density: {density}")
