...
from galileo_forecast.galileo_plots.plot_utils import get_min_max_adj, get_bins, get_labels_from_bins
...
class CategoricalBinner:
    """
    Add bins to a dataframe given as input and return labels based on bins

    Aimed at categorical features
    ...
    def get_labels(self):
        if not self._has_labels:
            raise ValueError("No labels have been set")
...
class BinEventRatePlot(PlotProtocol):
    ...
    def do_math(
        ...
        self.p_spread_col = p_spread_col

        # make fresh copy of df
        self.df = df.copy()

        # Calculate global event rate
        self.event_rate = np.mean(self.df[p_spread_col])
        ...
        self.df_binned = self.df.groupby("bins", observed=False).agg(
            {feature_col: [len], p_spread_col: ["mean"]}
        )
        ...
        return [
            ...
            # plot binned event rate baseline
            {
                "trace": go.Scatter(
                    ...
                    hoverinfo=self.hoverinfo,
                    name=f"Avg. spread of sampled probs: ({'{:.1%}'.format(self.event_rate)})",
                ),
                # share y
                "secondary_y": True,
            },
            # plot binned event rate
            {
                "trace": go.Scatter(
                    x=self.labels,
                    y=self.df_binned[f"{self.p_spread_col}_mean"],
                    mode="lines+markers",
                    line=dict(
    ...
    def get_secondary_y_axis_title(self):
        return "Standard deviation of sampled probabilities"
