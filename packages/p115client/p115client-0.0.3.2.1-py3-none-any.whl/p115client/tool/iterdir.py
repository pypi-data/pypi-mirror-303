            ...
            if find_ids := pids - id_to_dirnode.keys():
                try:
                    if len(find_ids) <= len(id_to_dirnode) // page_size:
                        ...
                        # TODO: 为了确保能及时完成，不要拉全量，可以在执行函数前，前获取当前的时间戳，以后到了 < 这个时间，就结束
                        # TODO: 如果发现 find_ids 非空，则先检查一遍，这几个 id 是否还存在，如果有不存在的，就报错，或者先不管了，把错误收集，返回给调用者
                        # TODO: 如果这些 find_ids 都存在，则流程再走一遍，反复走流程，直到 find_ids 为空
                        ...
                        # TODO: 如果有些 id 不能被拉取到，可能是拉取时，这些 id 被移动到头部了，可以尝试再拉一次，如果还拉不到，就报错，或者单独执行查询
                        # TODO: 也可能在拉取时，目录正好被删除了，可以通过 client.fs_files_skim 来判断一下是否存在，如果已经不存在了，那么所有涉及到此 id 的文件，就要重新拉取信息（先更新一下它们的mtime），直到最后确定文件的祖先能到 0，或者文件被删除
                        # TODO: 凡是发现目录被更新了，数据就要重新拉，可以专门拉那几个确定不了的，也可以报错，或者直接把这几个搞不了的返回，让调用方搞好了，再来
                        if find_ids:
