    ...
    **request_kwargs, 
    ...
    def normalize_path(attr: dict, /) -> str:
        ...
        if with_root and not use_abspath:
            relpath = joinpath(id_to_dirnode[cid][0], normpath(path) + ".strm")
        else:
    ...
    if async_:
        ...
        async def request():
            ...
            async with TaskGroup() as group:
                ...
                if discard:
                    create_task(to_thread(lambda: existing.update(iglob("**/*.strm", root_dir=save_dir, recursive=True))))
        ...
        try:
            if discard:
                executor.submit(lambda: existing.update(iglob("**/*.strm", root_dir=save_dir, recursive=True)))
