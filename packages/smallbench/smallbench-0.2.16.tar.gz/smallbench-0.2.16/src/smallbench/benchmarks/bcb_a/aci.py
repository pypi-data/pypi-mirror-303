...
{unit_test_context}""",
    ...
    def load_state(self, snapshot: str):
        ...
        self.unit_tests = {name: BCBUnitTest(**test) for name, test in state.get("unit_tests", {}).items()}
        self.sketch = state.get("sketch", "")
        self.current_solution = CurrentSolution(lines=state.get("current_solution", "").split("\n"))
