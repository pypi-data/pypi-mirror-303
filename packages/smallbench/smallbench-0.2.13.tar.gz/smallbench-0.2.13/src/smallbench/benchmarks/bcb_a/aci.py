...
{unit_test_context}""",
            ...
            # New 'cat' action added below
            ACIAction(
                action_name="cat",
                action_arg_spec={},
                action_description="Show the current code solution and all unit tests",
                action_context="Displays the current code solution and all unit tests using to_python().",
                transform=Transform(callable=self.cat, type="sync"),
            ),
    ...
    def get_submission_actions(self):
        if self.synchronous:
            ...
            submit = ACIAction(
                action_name="submit_solution",
                action_arg_spec={},
                action_description="Submit the solution for evaluation.",
                action_context="Submit the solution - you must choose this action for you code to be considered final and evaluated. Failing to submit before time is out guarantees a failure.",
                transform=Transform(callable=self._submit_solution_sync, type="sync"),
            )
            return [test, submit]
    ...
    def cat(self):
        """
        Returns the current code solution and all unit tests.
        """
        solution = "\n".join(self.current_solution.lines)
        unit_tests = "\n\n".join(
            [
                f"Unit Test '{name}':\n{test.to_python(index=i)}"
                for i, (name, test) in enumerate(self.unit_tests.items())
            ]
        )
