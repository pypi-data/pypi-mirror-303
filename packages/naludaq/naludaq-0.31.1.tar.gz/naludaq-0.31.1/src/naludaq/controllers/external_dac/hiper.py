...
class DACControllerHiper(BaseDACController):
    ...
    def _send_command_to_dac(
        ...
        self._write_control_register(f"dac_{side}_update", 0)

        # 1. set the instruction
        self._write_control_register(f"dac_{side}_cmd", command)
        self._write_control_register(f"dac_{side}_asic", dac_chip)
        self._write_control_register(f"dac_{side}_addr", dac_channel)
        self._write_control_register(f"dac_{side}_value", value)

        # 2. update the DAC
        self._write_control_register(f"dac_{side}_update", 1)
        self._write_control_register(f"dac_{side}_update", 0)
