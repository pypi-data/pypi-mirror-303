...
class CSVWriter:
    ...
    def export_thresholdscan_to_csv(self, filename:str, trigger_values, threshold_results:np.array, channels:list|None=None):
        ...
        try:
            ...
            raise TypeError(f"threshold_results should be a numpy array, got {type(threshold_results)}")
        ...
        if len(trigger_values) != threshold_results.shape[0]:
            raise ValueError("The length of trigger values and thresholdscan values should match ({} vs {})".format(len(trigger_values), threshold_results.shape[0]))
