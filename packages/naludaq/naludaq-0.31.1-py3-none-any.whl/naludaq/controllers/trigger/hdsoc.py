...
class TriggerControllerHdsoc(TriggerController):
    ...
    def tsel(self, values: dict[str, bool]):
        ...
        for side, value in values.items():
            if side in ["left", "0_15", TriggerGroups.LEFT]:
                regname = "tsel_left"
            elif side in ["right", "16_31", TriggerGroups.RIGHT]:
                regname = "tsel_right"
            else:
    ...
    def _set_references(self, values: dict[str, tuple[int, int]]):
        ...
        for side, (lower, upper) in values.items():
            if side in [0, "0_15", "left", TriggerGroups.CH0_15, TriggerGroups.LEFT]:
                side = "left"
            elif side in [
                1,
                "16_31",
                "right",
                TriggerGroups.CH16_31,
                TriggerGroups.RIGHT,
            ]:
                side = "right"
            else:
