...
class Widget(RawWidget):
    ...
    def draw(self, window: pygame.Surface):
        ...
        if self.clicked and self.is_clicked:
            color = self.clicked.color or color
            border = self.clicked.border or border
...
class Button(Widget):
    ...
    def __init__(self, x: int, y: int, width: int, height: int,
                 clicked: Clicked | None = None, color=(255, 0, 0), border: Border | None = None, cmd=lambda _: (),
