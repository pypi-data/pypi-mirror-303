...
from dacalarm2.utils.ycl_abandon_abnormal import get_time

use_dac = False
if use_dac:
    import common.redis
    import dacalarm2.utils.control_delta
    import dacalarm2.views.alarm_timing_utils
else:
    import dacalarm2.utils.fake_cache
...
class DacFacade:
    ...
    def get(self, key, default=None):
        if use_dac:
            ...
            return dacalarm2.utils.fake_cache.default_cache.get(key, default)

    def set(self, key, value, timeout):
        if use_dac:
            return common.redis.default_cache.set(key, value, timeout)
        else:
            return dacalarm2.utils.fake_cache.default_cache.set(key, value, timeout)

    def delete(self, key):
        if use_dac:
            return common.redis.default_cache.delete(key)
        else:
            return dacalarm2.utils.fake_cache.default_cache.delete(key)

    def clear(self):
        if not use_dac:
            dacalarm2.utils.fake_cache.default_cache.clear()
...
def query_actions(global_result, alarm_rule_id: int) -> dict:
    if use_dac:
        return dacalarm2.utils.control_delta.query_actions(global_result, alarm_rule_id)
    else:
...
def get_and_deal_rule_dict_data(period, **kwargs):
    ...
    if use_dac:
        return dacalarm2.views.alarm_timing_utils.get_and_deal_rule_dict_data(period, **kwargs)
        # points = research.utils.influx_utils.influxClient.query_by_time(kwargs['measurement'], kwargs['field'],
        #                                                                 kwargs['start'], kwargs['end'])
        # return points[kwargs['field']]
    else:
        from dacalarm2.utils.ycl_abandon_abnormal import get_time_str
