...
from metamart_schemas.base import Node as NodeTypes

from metamart_graph.graph import Graph
...
class GraphAnalyzer:
    ...
    def traverse_data_type_violations(
        ...
        for test_node in self.column_successors(node.spec.namespace, node.spec.name):
            ...
            edge_meta = edge_data.spec.metadata.metamart
            node_meta = test_node.spec.metadata.metamart
    ...
    def test_data_type_change(self, namespace: str, name: str, new_type: bool) -> List[Tuple[List[NodeTypes], bool]]:
        ...
        assert (
            current_node.spec.metadata.metamart.node_type == "Column"
        ), f"Data type violation tests can only be applied to columns {current_node.spec.metadata.metamart.node_type}"
    ...
    def test_unique_violations(
        ...
        assert (
            current_node.spec.metadata.metamart.node_type == "Column"
        ), f"Unique violation tests can only be applied to columns {current_node.spec.metadata.metamart.node_type}"
    ...
    def column_predecessors(self, namespace: str, name: str):
        ...
        col_predecessors = tuple(node for node in predecessors if node.spec.metadata.metamart.node_type == "Column")
    ...
    def column_successors(self, namespace: str, name: str):
        ...
        col_successors = tuple(node for node in successors if node.spec.metadata.metamart.node_type == "Column")
