...
class DateMetrics(BASE,Template):
    ...
    def setWeatherDefaults(self):
        ...
        if not Path("api_key").exists():
            ...
            print(f"{Fore.light_red}You need to make an account at {Fore.light_yellow}https://www.weatherapi.com/{Fore.light_red} and paste the API Key in {Fore.light_yellow}{str(Path("api_key").absolute())}{Style.reset}")
        else:
            print(f"{Fore.light_green}{Path("api_key").open("r").read()}{Fore.light_steel_blue} -> API KEY For {Fore.magenta}https://www.weatherapi.com/{Style.reset}")
