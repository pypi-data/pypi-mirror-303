...
from typing import Dict, List, Callable, Optional, Tuple, Awaitable, Any, TypeVar, overload
import inflection
...
def test(tested_node: Callable, name: Optional[str] = None, description: Optional[str] = None):
    ...
    def registered_function(func):
        ...
        def wrapped_function(*args,__template, **kwargs):
            if is_inside_assignation():
                raise NotImplementedError(
                    "You cannot run tests inside an assignation."
                )
        ...
        assert hasattr(tested_node, "__definition_hash__"), "The to be tested function or actor should be registered with the register decorator. Or have a __definition__ attribute."
