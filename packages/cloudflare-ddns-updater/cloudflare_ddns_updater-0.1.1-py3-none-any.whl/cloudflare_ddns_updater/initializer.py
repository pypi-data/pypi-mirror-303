...
def add_cron_job(interval):
    # Define the cron job
    cron_command = f"*/{interval} * * * * /usr/bin/python3 /usr/local/bin/ip_updater.py >> /var/log/ip_update.log 2>&1"
    # Read the existing crontab jobs
    try:
        current_crontab = subprocess.check_output("crontab -l", shell=True).decode()
    except subprocess.CalledProcessError:
        # If there's no crontab yet, this command will fail, so we start with an empty crontab
        current_crontab = ""
