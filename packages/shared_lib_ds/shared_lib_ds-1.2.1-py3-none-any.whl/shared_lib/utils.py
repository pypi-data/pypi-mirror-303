...
def home(request, template_name, email, context):
    #logger.info(context)
    if request.method == 'GET':
        ...
        context.update(make_post_context(request.POST, email))
        return render(request, template_name, context)
...
def portfolio_details(request, template_name, title, pk) -> HttpResponse:
    """
...
# =============================== Tesrms & Privacy =========================
def terms(request, template_name, company_name, sdate, title='이용약관'):
    context = {
        "breadcrumb": {
            "title": title,
        },
...
def privacy(request, template_name, company_name, sdate, owner, phone, email, title='개인정보 보호정책',
