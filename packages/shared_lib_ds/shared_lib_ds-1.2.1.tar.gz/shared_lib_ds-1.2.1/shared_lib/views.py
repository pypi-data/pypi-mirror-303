...
def test_home(request):
    context = {}
    return home(request,'shared_lib/test/_home.html', 'hj3415@gmail.com', context)


# =========================== Test ================================
def test_portfolio_details(request, pk):
    return portfolio_details(request, 'shared_lib/_test_portfolio_details.html', 'Portfolio Details', pk)

def test_terms(request):
    return terms(request, 'shared_lib/test/_test_pricacy.html', "가락삼성치과", '2024-10-10')

def test_privacy(request):
    return privacy(request, 'shared_lib/test/_test_pricacy.html', "가락삼성치과",
