...
class GPolyEncoder(object):
    def __init__(
        self, num_levels=18, zoom_factor=2, threshold=0.00001, force_endpoints=True
    ):
        ...
        if len(points) > 2:
            ...
            while len(stack):
                ...
                for i in range(current[0] + 1, current[1]):
                    temp = self._distance(
                        points[i], points[current[0]], points[current[1]]
                    )
                    if temp > max_dist:
                        max_dist = temp
                        max_loc = i
    ...
    def _distance(self, p0, p1, p2):
        ...
        if p1[1] == p2[1] and p1[0] == p2[0]:
            ...
            u = (
                (p0[1] - p1[1]) * (p2[1] - p1[1]) + (p0[0] - p1[0]) * (p2[0] - p1[0])
