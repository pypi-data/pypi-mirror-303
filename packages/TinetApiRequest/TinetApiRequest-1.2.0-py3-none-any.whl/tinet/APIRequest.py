...
class APIRequest:
    ...
    def doRequest(self, file, bean):
        ...
        for testcase in yamlTestcase:
            with allure.step(testcase.get('name')):
                ...
                # 1. 先看看参数中是否有需要替换的参数
                repParameter = self.replaceParameterAttr(dataSaveBean, testcase.get('parameter'), testcase.get('requestType'))
                ...
                # 3. 判断请求 requestType
                dataRequestParameter, jsonRequestParameter, paramsData, ModelData, requestType = self.requestType(testcase.get('requestType'), requestParameter)
                # 4. 开始请求
    ...
    def replaceParameterAttr(self, bean, parameter, requestType='json'):
        ...
        if parameter is None:
            return None
        # 开始调用方法替换
        if requestType is not None and requestType.lower() == 'json-text':
