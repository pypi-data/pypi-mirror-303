...
def test_filter_without_face(tfactory: TempDirFactory) -> None:
    ...
    gm = fr.filter_images_without_face(output_dir=t_path2,
                                       ...
                                       prob_threshold=0.8,
                                       return_info=False)
    # Account for model running on CPU variability
    assert len(gm) == 3 or len(gm) == 2
...
def test_filter_multiple_face(tfactory: TempDirFactory) -> None:
    ...
    # Repeat tests for return_info=True
    (gm, info_dict) = fr.filter_images_with_multiple_faces(output_dir=t_path2,
                                                           min_face_size=20,
                                                           prob_threshold=0.9,
                                                           return_info=True)
