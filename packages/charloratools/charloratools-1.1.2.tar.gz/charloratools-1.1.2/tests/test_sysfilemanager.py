...
def test_duplicates(hashtype: str, tfactory):
    ...
    t_path2 = tfactory.new_temp('test_duplicates2')
    ri2 = tfactory.create_random_image('test_duplicates2', 'random2')
    im2 = ImgManager(path=ri2,
                     hashtype=hashtype)
    copy_path1 = t_path / 'random2_copy.jpg'
    copy_path2 = t_path / 'random2_copy2.jpg'
    copyfile(ri2, copy_path1)
    copyfile(ri2, copy_path2)
    gm2 = GalleryManager(path=t_path2,
                         hashtype=hashtype)
    gm2.delete_duplicates()
    assert len(gm2) == 1
    assert im2 in gm2
