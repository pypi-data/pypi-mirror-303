...
def torch_import() -> tuple[ModuleType]:
    """
    Attempts to import the PyTorch library and its torchvision transforms.
...
def dirisvalid(path: str | Path,
               ...
               ) -> Literal[True] | Path | tuple[Path, dict]:
    """
    ...
    Path or tuple or True
        ...
        If check_images and create_if_not_found are False returns True
        if the directory is valid, raises an error otherwise.
    ...
    if not path.exists():
        if create_if_not_found:
            ...
            path = path.resolve()
            logger.info(f'Created empty directory {path.resolve()}')
            return path
        else:
...
def save_with_detection_box(img_path: str | Path, outdir: str | Path,
...
def InfoDict2Pandas(info: dict | list) -> dict:
    """
    ...
    if isinstance(info, dict):
        ...
        es = "info_dict_lst must be either a dict or lst"
        raise errors.InfoDictFormatError(es)
    # ----
...
def is_matched(df_el: pd.Series) -> pd.Series | None:
    """
...
def not_is_matched(df_el: pd.Series) -> pd.Series | None:
    """
...
def split_matched(info_dict: dict | list) -> dict:
    """
    Separates an info dictionary into matched and not matched items.
...
def initialize_driver(headless: bool = True,
                      incognito: bool = True,
                      add_arguments: list | None = None) -> webdriver.Chrome:
    """
    Initializes a Selenium Chrome Web Driver with specified options.
...
def page_scroll(driver: webdriver.Chrome,
                n: int,
                webpage_wait_time: int | float) -> None:
    """
    ...
    webpage_wait_time : int or float
...
def download_from_src(srcs: list[str],
                      prefix: str,
                      save_path: str | Path,
...
def img_path_to_tensor(img_path: str | Path,
                       nsize: tuple[int, int] | None = None):
    """
...
def dir_path_to_img_batch(path: str | Path):
    """
    Converts all images in a directory to a batch of tensors.
