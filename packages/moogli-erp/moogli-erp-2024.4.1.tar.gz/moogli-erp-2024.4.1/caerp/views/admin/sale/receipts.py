...
PaymentModeAdminView = get_model_admin_view(
    PaymentMode, r_path=RECEIPT_URL, can_disable=False
)

BankAdminView = get_model_admin_view(Bank, r_path=RECEIPT_URL)

BankAccountAdminView = get_model_admin_view(BankAccount, r_path=RECEIPT_URL)
...
def add_views(config):
    config.add_admin_view(ReceiptIndexView, parent=SaleIndexView)
    for view in (
        MainReceiptConfig,
