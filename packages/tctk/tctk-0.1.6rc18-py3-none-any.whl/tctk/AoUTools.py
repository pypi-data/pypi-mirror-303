...
class dsub:
    ...
    def __init__(
        ...
        if output_folder is not None:
            ...
            self.output_folder = (
                f"{self.bucket}/dsub/results/{self.job_name}/{self.user_name}/{self.date}/{{job-id}}"
            )
        ...
        if log_file_path is not None:
            ...
            self.log_file_path = (
                f"{self.bucket}/dsub/logs/{self.job_name}/{self.user_name}/{self.date}/{{job-id}}/{self.job_name}.log"
            )
    ...
    def _dsub_script(self):
        ...
        if self.multiple_output_files:
            ...
            if self.output_file_name != "":
                output_flag += f"--env OUTPUT_FILE_NAME={self.output_file_name}"
                output_flag += f"--output OUTPUT_FILE={self.output_folder}/{self.output_file_name}"
            else:
                print("Single output file requires output_file_name.")
                sys.exit(1)

        main_script = f"--script {self.main_script_name}"

        script = base_script + input_flags + output_flag + main_script
        if self.preemptible:
            script += " --preemptible"
