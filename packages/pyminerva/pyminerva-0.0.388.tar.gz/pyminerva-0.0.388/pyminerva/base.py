...
def get_stock_history_by_fmp(ticker:str, periods:list):  # period: 1min, 5min, 15min, 30min, 1hour, 4hour, 1day
    ...
    for period in periods:
        ...
        try:
            ...
            # 예시:
            # {'date': '2019-12-27', 'open': 25.39, 'high': 25.44, 'low': 25.21, 'close': 25.21, 'adjClose': 18.97, 'volume': 3866451, 
            #  'unadjustedVolume': 3866451, 'change': -0.18, 'changePercent': -0.70894, 'vwap': 25.3125, 'label': 'December 27, 19', 
            #  'changeOverTime': -0.0070894}, 
            df = pd.DataFrame(buf, columns=['date','open','high','low','close','adjClose','volume','unadjustedVolume','change','changePercent','vwap','label','changeOverTime'])
            print(f">>>>>>>>>: {df}")
            df['ticker'] = ticker
