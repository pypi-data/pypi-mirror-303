...
def run_as_admin():
    if not is_admin():
        ...
        try:
            # Start a new process with administrative privileges
            result = subprocess.run(["powershell.exe", "-Command", 
                                     f"Start-Process 'python' -ArgumentList '\"{script}\" {params}' -Verb RunAs"],
                                     check=True)
            return result.returncode == 0  # Return True if the process succeeds
...
def install_rust():
    cmd = subprocess.run(['winget', 'install', '--id', 'Rustlang.Rustup', '-e', '--source', 'winget'])
    if cmd.returncode == 0:
        print("Installation of Rust completed successfully.")
        return True
    else:
        print("Error occurred while installing Rust.")
        return False
    
def install_vscr():
    
    vcredists = [
        'Microsoft.VC++2015-2022Redistributable-x86',
        'Microsoft.VC++2015-2022Redistributable-x64',
        'Microsoft.VC++2013Redistributable-x86',
        'Microsoft.VC++2013Redistributable-x64',
        'Microsoft.VC++2012Redistributable-x86',
        'Microsoft.VC++2012Redistributable-x64',
        'Microsoft.VC++2010Redistributable-x86',
        'Microsoft.VC++2010Redistributable-x64',
        'Microsoft.VC++2008Redistributable-x86',
        'Microsoft.VC++2008Redistributable-x64'
    ]

    for vcredist in vcredists:
        command = f'winget install {vcredist} -e'
        os.system(command)
        return True
    ...
    'Big Data Modules',
        ...
        try:
            ...
            if 1 <= selected_module_type <= len(module_types):
                ...
                clear()
                if install_option in yes:

                    try:
                        versions = os.listdir(f"C:\\Users\\{user}\\AppData\\Local\\Programs\\Python")
                    except:
                        versions = os.listdir("cd C:\\Program Files\\python\\")
                        
                    if len(versions) > 0:
                        print(f"\nYou have {len(versions)} Python installation folders:")
                        
                        for i, version in enumerate(versions):
                            print(f"({i + 1}) {version}")
                        
                        try:
                            python_folder_index = int(input(f"Select your Python folder (1 to {len(versions)}): ")) - 1
                            python_folder = versions[python_folder_index]
                            print(f"You selected: {python_folder}")
...
def run():
    try:
        ...
        print(f"An error occurred: {e}. Please run this as Administrator Command Prompt / PowerShell")
