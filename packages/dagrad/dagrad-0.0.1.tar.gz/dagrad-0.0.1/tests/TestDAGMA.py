...
class TestLinear(unittest.TestCase):
    ...
    def testDagmaGauss1(self):
        ...
        W_est = learning_dag(X = X, model = 'linear', method = 'dagma', method_options = method_options)
        time_end = time.time()
        print('Working with linear Gaussian data')
        print('Time taken for dagma numpy: ', time_end - time_start)
        acc = utils.count_accuracy(B_true, W_est != 0)
        print('Accuracy for dagma: ', acc)
    ...
    def testDagmaGauss2(self):
        ...
        W_est = learning_dag(X = X, model = 'linear', method = 'dagma',
                               compute_lib = 'torch', device = 'cpu', method_options = method_options)
        time_end = time.time()
        print('Working with linear Gaussian data')
    ...
    def testDagmaLogistic1(self):
        ...
        X, W_true, B_true = self.generate_linear_data(n,d,s0,graph_type,sem_type)
        time_start = time.time()
        W_est = learning_dag(X = X, model = 'linear', method = 'dagma', loss_fn = 'logistic')
        time_end = time.time()
        print('Working with linear logistic data')
        print('Time taken for dagma numpy: ', time_end - time_start)
        acc = utils.count_accuracy(B_true, W_est != 0)
        print('Accuracy for dagma numpy: ', acc)
    ...
    def testDagmaMLP1(self):
        ...
        time_start = time.time()
        W_est = learning_dag(X = X, model = 'nonlinear', method = 'dagma', reg = 'l1', method_options={'verbose': True})
        time_end = time.time()
