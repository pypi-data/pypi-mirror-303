...
def benchmark_pipeline(
    ...
    asr = (
        ASR()
        .add_pipe(
            "detector", num_threads=num_threads, compile=True, model_dir=vad_model_dir
        )
        .add_pipe("recognizer", model_dir=model_dir, batch_size_s=asr_batch_s)
    ...
    def run_fasr(urls):
        start = time.perf_counter()
        _ = asr(urls, batch_max_duration=vad_batch_s, batch_size=100)
        end = time.perf_counter()
        took = round(end - start, 2)
        return took
