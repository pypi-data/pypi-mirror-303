...
"""
    ...
    , compress=False, quiet=False, use_single_float=True, dirSize=10240):
        ...
        if compress:
            ...
            self.compressAlgorithm = 'lz4framed'
            abstract['compression'] = self.compressAlgorithm
        else:
            self.compress = None
    ...
    def _serialize_sections(self):
        ...
        try:
          while statistics[NSections] < maxSections\
            ...
            # compress and create object for packing
            if self.compress is not None:
                npPacked = self.compress(npPacked)
            toPack = {'tstart':self.section['tstart']
            ,'tstart':self.section['tstart']
            ,'compress':self.compressAlgorithm
            ,'pars':npPacked}
