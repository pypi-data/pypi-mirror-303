...
#
__version__ = '2.0.6 2021-09-20'
#TODO: simultaneous data taking and writing
        ...
        with self.lock:
          for devPar,props in args[0].items():
            if apstrim.Verbosity >= 1:
                _printd( _croppedText(f'delivered devPar: {devPar,props}'))
            try:
        ...
        try:
          while statistics[NSections] <= maxSections\
            ...
            try:
                ...
                noData = True
            if noData:
                _printd(f'No data recorded in section {statistics[NSections]}')
                continue
