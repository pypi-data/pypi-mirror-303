from typing import Dict, List, Literal, Optional, TypeVar, Union
from uuid import UUID

from metamart_schemas.v1 import EdgeV1, NodeV1
from metamart_schemas.v1.edge import EdgeNamedID, EdgeUuidID
from metamart_schemas.v1.metadata.metadata import (
    MetamartMalformedEdgeMetadataV1,
    MetamartMalformedNodeMetadataV1,
)
from metamart_schemas.v1.node import NodeNamedID, NodeUuidID

from metamart_client.endpoints.client import ClientOptions
from metamart_client.endpoints.rest import get
from metamart_client.endpoints.utilities import (
    handles_bad_metadata,
    is_valid_uuid,
    paginated,
)
from metamart_client.endpoints.v1.client import ClientV1
from metamart_client.schemas.labels import EdgeLabels, NodeLabels, WorkspaceLabels
from metamart_client.schemas.workspace import Workspace

T = TypeVar("T", NodeV1, EdgeV1)
X = TypeVar("X")


@handles_bad_metadata(MetamartMalformedNodeMetadataV1)
def node_builder(resp: Dict[str, X]) -> NodeV1:
    return NodeV1.from_spec(resp)


@handles_bad_metadata(MetamartMalformedEdgeMetadataV1)
def edge_builder(resp: Dict[str, X]) -> EdgeV1:
    return EdgeV1.from_spec(resp)


@get.register
def get_node_by_label_v1(
    client: ClientV1, metamart_type: NodeLabels, options: ClientOptions = ClientOptions()
) -> List[NodeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (NodeLabels):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    url = client.get_url(metamart_type)
    resp = paginated(get)(client, url, options)
    return [node_builder(obj) for obj in resp]


@get.register
def get_node_v1(client: ClientV1, metamart_type: NodeV1, options: ClientOptions = ClientOptions()) -> Optional[NodeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (NodeV1):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    return get(client, metamart_type.spec, options)


@get.register
def get_nodes_by_uuid_str_id(
    client: ClientV1,
    metamart_type: NodeLabels,
    node_uuid: Union[str, UUID],
    options: ClientOptions = ClientOptions(),
) -> NodeV1:
    """

    Args:
        client (ClientV1):
        metamart_type (NodeLabels):
        node_uuid (Union[str, UUID]):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    if not is_valid_uuid(node_uuid):
        raise ValueError(f"The provided node id {node_uuid} is not a valid uuid.")

    url = f"{client.get_url(metamart_type)}{node_uuid}/"

    resp = get(client, url, options=options)
    resp = resp.json()
    return node_builder(resp)


@get.register
def get_from_node_uuid_id(
    client: ClientV1, metamart_type: NodeUuidID, options: ClientOptions = ClientOptions()
) -> Optional[NodeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (NodeUuidID):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    return get(client, "Node", metamart_type.id, options=options)


@get.register
def get_from_node_named_id(
    client: ClientV1, metamart_type: NodeNamedID, options: ClientOptions = ClientOptions()
) -> Optional[NodeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (NodeNamedID):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    options = options.copy()
    options.query_args = {
        **options.query_args,
        "name": metamart_type.name,
        "namespace": metamart_type.namespace,
    }

    result = get(client, "Node", options=options)

    num_results = len(result)
    if num_results == 0:
        return None
    elif num_results == 1:
        return result[0]
    else:
        message = (
            f"A node query for name={metamart_type.name}, namespace={metamart_type.namespace} in the "
            f"workspace={options.query_args.get('workspace', '<unknown workspace>')} returned more than one result. "
            f"This is a defensive error which should not be triggered. If you encounter it "
            "please open an issue at www.github.com/meta-mart/metamart-core/issues"
        )
        raise Exception(message)


# ----- Edges ----- #


def finalize_edge(client: ClientV1, resp: Dict, options: ClientOptions = ClientOptions()) -> EdgeV1:
    """

    Args:
        client (ClientV1):
        resp (Dict):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    nodes = [
        get(client, "node", resp["source"]),
        get(client, "node", resp["destination"]),
    ]

    resp["source"] = nodes[0].spec
    resp["destination"] = nodes[1].spec
    return edge_builder(resp)


@get.register
def get_edge_by_label_v1(
    client: ClientV1, metamart_type: EdgeLabels, options: ClientOptions = ClientOptions()
) -> List[EdgeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (EdgeLabels):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    url = client.get_url(metamart_type)
    resp = paginated(get)(client, url, options)
    return [finalize_edge(client, edge) for edge in resp]


@get.register
def get_edge_by_uuid_str_id(
    client: ClientV1,
    metamart_type: EdgeLabels,
    edge_uuid: Union[str, UUID],
    options: ClientOptions = ClientOptions(),
) -> EdgeV1:
    """

    Args:
        client (ClientV1):
        metamart_type (EdgeLabels):
        edge_uuid (Union[str, UUID]):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    if not is_valid_uuid(edge_uuid):
        raise ValueError(f"The provided node id {edge_uuid} is not a valid uuid.")

    url = f"{client.get_url(metamart_type)}{edge_uuid}/"

    resp = get(client, url, options=options)
    return finalize_edge(client, resp.json(), options)


@get.register
def get_edge_v1(client: ClientV1, metamart_type: EdgeV1, options: ClientOptions = ClientOptions()) -> Optional[EdgeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (EdgeV1):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    return get(client, metamart_type.spec, options)


@get.register
def get_from_edge_uuid_id(
    client: ClientV1, metamart_type: EdgeUuidID, options: ClientOptions = ClientOptions()
) -> Optional[EdgeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (EdgeUuidID):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    return get(client, "Edge", metamart_type.id, options=options)


@get.register
def get_from_edge_named_id(
    client: ClientV1, metamart_type: EdgeNamedID, options: ClientOptions = ClientOptions()
) -> Optional[EdgeV1]:
    """

    Args:
        client (ClientV1):
        metamart_type (EdgeNamedID):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """
    options = options.copy()
    options.query_args = {
        **options.query_args,
        "name": metamart_type.name,
        "namespace": metamart_type.namespace,
    }

    resp = get(client, "Edge", options=options)

    num_results = len(resp)
    if num_results == 0:
        return None
    elif num_results == 1:
        return resp[0]
    else:
        message = (
            f"An edge query for name={metamart_type.name}, namespace={metamart_type.namespace} in the "
            f"workspace={options.query_args.get('workspace', '<unknown workspace>')} returned more than one result. "
            f"This is a defensive error which should not be triggered. If you encounter it "
            "please open an issue at www.github.com/meta-mart/metamart-core/issues"
        )
        raise Exception(message)


# ----- Workspaces ------ #


@get.register
def get_all_workspaces(
    client: ClientV1,
    metamart_type: WorkspaceLabels,
    options: ClientOptions = ClientOptions(),
) -> Optional[List[Workspace]]:
    """

    Args:
        client (ClientV1):
        metamart_type (WorkspaceLabels):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """

    resp = paginated(get)(client, client.get_url(metamart_type), options)

    if len(resp) == 0:
        return None
    else:
        return [Workspace(**item) for item in resp]


@get.register
def get_workspace_by_name_v1(
    client: ClientV1,
    metamart_type: WorkspaceLabels,
    name: str,
    options: ClientOptions = ClientOptions(),
) -> Optional[Workspace]:
    """

    Args:
        client (ClientV1):
        metamart_type (WorkspaceLabels):
        name (str):
        options (ClientOptions, optional):  (Default value = ClientOptions())

    Returns:

    Raises:

    """

    if is_valid_uuid(name):
        url = f"{client.get_url(metamart_type)}{name}/"
        return get(client, url, options=options).json()
    elif len(name.split("/")) == 2:
        # this is a ref string i.e. org-name/workspace-name
        url = f"{client.get_url(metamart_type)}?ref={name}"
    else:
        url = f"{client.get_url(metamart_type)}?name={name}"

    resp = paginated(get)(client, url, options)

    num_resp = len(resp)
    if num_resp == 0:
        return None
    elif num_resp == 1:
        return Workspace(**resp[0])
    else:
        raise Exception(
            f"We were unable to identify a unique workspace matching `{name}` because more than one result was "
            f"returned. This may be the result of belonging to multiple organizations with identical workspace "
            f"names. You can narrow your query by instead providing a workspace ref composed of  "
            "{org-name}/{workspace-name} or the UUID of the desired workspace.",
        )
