...
async def start_proxy(moonraker_url, printer_key, login, password, moonraker_login, moonraker_password):
    ...
    await asyncio.gather(
        moonraker_to_reverb(f"{moonraker_url}?token={moonraker_api_key}", channel_name, bearer),
        reverb_to_moonraker(f"{moonraker_url}?token={moonraker_api_key}", channel_name, bearer)
    )
...
def get_moonraker_token(moonraker_url, username, password):
    response = requests.post(f"{moonraker_url}/access/login", json={
        'username': username,
        'password': password,
        "source": "moonraker"
    })
    ...
    response = requests.get(f"{moonraker_url}/access/api_key", headers={
        "Authorization": f'Bearer {bearer}'
    })
    data = response.json()
