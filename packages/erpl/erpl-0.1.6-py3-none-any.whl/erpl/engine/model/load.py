...
from .view import View,ViewSketch,Rect,Triangle,Circle,Polygon,Line,Ellipse,Square,Arc
from .hitbox import HitboxRect, HitboxTriangle, HitboxCircle, HitboxPolygon, HitboxLine, HitboxEllipse, HitboxSquare, HitboxArc
...
def load_transitions(data_transitions):
    ...
    for data_transition in data_transitions:
        ...
        music = data_transition['music']['src'] if 'music' in data_transition else None
...
def load_view_sketch(data_view, hitboxes):
    ...
    for data in data_draws:
        ...
        if(draw_type == 'FILL'):
            ...
            arcstop = data['arcstop']
            arc = Arc(x,y,w,h,arcstart,arcstop,color)
            sketch.add_draw(arc)
        elif(draw_type == 'LINE'):
            x1,y1 = (data['point1']["x"],data['point1']["y"]+HEIGHT_INV)
            x2,y2 = (data['point2']["x"],data['point2']["y"]+HEIGHT_INV)
            line = Line(x1,y1,x2,y2,color)
            sketch.add_draw(line)
...
def load_texts(data_texts):
    ...
    for data in data_texts:
        ...
        if 'format_text' in data:
            ...
            format_text = False
        texts.append(Text(id,text,x,y,width,color,format_text))
    return texts
...
def load_advanced_hitboxes(data_hitboxes):
    ...
    for data in data_hitboxes:
        ...
        if type == 'RECT':
            ...
            arcstart = data['arcstart']
            arcstop = data['arcstop']
            arc = HitboxArc(id,x,y,w,h,arcstart,arcstop)
            hitboxes.append(arc)
        elif type == 'LINE':
            x1,y1 = (data['point1']["x"],data['point1']["y"]+HEIGHT_INV)
            x2,y2 = (data['point2']["x"],data['point2']["y"]+HEIGHT_INV)
            line = HitboxLine(id,x1,y1,x2,y2)
            hitboxes.append(line)
    print("advanced hitboxes",hitboxes)
    return hitboxes 

def load_hitboxes(data_view):
    type = data_view['hitbox_type']
    print(type,data_view['type'])
    if type == 'NO':
        return []
    elif type == 'ADVANCED':
