...
from .hitbox import HitboxRect, HitboxTriangle, HitboxCircle, HitboxPolygon, HitboxLine, HitboxEllipse, HitboxSquare, HitboxArc, Hitbox
...
class View:
    ...
    def make_hitboxes_bbox(self):
        ...
        for hitbox in self.hitboxes:
            if isinstance(hitbox, HitboxRect) or isinstance(hitbox,HitboxSquare) or isinstance(hitbox,HitboxArc) or isinstance(hitbox, HitboxEllipse):
                ...
                hitbox_xmax = max(xs)
                hitbox_ymax = min(ys)
            elif isinstance(hitbox, HitboxLine):
                hitbox_xmin = min([hitbox.start[0],hitbox.end[0]])
                hitbox_ymin = min([hitbox.start[1],hitbox.end[1]])
                hitbox_xmax = max([hitbox.start[0],hitbox.end[0]])
                hitbox_ymax = min([hitbox.start[1],hitbox.end[1]])
...
class ViewSketch:
    ...
    def make_bbox(self):
        ...
        for draw in self.draws:
            if isinstance(draw, Rect) or isinstance(draw,Square) or isinstance(draw,Arc) or isinstance(draw, Ellipse):
                ...
                draw_xmax = max(xs)
                draw_ymax = min(ys)
            elif isinstance(draw, Line):
                draw_xmin = min([draw.start[0],draw.end[0]])
                draw_ymin = min([draw.start[1],draw.end[1]])
                draw_xmax = max([draw.start[0],draw.end[0]])
                draw_ymax = min([draw.start[1],draw.end[1]])
...
class Line(Draw):
    ...
    def draw(self,screen):
        pygame.draw.line(surface=screen,color=self.color,start_pos=self.start,end_pos=self.end)
...
class Arc(Draw):
    def __init__(self,x,y,w,h,start,stop,color):
        self.rect = pygame.Rect(x,y,w,h)
        #vem em degrees 
        self.start = start
        self.stop = stop
    ...
    def draw(self,screen):
        pygame.draw.arc(surface=screen,color=self.color,rect=self.rect,start_angle=self.stop,stop_angle=self.start)
