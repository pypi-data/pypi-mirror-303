...
class HitboxSquare(Hitbox):
    ...
    def collide(self,px,py):
        return px >= self.x and px <= self.x + self.r and py >= self.y and py <= self.y + self.r
...
class HitboxTriangle(Hitbox):
    ...
    def collide_point_triangle(self,px, py):
...
class HitboxArc(Hitbox):
    def __init__(self,id,x,y,w,h,start,stop):
        self.id = id
        self.x = x
        self.y = y
        self.w = w
