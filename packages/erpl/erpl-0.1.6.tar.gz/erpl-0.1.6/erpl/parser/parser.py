...
class Interpreter(Interpreter):
            ...
            if 'size' not in object: #se not temos de ver o size view a view
                for view in object['views']:
                    ...
                    if 'size' in view:
                        ...
                        img = view['sources'][0]
                        if(img[0] == 'PATH'):
                            #TODO: tem de verificar se é path do pc ou url
                            ...
                            #TODO: melhor erro
                            print("ERROR: can't now the size..")
            else:
                for view in object['views']:
                    if 'size' not in view:
    ...
    def var(self,var):
        ...
        if type in ["View", "Sound", "Object", "Scenario", "Transition", 'Challenge', "Event","Draw","Hitbox","Variable","Object_Text"]:
            value["id"] = var_name
    ...
    def value(self,val):
        '''value : ID|python_call|list_text_constructor|text_constructor|size_constructor|position_constructor|number_constructor|view_constructor|object_constructor|object_imported|sound_constructor|scenario_constructor|event_constructor|challenge_constructor|transition_constructor'''
    ...
    def size_constructor(self,size_constructor):
        ...
        return('Size',{"x" : w,"y" : h}) #(type,value)
    ...
    def obj_text_constructor_id(self,obj_text_constructor_id):
        '''obj_text_constructor_id : "Text." ID "(" param_text ("," param_color)? "," param_position ("," param_width)? ")"'''
    ...
    def param_images(self,param_images):
        ...
        for value in values:
            try:
                file = open(value)
                srcs.append(["PATH",os.path.realpath(file.name)])
                file.close()
            except:
                try:
                    headers = {
