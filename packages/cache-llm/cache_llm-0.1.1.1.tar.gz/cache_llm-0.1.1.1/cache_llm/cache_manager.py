...
def llm_cached_call():
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            cache_key =await acreate_hash_key("chat", messages, parameters)
            ...
            self.cache.set(cache_key, result.output)
            return result
...
def embedding_cached_call():
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            total_tokens = 0
            # 并行创建key并获取缓存
            cache_key_tasks = [acreate_hash_key("embedding", content, parameters) for content in input_content]
