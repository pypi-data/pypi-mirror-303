...
def help_branch():
    ...
    """
    echo(f"Ayuda sobre las diferentes ramas y su uso")
    echo(
        f"trunk: Rama madre de 'develop' y 'hotfix', es la principal y no se debe guardar cambios directamente en ella"
    )
    echo(
        f"  hotfix: Rama que se usa para corregir bug, se cierra contra 'trunk' al finalizar el arreglo"
    )
    echo(
        f"  develop: Rama madre de 'feature' y 'release', es la que cordina el desarrollo y lo mantiene escalonado y no debe guardar cambios directamente en ella"
    )
    echo(
        f"    feature: Rama para agregar nuevas caracteristicas, se cierra contra 'develop', y 'trunk' si fuera nesesario, al finalizarla"
    )
    echo(
        f"    release: Rama para preparar un lanzamiento, se cierra contra 'develop' y 'trunk' al finalizarla"
    )
...
def branch():
    ...
    if current_branch == "trunk":
        if not confirm(
            f"En esta rama solo se pueden adicionar ramas 'hotfix', estas seguro de continuar?"
        ):
            help_branch()
            return
        ...
        if not confirm(
            f"En esta rama solo se pueden adicionar ramas 'feature' y 'release', estas seguro de continuar?"
        ):
            help_branch()
            return
