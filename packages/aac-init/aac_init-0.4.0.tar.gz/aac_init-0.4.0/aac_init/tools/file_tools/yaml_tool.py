...
class YamlTool:
    ...
    def __init__(self, data_path: str = None, output_path: str = None):
        ...
        self.logger.debug(
            f"YAML tool initializing with data_path: '{data_path}', "
    ...
    def render_j2_template(
        self, template_rel_path: str, env: Environment, **kwargs: Any
    ):
        ...
        yaml_data = self.load_yaml_files()
        ...
        rendered_template_path = template_rel_path.replace(".j2", "")
        with open(rendered_template_path, "w") as file:
            file.write(result)

        if os.path.exists(rendered_template_path):
            self.logger.debug(
                f"Render Jinja2 template: '{rendered_template_path}' to '{rendered_template_path}' successfully."
            )
            os.remove(template_rel_path)
            self.logger.debug(
                f"Delete rendered Jinja2 template: '{rendered_template_path}' successfully."
            )
            return True
        else:
            self.logger.error(
                f"Failed to render Jinja2 template: '{rendered_template_path}'!"
            )

        return False

    def render_j2_templates(self, templates_path: str):
        """Render Jinja2 templates"""

        self.logger.debug(f"Start to render templates: '{templates_path}'...")

        # Copy template to output dir
        templates_dst = os.path.join(self.output_path, os.path.basename(templates_path))
        ...
        for dir, _, files in os.walk(templates_dst):
            for file in files:
                ...
                if not self.render_j2_template(template_rel_path, env):
                    return False
