...
class PricePattern(bt.Indicator):
    ...
    def determine_pattern(self, bar_open_price, bar_high_price, bar_low_price, bar_close_price, ema_50, price_rate_list, high_price_in_20, low_price_in_20):
        ...
        elif self.lines.pattern[-1] == PricePattern.UPTREND and not ((ema_50 > bar_low_price) & (ema_50 < bar_high_price)) and not self.determine_trend(price_rate_list, self.count_up_trend(price_rate_list)) and not self.determine_trend(price_rate_list, self.count_down_trend(price_rate_list)):
            ret = PricePattern.END_OF_UPTREND
        elif self.lines.pattern[-1] == PricePattern.DOWNTREND and not ((ema_50 > bar_low_price) & (ema_50 < bar_high_price)) and not self.determine_trend(price_rate_list, self.count_up_trend(price_rate_list)) and not self.determine_trend(price_rate_list, self.count_down_trend(price_rate_list)):
