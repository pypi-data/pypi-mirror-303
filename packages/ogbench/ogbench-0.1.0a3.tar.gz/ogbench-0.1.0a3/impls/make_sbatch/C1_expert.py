...
def main():
    ...
    if preset == 'P':
        ...
        conda_command = 'conda activate ogcrl'
    else:
        num_job_group = 1
        sh_command = './run.sh'
        pre_sbatch_command = 'PYTHONPATH=. MUJOCO_GL=egl WANDB__SERVICE_WAIT=86400 XLA_PYTHON_CLIENT_PREALLOCATE=false OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 NUMEXPR_NUM_THREADS=1 WANDB_API_KEY=ce68c4082bf6e41fc43238c5ada9a84c7bcd0bfb'
        if preset == 'T':
            num_groups = 2
            num_cpus = 2
            ...
            num_groups = 4
            sbatch_command = f'-A co_rail -p savio3_gpu --gres=gpu:TITAN:1 -N 1 -n {num_groups} -c 1 --qos=savio_lowprio -t 5-00:00:00'
        elif preset == 'G':
            num_groups = 2
    ...
    python_command = 'python ../data_gen_scripts/main_sac.py'
    ...
    for env_name in ['online-antball-v0']:
        for i in range(2):
            seed += 1
            base_dict = dict(
                default_args,
            ...
            tests.append(
                dict(
                    ...
                    save_interval=12000000,
                    log_interval=20000,
                    agentIlayer_norm=True,
                    terminate_at_end=1,
                )
            )
    for env_name in ['online-humanoid-xy-v0']:
        for i in range(2):
            seed += 1
            base_dict = dict(
                default_args,
            ...
            tests.append(
                dict(
                    ...
                    train_interval=4,
                    eval_interval=500000,
                    save_interval=40000000,
                    log_interval=20000,
                    agentIvalue_hidden_dims='"(1024, 1024, 1024)"',
                    agentIlayer_norm=True,
                    agentImin_q=False,
                )
            )
    ...
    if preset == 'P':
        ...
        for i, test_command in enumerate(test_commands):
            ...
            command = f'{pre_command} CUDA_VISIBLE_DEVICES={gpu_idx} SLURM_JOB_ID={pseudo_slurm_job_id} {python_command}{test_command}'
