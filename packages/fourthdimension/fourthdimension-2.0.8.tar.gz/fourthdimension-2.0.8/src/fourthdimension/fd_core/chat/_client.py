...
from .core import FourthDimensionAIError
from .core import HttpClient, FourthDimensionAI_DEFAULT_MAX_RETRIES
...
class FourthDimensionAI(HttpClient):
    ...
    def __init__(
        ...
        if api_key is None:
            api_key = os.environ.get("FourthDimensionAI_API_KEY")
        # if api_key is None:
        #     raise FourthDimensionAIError("未提供api_key，请通过参数或环境变量提供")
        self.api_key = api_key
        self._disable_token_cache = disable_token_cache

        if base_url is None:
            base_url = os.environ.get("FourthDimensionAI_BASE_URL")
        if base_url is None:
            # base_url = f"https://open.bigmodel.cn/api/paas/v4"
            base_url = f"https://127.0.0.1:9090"
