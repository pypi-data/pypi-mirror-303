...
parser.add_argument(
    "--intensity-only",
    action="store_true",
    help="Only ingest the intensity data from the FITS file, not polarization.",
)
...
def main():
    ...
    with db.get_session() as session:
        ...
        for fits_image in fits_file.individual_trees:
            if args.intensity_only and fits_image.identifier != "I":
                continue
            ...
            band = tilemaker.orm.Band(
                ...
                tile_size=tile_size,
                frequency=str(fits_image.header.get("FREQ", "f000").replace("f", "")),
